...
**批次 v0.5.31 之后：四图联动的三条真实缺陷 + 测试账本收口（2026-09-24）**
- 已合并到 main（按合并顺序）：
  ...
  - **#188 → 4a44290**（issue **#186**）四图时间轴联动按**逻辑索引**广播 → 混周期下把 1m 那格挤到只剩两三根 K 线的可视窗口。
  ...
  - **#192 → 60ba2fa**（issue **#183**）多图十字光标的回声改按**指针归属**判定：非空事件若既没有 `:hover` 落在本格容器上、
    也不是触屏十字光标，那它必然是自家 `setCrosshairPosition()` 的回流 —— 既不广播，也不再重写 `data-crosshair-time`。
    v0.5.31 的 #177 与 PR #189 的 v2 方案都是**按值判等**，漏掉两类真实情况：① 一格在一次联动里被连写多个值时，
    前一个值的迟到回声被当成指针上报再广播（四格互相刷值，实测指针移动不再改任何一格的时刻）；② 「移出」的 null 先落地、
    旧落点随后回流，把清除盖回去（移出后其余三格永不归零）。四格同周期时吸附是恒等映射，所以默认配置一直绿 ——
    缺陷躲在门禁后面。PR #189 因禁 force-push 走「新分支 + 关旧 PR + 删远端分支」重做为 #192
- 待合并：**#195**（issue **#185**）多图同步撤销时收掉接收格的 OHLC 浮层，并丢弃迟到的自家回流
  （接收格没有指针，浮层只能由回流立起来，而 `clearCrosshairPosition()` 不回抛任何事件 ⇒ 没有任何路径把它置回 null；
  #192 的构建上实测移出后四格时刻全 null 但页面仍留 3 个 `crosshair-time`）
- 本批新立单未开工：**#193** 同一时刻的重复广播被按值去重挡掉 ⇒ 接收格换周期后落点不自愈（本批 e2e 靠「每轮换一个横向落点」绕开）；
  **#194** `multi-chart-sync` 的四格视角同段断言在 webkit 偶发红（`跑偏 1/4`，容差已在 #188 从 2 分钟放宽到 5 分钟仍红）
- 门禁现状：`npm test` → **2025 passed / 175 files**（#188 的三例换算测试）；
  `node scripts/run-ci-e2e.mjs --list` → **417 tests in 19 files**（#188 加了混周期那条）；
  #192 的 CI 三浏览器 E2E **19m55s 全绿**；Vercel 那条是账号级构建限流，按既定口径不作判定依据
...
- **一条方法论（本批付了 twice 的代价）**：E2E 的绿只有在确认盘上的 `dist` 含本次改动之后才可信。
  `serve-static.mjs` 永不构建；而**注释级改动重建后产物哈希不变**，所以「哈希和上次一样」分不清是没重建还是内容等价 ——
  改用产物里的独有字符串核对（`grep -l '<标记串>' dist/assets/*.js`）。本批前一半「混周期偶发红」的结论全部跑在旧包上，
  据此起草的缺陷描述（「接收格停在离源格几十小时的地方」）是错的；#193 与上面这几条记录都是按新构建重测后的版本
- 版本判断：本批三条用户可见的图表行为修复（#188 / #192 / #195）合并后定 **v0.5.32**，不逐条发版；
  无数据迁移、无设置快照变更（`version` 仍为 2）
- 本地已知噪声：`period-anchor` A2 在本机负载下约 1/3 红（`回到最新` 按钮 8s 内不可见），A/B 过 main 构建同样红 ⇒ 既有 flake
- 下一项：v0.5.32 定档（版本号 → CHANGELOG → release notes → 回滚检查 → 全量验证 → tag → 部署 → 线上抽查）；
  #56 双击复位的「价格轴回自适应」那一半仍无任何测试覆盖
- 更新日：2026-09-24
