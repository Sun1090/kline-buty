...
- 下一里程碑：**v0.5.x 继续**。已排队的下一项：
  ...
  ④ `smoke*` 整族（含 40 条画线用例）不在 CI 的 15 条清单里，#154 这类测试侧改动 CI 完全不执行，
  本地才有红绿信号——与 ③ 同属「CI 覆盖盲区」，需要一起定策略（跑哪些、在哪些浏览器、多少分钟可接受）；
  ⑤ 多锚点工具连点偶发吞点（`斐波那契扩展` 全量串行 6 次里 1 次红、单跑 8/8 绿）：三次
  `page.mouse.click` 之间没有任何中间态等待，负载高时其中一次被吞 → 锚点没集满 → 不提交；
  ⑥ 移动端画完一条后不重新选工具，直接点第二下会被 `TouchDrawingGestureLock` 吞掉（桌面同一工具可连画）
...
- **#154 → main b79ca03** `test(e2e)`：画线命中族收尾——`十字线`/`多段线` 是 05d4616 漏掉的最后两处
  ...
  6 次里唯一一次红在 `斐波那契扩展`——那是排队项 ⑤ 的另一类成因（多锚点连点被吞），与像素漂移无关。
