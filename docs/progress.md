...
- 待合并：
  - **#203**（#202 的归因面，不动产品行为）A2 那条 CI 真红原先只留下一句 `element(s) not found`，
    分不清「手势没生效」还是「视角挪开了但 `atLatest` 判错」。改成把前提钉成观测量：
    拖到「可视右缘与末根的下标差 `away > 0`」为止（复用本文件的 A11 文本反查下标那件工具），
    成立之后再等按钮并把 `away` 写进断言消息；时序预算 280s→360s 同步更新注释里的加法。
    变异复验：把 45 步位移改成原地不动 ⇒ 红在前提那一步
    （`Received: "视角仍在最新（away=0）"`），而不是红在按钮上。
...
- 已合并：**#200**（收口 issue **#193**）一条 webkit/真浏览器抽测反复红过的链，根因**不在**十字光标：
  ...
  CI：三浏览器 E2E 全绿后 squash 合并 → main **63755fb**，issue #193 随之关闭。
- 本批新立单：**#199** quad 里换一格周期会把其余三格的可视窗口甩走（实测指针不动、源格时刻跳 2～3 小时，
  一次跳 24 小时）；方向是粗周期锚定后的跨度被广播出去、细周期那格装不下就按索引 clamp 到数组左缘。
  需要一次口径裁定：换周期**该不该**带动其余三格（issue 里倾向「带动，但按时间夹住、保右缘」）
- 仍开：**#194** `multi-chart-sync` 四格视角同段在 webkit 偶发红；**#191** app-shell 与 AGENTS.md 边界冲突（待产品裁定）；
  **#56** 双击复位的「价格轴回自适应」那一半至今无任何测试覆盖
- 门禁现状（main@63755fb）：`npm test` → **2030 passed / 175 files**（#198 +1、#200 +4）；
  `node scripts/run-ci-e2e.mjs --list` → **426 tests in 20 files**（新增 `e2e/period-resnap.spec.ts` 三条）；
  #198 与 #200 的 CI 三浏览器 E2E 全绿（run 35900507623 / #200 的 E2E job）
- ⚠️ **main 上有一条已知真红（已立单 #202）**：run 35899227852（main@c8cbd24）的 E2E job 红在
  ...
  #200 之后本地 3 轮 chromium 抽测全绿，**不作为结论**，继续按装载路径改动后的必测点跟
- 版本判断：**#198 与 #200 不进 v0.5.32**（tag 已经打在 c8cbd24），与下一条用户可见修复一起定 **v0.5.33**；
  不逐条发版。#198/#200 都是用户可见的图表行为修复
- 本地已知噪声：`period-anchor` A2 本机负载下约 1/3 红（见上条 #202，已升级为待查项而非噪声）
- 下一项：等 #203 合入后按它留下的 `away` 读数给 #202 定性（手势还是 `atLatest`）→ #199 口径裁定与实现 →
  #56 价格轴自适应的测试面
- 更新日：2026-09-24
