...
- 版本判断：**已定 v0.5.33**（#198/#200/#204 是用户可见的图表行为修复，#203/#205/#206/#207 是同批测试面），
  不逐条发版
- 下一项：**`localOnly → ?perf` 这条已经清完**（7 条留存理由现在全部落在具体机制上，最后一条编的
  是 `market-tape`，见 #230），roadmap 的 in-scope 队列里剩下的是 **E2E flake 的源头收敛**：
  把仍然靠「时间预算 + 像素漂移」撑着的断言换成契约式/属性式判据（`?perf` 数据契约已经给了这条路
  的入口）—— 第一步已经落成上面那份**有名字的名单**与取数方法。#191 仍需一次产品口径裁定（阻塞在决定，
  不在代码）；#222 已按 A4c 的判词改了口径（不是墙钟预算，是一次落定了的纯平移），现在等的是一次带
  **逐拍轨迹**（#232）的 CI 红：它才能回答「第几拍开始挪」。
...
- 更新日：2026-09-24（`localOnly → ?perf` 迁移收尾轮 · 移动端一族 + A4c 观测窗按落位收尾 +
  方法论 6 的时钟用密采样重定过 + 账本里最后一条编造理由被抓出来 +
  三浏览器偶发改成有名单的统计，#222 的 A4c 口径随之更正）
