...
- **评审中 · 移动止损 + 结算链路合并（分支 feat/v05-trailing-stop，PR #111）**：
  ...
  - 验证：typecheck / lint 0 err / audit:i18n / unit **1849**（171 files，+34）/
    chromium E2E `tpsl-guard` **5/5** + `limit-orders` **4/4**；变异校验：移除写回 → hook 4 红、
    移除 settled 过滤 → 1 红、禁用推进 → 单测 3 红 + E2E 2 红、去掉价格改善 → 5 红（均已还原并 diff 确认干净）
  - **过程中修掉两个真实缺陷**：
    1. 价位编辑器加第三格后 320px 下三个输入被挤出视口并撑出横向滚动（违反移动端规则）→ 价位行改 `grid auto-fit`
    2. 结算诚实化后暴露：限价单跨过价差时按挂单价成交（市价 50,700 挂 60,800 买入 → 开仓价 60,800、
       默认止损 59,644 立刻被击穿 → 刚开的仓当场亏平）。改按交易所口径给价格改善
       （买单价高于市场价即按市场价成交、卖单镜像），名义额/手续费/余额判定/持仓开仓价统一用成交价；
       纯函数新增 `fillPrice`，`planFills` 增加成交价取值参数。据此更新 `limit-orders` E2E 与撮合 hook 单测
  - 另记录（未在本批修）：打开「仓位」侧栏后再点顶栏 More 下拉的「交易流水」，点击被侧栏内下单表单
    按钮拦截至超时（侧栏 z 90、头部下拉 z 96，层叠上下文判定待实测）→ 独立批次处理；
    该 E2E 暂改为读 `paperOrders` 存储，不再同时叠两个面板
