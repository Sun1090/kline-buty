...
- 待合并（四条的 E2E 都在跑）：**#235**（`smoke-drawings` 回归通道那条改成点真实画线像素 —— 那次质心
  点击在 CI 上是**硬 failed**：run 36004500928 初始 + 两次重试全卡在「删除」等不到）、**#236**（启动族
  去掉 `goto → evaluate(localStorage.clear()) → reload` 的第二遍导航：#233 名单里 8 条 webkit 偶发有
  4 条死在这次 reload 上，名单窗口外还有第 5 条）、**#237**（水平射线那条的「所在行」改按当前像素重扫
  —— ±8px 的带子扛不住 +1078ms 那次 15px 重缩放）、**#238**（趋势线拖整线的起手点从质心换成真实像素
  —— 这条在 CI 窗口里从没红过，是按同族写法扫出来的第三个点，不记进名单）。
- 已合并（本轮迁移批之后）：**#230 → 4c3e4a0**、**#231 → 26911f6**、**#232 → 9e36ad0**、
  **#233 → 6d0cc09**、**#234 → 7d625ff**。迁移批本身（**#212 / #215 / #216 / #218 / #219 / #221 /
  #223 / #224 / #225 / #226 / #227 / #228 / #229 / #231**，#213 与 #214 关闭后由 #215、#218 重落）
  在 `26911f6` 之前就全部落地了。#234 随带一行产品代码：`ChartView` 上那个
  `data-value-time` 是测试钩子，攒进下一个 release，不为它单独发版。
  **#230 这里要改口**：原先写它「首轮 E2E 红在 `[webkit] smoke-drawings` 趋势线拖拽」，现在取不到任何
  支撑 —— 重跑后留下的 attempt 2 日志里，那格唯一的 webkit flaky 是 `alerts-features:101 E7`，死在
  启动那次 reload 上（正是 #236 要去掉的东西）；attempt 1 的日志已被重跑覆盖、取不回来，而
  `smoke-drawings.spec.ts:2100` 在能读到的 11 份作业日志里一次都没出现过。这句话按「无法复核」处理，
  不要往下抄。
- **CI 上的三浏览器偶发，现在有名单了（2026-09-24，口径=最近 10 次 CI 的 E2E 作业日志）**。
  ...
  （普通 `gh api` 会因响应里有终端转义序列直接拒绝返回）。数出来的分布：
  `A4c quad-switch-no-drag` **webkit 3 次**、`B1 indicator-crosshair` **firefox 2 次**、
  `smoke-drawings` 回归通道 **1 次（硬 failed，不是 flaky）**、水平射线 1 次、`smoke-depth` 快捷下单 1 次、
  `alerts-features` E1/E4/E6 各 1 次。
  **A4c 那三次要改口**：不是 #222 记的「墙钟预算用尽」—— 判词显示观测窗每次都收尾了（`末次起连续 19 拍
  一致`、`区间缺失 0 拍`），挪的是**接收格**、形态是**纯平移**（跨度一字未变）、方向**朝更早**、量级
  **2/3/4 根 5m**。本机 webkit `--retries=0` 复现不出来：空闲 8/8 绿；再开 10 个 CPU 抢占进程复跑 6/6 绿，
  但**那次抢占没生效**（单次耗时与空闲同档 17s vs 17s，这条规格不吃 CPU），所以它只能记成「没复现」，
  不能记成「负载下也不复现」。另一条没排除的混淆项：本地这条规格的可视跨度是 **2700s**，CI 报的是
  **26400s**，两边几何不是一套。处置顺序已写进 issue #222：先靠 #232 的逐拍轨迹拿「第几拍开始挪」，
  再决定是修规格的基线取值还是查广播；**不把这张表现做成 CI 允许名单** —— 允许名单会静默吞掉真缺陷，
  「超名单即红」又会把每次合并绑在 runner 的脾气上。
  **这 12 条现在的去向**：`B1 firefox 2 次`由 #234 换掉前提（那两条红的原文是
  `expect(rightText).not.toBe(leftText)`，**不是**「0.9 处没有十字光标」—— 后者是本地另测到的一件事，
  两者别混着抄成因果）；`alerts E1/E4/E6 3 次` + `smoke-depth 快捷下单 1 次`归 #236（同一次启动 reload；
  名单窗口外还有 `alerts E7` 一条，run 36011943582）；`回归通道 1 次`归 #235；`水平射线 1 次`归 #237；
  剩下 `A4c webkit 3 次`继续在等一次带逐拍轨迹的 CI 红（#81，轨迹由 #232 接上）。
- **一条可复用的判据：画线质心不能当抓手（2026-09-25 实测）**。`findDrawnLineCenter` 打的是「所有画线
  墨迹」的均值（选中态那圈蓝色也算进去）。本机 1280×800、`?perf=600`、与用例同一套流程的探针量它到
  **最近真实墨迹**的距离：回归通道 chromium 0.6–0.8px（4 次）／webkit 3.5、8.5、8.8、9.6px（4 次里
  3 次越过命中阈值 `HIT_THRESHOLD_PX = 8`，点质心 3/4 选不中 —— 这就是 CI 那个失败形态的复现）；
  趋势线 chromium 0.4–2.4px／webkit 0.6、7.9、7.4px（**单条线**也能偏到离抓空只差 0.1px）。
  偏进去的那部分墨迹到底是锚点圆点、价格标签还是抗锯齿，**没有拆开查**；判据只到「质心可以落在阈值
  之外」这一层，所以修法一律是「扫真实像素 + 按结果重试」（`hitDrawnPixelUntil`，第四参可以给一段
  拖拽 dispatch），而不是给质心加容差或加等待。
- 已合并：**#204**（收口 issue **#199**）quad 里换一格周期会把其余三格的可视窗口甩走。
    ...
    （指针/滚轮/触摸/键盘落在这一格之后，它报出的视角才广播），程序化落位只发布状态（A11、atLatest、锚定基准）。
    插桩坐实三条：① 锚定按 `PERIOD_MS[新周期]` 算根数、用的却还是旧周期那片数据（`ANCHOR gapMs=300000
    target=3600000 → {from:1427,to:1472}`）⇒ 视角当场压成 1/比值；② 数据晚一拍到位后整窗装载不重落
    ⇒ 同一索引区间在新周期上是比值倍的时间（e2e 实测 **14700s → 172800s**，5m→1h 正好 12 倍）；
    ③ 这两次外加装载收尾那次「不可信补读」（实测能报出 `t=同一根` 的零宽区间）都被当作用户改动广播出去。
    修法：`switchIntentRef` 记住换周期那一刻的**时间意图**（右缘 + 跨度），两拍都按当前这片数据自己的间距落位，
    用户重新接管视角时作废；`viewOwnedByUserRef` 记归属。**不能用 try/finally 罩同步开关** ——
    `setVisibleLogicalRange` 的补发回调是逐帧异步到的（插桩确认它在 finally 之后才落），第一版就是这么写、
    然后被自己的变异测试否掉的。
    验收 `e2e/quad-switch-no-drag.spec.ts`（进 CI 账本，+1 file）：四格统一 5m → 拖进历史（前提钉成
    「四格都出现回到最新」）→ 只换 SOL ⇒ **观测 14s**、其余三格起止漂移合计 ≤ 3 根。
    三条变异各自红在自己那条断言上（撤广播门 ⇒ `BTCUSDT 被挪走 162600s`；去掉第二拍重落 ⇒ 跨度 12 倍；
    锚定读 PERIOD_MS ⇒ 右缘跳 6 小时）。**第一版规格写成「采样一次」时撤掉整个广播门照样绿** ——
    广播发生在换周期之后几百毫秒，t=0 那一刻确实没动；判据因此是观测窗 + 「换的那一格至少报出过一次不同区间」的自检。
    顺带给 A11 可视范围条补 `data-visible-from/to`（原始秒）：按 locale 文本反查下标跨月/跨年做差不可靠。
    **线上复验**（v0.5.33 发布后，生产域名 1440×900 真 chromium，`?perf=1500` 四格统一 5m 再只换 SOL⇒1h）：
    其余三格 14s 观测窗内**逐字未动（最大漂移 0s）**，换的那格自己确实报出过不同区间（自检通过，不是空转），
    且它的跨度 `3300s → 3600s`（比值 1.09）—— 按索引锚定会给 12 倍，这正是时间意图锚定要证的数字。
- 已合并：**#203**（#202 的归因面，不动产品行为）A2 那条 CI 真红原先只留下一句 `element(s) not found`，
    分不清「手势没生效」还是「视角挪开了但 `atLatest` 判错」。改成把前提钉成观测量：拖到「可视右缘与末根的
    下标差 `away > 0`」为止，成立之后再等按钮并把 `away` 写进断言消息；时序预算 280s→360s。
    变异复验：把 45 步位移改成原地不动 ⇒ 红在前提那一步（`Received: "视角仍在最新（away=0）"`）。
    本机空闲时拖一次就 away=63 且 3s 内不回弹 ⇒ #202 的症状没能复现，本条的产出是**下一次 CI 红能直接读数**。
    合并 → main **a754f4d**（远端分支已删）。
- 已合并：**#200**（收口 issue **#193**）一条 webkit/真浏览器抽测反复红过的链，根因**不在**十字光标：
    `ChartView` 的 `prefixSame` 只比**末根时刻**判定「同一批数据尾沿长了一根」，
    而换周期恰好能伪装成它 —— 15m 与 1h 两片等长序列的末根常落在同一个对齐边界上
    （1/4 的时候重合，1m↔5m 是 1/5），于是新周期的那片数据**根本没进图表**，
    只把最后一根 `updateCandle` 盖上。界面看是「换了周期图没怎么变」，
    十字光标则永远按旧序列吸附（旧值还在继续被广播）。
    插桩实测两拍：`load period=1h wholeWindow=true gap=900` → `pass period=1h wholeWindow=false gap=3600`，
    此后该格再没有 `setCandles`。第一拍已经用掉 `keyChanged`，第二拍只剩 `prefixSame` 决定走不走整窗装载
    —— 这就是它只在「数据比 period 晚到一拍」时踩得到的原因。
    修法两条：① `prefixSame` 比**两根**；② 整窗装载后按 `externalPendingCrosshairRef` 就地重落一次外部十字光标
    （广播链路两处都按值去重，源格再报同一个时刻也不会重发，所以数据换了必须本格自己重算）。
    只修②时 e2e 仍红（数据没换，重落还是按旧序列吸附），两条各被单测钉住。
    合并 → main **63755fb**，issue #193 随之关闭。**#204 修的正是 #200 暴露出来的下游**：数据真换进来之后，
    第二拍那个「按索引保住的视图」才第一次被人看见。
- 已合并（本批测试面，不动产品行为）：
  - **#205**（内部条目 #56）合并 → main **5a1b5ea**。双击复位原先只有「时间轴回去」那一半有测试，
    价格轴那一半连观测量都没有。`adapter` 补 `publishPriceAutoscale()` 把 `priceScale('right').options().autoScale`
    抄到 `.chart-container[data-price-autoscale]`，新 `e2e/price-autoscale-reset.spec.ts` 用 CDP 真触摸跑
    「捏合 ⇒ off → 双击 ⇒ on」，每条断言的消息里写清它钉的是哪个调用点。
    子图那条路径（`priceScale('sub',1)`）**故意不发布** —— 它不是同一根轴，混进去会让这个属性说谎。
  - **#206**（issue **#194** 的判词面）合并 → main **3d4c0f2**。多图同步原来只报一句「动了 3/3，跑偏 1/4」，
    看不出哪一格、偏多少、是两缘同向挪还是被压窄 —— 而这三种形态要查的地方完全不同。
    判词补成每格相对锚点的两缘位移 + 本格跨度，按这条把 #194 定性成**接收格落地后被回声往返覆盖**
    （三格逐字贴锚点、只有一个接收格两缘各偏 4~6 分钟且跨度少 2 根；取整只会 ±1 根且不会两缘同向，
    源格 Δ0..0 也排除广播侧）。同批给 `period-resnap` 补回「落点离源格不超过一根本格周期」这条不变量，
    并诚实写明它**不是 #199 的门**（A/B 显示 #204 之前它也成立），#199 的门是 `quad-switch-no-drag`。
  - **#207** 合并 → main **b1b941d**。把 testing-library 的 `asyncUtilTimeout` 从默认 1s 抬到 8s，
    收掉一条长期被记成「机器忙」的负载 flake（详见下面「方法论」）。
- **本批关闭的 issue：#193、#194、#197、#199、#202。** 唯一仍开的是 **#191**
  （app-shell 与 AGENTS.md 产品边界自相矛盾）—— 它阻塞在一次产品裁定上，不在代码，roadmap 也已无待做项。
- ⚠️ 编号歧义（本条记录自己踩过）：progress 里长期写的「#56 价格轴自适应无覆盖」是**内部条目号**，
  而 GitHub 的 #56 是 v0.5.11 的 release PR；`gh issue view 56` 会返回一个 MERGED 的无关对象。
  今后在文档/PR 标题里写 `(#NNN)` 之前，先确认它在本仓库 tracker 里是**同类**对象。
- **#202 的读数结论（不是 flake，是前提没被保证）**：#203 把它拆成两种互斥读数之后，
  main@a754f4d 与 main@839d28c 两次 CI 的三浏览器 E2E **全在 attempt 1 绿、零重试**，
  也就一次都没打印出「视角已离开最新（away=N）却没出现回到最新」这句 —— 那条才是产品态错。
  本机另在 3d4c0f2 抽样 chromium × 3 全绿（每次约 3.2s）。**决定性的一点是 a754f4d 本身就把它扭成绿了**，
  而那是纯测试改动、不动产品代码 ⇒ 原红的含义是「8s 内前提还没成立」，不是「按钮该出现却没出现」。
  原来 `for(5 次){ if (按钮可见) break; 拖一次 }` + 死等 8s 在 CI runner 上不构成前提保证。
- **#194 的因果计数**：#204 之后（main@3d4c0f2）同规格同配方 `--project=webkit --repeat-each=24 --retries=0`
  ⇒ **48 轮全绿 0 红**；对比 #204 之前（main@a754f4d）36 轮红 5 轮（14%）。若无因果，纯随机全绿的概率约 0.86^48 ≈ 0.08%。
- 门禁现状（main@00055f9，release 工作区实测）：`npm test` → **2034 passed / 175 files**；
  `node scripts/run-ci-e2e.mjs --list` → **432 tests in 22 files**；`tsc -b --noEmit` 与 `eslint` 干净；
  PR #208 的 CI 六项（含三浏览器 E2E）**全部 attempt 1 绿**
- **CI 之外那一组也刚核过（2026-09-24，main@56830f3）**：`e2e/ci-specs.json` 的 `localOnly` 12 个文件
  是刻意不进 CI 的（都要真实行情流：REST/aggTrade WS/线上蜡烛），账本只保证「登记过」，不保证「还跑得动」——
  这个仓库已经被「CI 没覆盖 ⇒ 静默腐化」咬过两次（#12、#52）。本机补跑：
  `npx playwright test --project=chromium --workers=1 --retries=0 <那 12 个文件>` ⇒ **132 passed (5.5m)**，零红零跳过。
  前提是 `api.binance.com/api/v3/ping` 通（本机 200）；CI runner 出网不可靠正是它们留在 localOnly 的理由。
  ⚠️ 这一组**没有任何自动化会告诉你是谁坏了**，下次动 `smoke*`/`feature-gaps*`/`mobile` 覆盖的路径后要手动再跑一遍。
- **一条被自己否掉的候选门（记下来免得有人重做）**：想给 #200 补一条 e2e 窗口面不变量
  （「换周期后这一格的可视窗口起止必须重排到新周期的网格上」，15m→1h，并按「换之前起点不在 1h 网格上」设前提）。
  写完在 chromium 上绿 —— 但把 `prefixSame` 退回到只比末根那个坏版本上，**它照样绿**（829ms），所以它不承重，已删。
  #200 真正的门是单测 `末根时刻重合的换周期必须整窗换新数据，不能只盖最后一根`：它在同一个变异下确实红，
  而且比 e2e 强 —— 单测能**造出**「两片序列末根时刻重合」这个 1/4 概率的巧合，
  e2e 只能守着真实时序等它出现，等不到就只是碰巧没测。
  换句话说：凡是失效条件本身是个小概率对齐的缺陷，e2e 里加「等相位」的前提也换不来牙齿，直接写单测。
- 方法论（本批新增，都是可复用的判据而不是一次性结论）：
  1. **「负载 flake」要把预算当被测对象**，不要停在「机器忙」这句话上。三步：调低到能**精确复刻**那条红
     （1100ms 复现了 `Unable to find [data-testid="sentiment-panel"]`）⇒ 证明配方有效且因果在此；
     开到极大去**量**真实需求（整条测试极限负载下 4.6s）⇒ 拿到上界；再拿「永远不出现的钩子」当对照组
     证明真失败仍红且**报对判词**（8006ms 报的是 DOM dump 而不是 `Test timed out`）。中间那个数才是被测量出来的。
  2. **抬上限不改「等没发生」类断言的语义**：`waitFor` 在回调不抛异常时即刻 resolve，
     所以 `waitFor(() => expect(spy).not.toHaveBeenCalled())` 第一次轮询就过 —— 全仓只有这一类需要排查，排查过即可放心全局改。
  ...
  5. **一条断言「精简后红、逐字版绿」不等于找到了因果。** sentiment-live 拆出去时我删掉三条标题断言，
     ...
     不把猜测写成机制 —— 写错的机制比没写更贵。
  6. **「现扫坐标」必须配「按结果重试」，否则扫到的还是上一个刻度下的位置。** `?perf` 的价格轴会在
     蜡烛首次上屏之后约 **0.42s 再做一次离散重缩放**：画线质心一次性从 `y=422.3` 跳到 `y=449.9`
     （**27.6px**）然后才稳，3/3 复现。（这里原先还写着「线上 0/3 有跳变，是网络把测试推过了那个时刻」——
     那是把一次粗采样当成了机制，密采样重测见本条末尾。）于是「扫一次质心 → 照着它 tap/长按」在 perf 下会按在空白上：
     画线**创建**照常成功、localStorage 里两点俱在，只有随后的**编辑/长按**整条落空。
     移动端两条用例就是这么红的（一条四个增量全 0，一条 `mobile-text-editor` not found）。
     修法**不是**加大超时或加固定 sleep：一次性迟到的重缩放，睡多久都证明不了它已经过去；
     要按结果收敛 —— 没挪动／没打开就重扫当前像素再来一轮，并把「第几轮成的」写进判词。
     验证重试不是装饰，就把上限压成 1：整线拖动 8 跑红 6（判词 `{"used":1,"dP0":0,"dP1":0}`），
     恢复 3 轮后 8 跑全绿；长按文本不重试基线是 live 8/8、`?perf` 7/8，加重试后 `?perf` 16/16。
     同一条 `?perf` 契约也否掉了一个想法：`waitCandlesRendered` 只是「像素有了」的门，
     **不是**「图表落定了」的门，凡后面还要照像素做手势的都欠它一个收尾条件。
     **这条契约的时钟与边界，2026-09-24 用密采样重新量过**（本地 `main@8d220f9` 的 dist，chromium，
     一条固定价位的水平线放上屏那一刻记作 t=0，每 120ms 采一次质心、采 6s）：
     线上真实行情 **46 拍零跳变**（`y=362.5 → 364.5`，总变化 2px）；`?perf=600` 前 934ms 稳在 `332.5`，
     **t=+1078ms 一次性跳 15px 到 347.5**，此后 4.8s 再没动过。两个结论因此改掉：
     ① 线上不是「被网络推过了那个时刻」，而是**这条窗口里根本没有那次重缩放**；
     ② perf 的 pop 是**一次性、约 1s**（原来的 0.42s 是相对 `waitCandlesRendered` 的时钟，两者不矛盾，
     但没写参照点的「几秒」就是这次要改的毛病）。**成因仍未定** —— 只跳一次 ⇒ 不是每秒的合成 tick，
     没有插桩就不写成机制。产品口径按现状留着：主路径看不见它，`?perf` 压测模式会看见一次 pop，
     测试侧一律按结果收敛，不为此改产品代码。
     顺带记一条**被否掉的量法**：在 prod 域名上想用「窄带像素扫横向网格线行号」代替画线质心，量不出来——
     4~6px 宽的采样带连蜡烛实体的连续游程都凑不齐，`?perf=200/600/1500` 与线上四组里三组只能报
     「不下结论」，剩下那组的「跳变」其实是空图→有数据那一次落位。这条线上量法别再重试，画线质心是唯一可信标记。
- 版本判断：**已定 v0.5.33**（#198/#200/#204 是用户可见的图表行为修复，#203/#205/#206/#207 是同批测试面），
  不逐条发版
- 下一项：**flake 源头收敛这一轮已经把名单清到只剩 A4c 一处**。12 条的去向见上一条；#235 / #236 /
  #237 / #238 四条 PR 在等 CI，合并之后还会响的只剩 `quad-switch-no-drag`（A4c）那 3 次 webkit，
  而它等的是「一次带**逐拍轨迹**的 CI 红」—— 轨迹已由 #232 接上（#81），不去改容差、也不建允许名单。
  #191 仍需一次产品口径裁定（阻塞在决定，不在代码）。同族写法还剩两处没动：`smoke-mobile:285` 与
  `:391` 都把画线质心当**触屏抓手**用，但它们只在 chromium 跑，而实测 chromium 上质心离真实墨迹
  0.4–2.4px（阈值 8px），余量比 webkit 那三条大得多 —— 先记下，等它真红或有整轮 CI 预算再收。
  这条迁移本身的账：本轮把它从计划做成了事实，并且**证伪了大半条账本**：
  原先 12 个挂 `localOnly` 的文件里，只有 5 个的理由经得起实测，其余 7 个的「依赖实时行情」是抄来的。
  CI 覆盖因此从 **432 tests / 22 files** 推到 **801 tests / 32 files**（#212 → #215 → #218 → #216 →
  #221 → #226 → #227 → #228，逐组都在合并后的 main 上量过），`localOnly` 从 12 个文件缩到 7 个。
  ...
  每条拆分各留一个 live 尾巴（`smoke.spec.ts` 拆出 `smoke-live`、`smoke-depth` 拆出 `sentiment-live`、
  `smoke-mobile` 拆出 `smoke-mobile-live`），所以 `localOnly` 一直是 **7 个文件**（进一个出一个），
  而 CI 落到的数字现在是在 main 上跑 `node scripts/run-ci-e2e.mjs --list` 量出来的，不再是预告值：
  #226 合完之后是 **690 tests / 30 files**，#227 合完之后是 **744 tests / 31 files**，
  #228 合完之后是 **801 tests / 32 files**（+19 例 × 3 浏览器 = +57，别按例数读；这三组都是在合并后的
  main 上跑 `--list` 量出来的，不是预告值）。七条 live 尾巴分别是：
  `chart-ready`（要的就是真实 klines 请求迟到）、`feature-gaps-live`（全市场 ticker 行）、
  `market-tape`、`smoke-live`（资金费率）、`sentiment-live`（fapi 数值）、
  `smoke-mobile-live`（`market-row-*` 行数与点行换交易对）、`mobile-live`（同一段点行换交易对）——
  `mobile` 一族这一轮已逐条判完：19 例里只有「点榜单行换交易对」真离不开线上，已拆出
  `mobile-live.spec.ts`、其余 18 例迁 `?perf` 进 ci（#228），方法论 6 那条坑也确实踩到并按结果收敛修掉。
  ⚠️ `market-tape` 这一条原先两处（账本 + 这里）都写着「aggTrade WS」，那是**编造的机制**：
  `grep -rn aggTrade src/` 零命中，Tape 是 `useRecentTrades` 每 5s 轮询 REST 累积出来的，
  进不了 CI 的真实原因是该 hook 在 `isPerfMode()` 下早退（`src/hooks/useRecentTrades.ts:18`）。
  结论没变（它确实要线上），理由是抄的 —— 改掉它的是 #230。
  方法论沿用这个仓库为此付过的两次账：PR **#182**（把假绿改写成能红）、PR **#187**
  （清掉 32 个死钩子并把账本本身变成 CI 的红条件）—— 账本只保证登记过，不保证还跑得动；
  这一轮再加半句：**账本也保证不了「理由」是真的**，`ci-specs.test.ts` 只查每条非空，
  一条写得像真的、但从没人去 grep 过的机制会一直过关（`market-tape` 就是例子）。
...
- 更新日：2026-09-25（三浏览器偶发的收敛轮 · 名单里 12 条各归到 #234/#235/#236/#237 四条 PR、
  只剩 A4c 等在跑红 + 「画线质心不能当抓手」这条判据量出来了（阈值 8px，实测可偏到 9.6px）+
  #230 那行「首轮红在趋势线拖拽」按无法复核改口 + 上一轮 `localOnly → ?perf` 迁移收尾记录：
  移动端一族 + A4c 观测窗按落位收尾 + 方法论 6 的时钟用密采样重定过 +
  账本里最后一条编造理由被抓出来）

**里程碑 v0.5.33 发布完成（2026-09-24）**
- 版本号：**0.5.33**（package.json / package-lock 根两处 / index.html meta `app-version`，共 4 行改动）
- 分支：`release/v0.5.33`；发布 PR：**#208**（squash 合并 → main **00055f9**，合并后即删远端分支）
- tag：**v0.5.33 @ 00055f9**，由 **Release Tag workflow 自动创建**（run 35938496388 success）——
  本仓库的 `release-tag.yml` 在 push main 时按 `package.json` 的 version 幂等打轻量标签，
  所以 RELEASE_FREEZE 清单里的「tag → push tag」一步在 CI 侧自动完成，本地 `git fetch` 即回收；
  本仓库惯例是 **tag-only，不建 GitHub Release**（`gh release list` 历来为空，v0.5.32 同样如此）
- 部署：Deploy to GitHub Pages run 35938496277 **success**；线上 `meta[app-version]` 实测已是 **0.5.33**
  （部署前基线为 0.5.32，构成真实的前后对照）
- 线上抽查（生产域名真 chromium 1440×900，`?perf=1500`，按本版主题做）：四格统一 5m 后只换 SOL⇒1h，
  其余三格 14s 观测窗内**逐字未动（最大漂移 0s）**；换的那格自检通过（确实报出过不同区间），
  跨度比 **1.09**（按索引锚定应为 12 倍）⇒ #199/#204 的修在线上成立
- 本版收录见上「批次 v0.5.33 已发布」条与 CHANGELOG 的 `[v0.5.33]` 段
- 迁移与回滚：`SNAPSHOT_VERSION` 仍为 **2**（`src/utils/settingsSnapshot.ts:13`），无数据迁移；
  回滚 = revert release 提交 + 删 tag `v0.5.33`
- 更新日：2026-09-24

**里程碑 v0.5.32 发布完成（2026-09-24）**
- 版本号：**0.5.32**（package.json / package-lock 根两处 / index.html meta `app-version`）
- 分支：`release/v0.5.32`；发布 PR：**#196**（squash 合并 → main **c8cbd24**，合并后即删远端分支）
- tag：**v0.5.32** @ c8cbd24（Release Tag workflow run 35899227765 success）；Pages 部署 run 35899228198 success
- 本版收录（批次主题：**四图联动现形的三条真实缺陷 —— 默认四格同为 1m 时它们全都隐身**）：
  - **#181 → 87c11c1** 移动端双击复位去掉「几秒前的像素快照」基线，改按可视时间跨度断言
  - **#182 → d67e024** `period-crosshair` 改按同一帧快照比对源格当前时刻（半周期容差在四格同周期时等于要求逐字相等）
  - **#187 → 5a68b55** 零引用 `data-testid` 清点：24 个控件补真实断言、32 个死钩子删除、账本变 CI 红条件
  - **#188 → 4a44290**（issue #186）四图视角联动改按**时间**广播，混周期不再把细周期那格压扁；
    顺带修正流水定位把秒当索引喂给图表
  - **#192 → 60ba2fa**（issue #183）多图十字光标的回声改按**指针归属**判定（`:hover` + 触屏），不再按值判等
  - **#195 → 3e5747d**（issue #185）接收格的 OHLC 浮层随同步一起撤销，并丢弃撤销之后才到的自家回流
  - 已知遗留随版记录：**#193**（本批由 #200 收口）、**#194**
- 定档门禁（最终工作区即 release 提交 c8cbd24）：
  - `npm test` → **2025 passed / 175 files**；`node scripts/run-ci-e2e.mjs --list` → **417 tests in 19 files**
  - PR #196 CI → Typecheck+Lint / Unit+Coverage / Production build / CodeQL / KB validation /
    **E2E（deterministic, 3 browsers）全绿**；Vercel 那条是账号级构建限流，按既定口径不作判定依据
- 部署与线上抽查（Pages run 35899228198 之后，真 chromium 1280×800 打 https://sun1090.github.io/kline-buty/?perf=1500）：
  - `meta app-version=0.5.32`、`data-candles=1500`、`/api/v3/ping` 请求数 **0**、`pageerror` **0**
  - A11 可视时间范围条：`09/23 17:16 — 09/23 18:06`（#176 修的就是「日常数据量下这条在线上完全不出现」）
  - **混周期四图**（1m/5m/15m/1h）hover 源格：四格 `data-crosshair-time` 分别是
    `1790184900 / 1790184900 / 1790184600 / 1790186400` —— **每一格都落在自己周期的整数倍上**，
    OHLC 浮层 **4** 份（一格一份）
  - **指针移出后**：四格时刻全 `null` 且 `[data-testid="crosshair-time"]` 元素数 **0**
    —— 对照 v0.5.31 线上实测「四格全 null 但页面仍留 2 个浮层」，#185 这条在线上闭环
- 风险 / 回滚：本版含 `ChartView` 多图回调、`cull.ts` 换算与装载判定的行为改动，
  回滚方式为 revert release 提交并重打 tag；无数据迁移、无设置快照变更（`version` 仍为 2）
- 更新日：2026-09-24
