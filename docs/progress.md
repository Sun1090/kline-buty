...
- 当前工作：**A4c quad-switch-no-drag 的 WebKit 偶发仍未复现**。本机静态构建 WebKit、`--retries=0
  --repeat-each=5` 于 2026-09-25 连续 **5/5 通过**，单次约 16.7–17.7s；这只能记为「当前环境未复现」，
  不能外推为 CI 负载下不复现。下一步仍是等待同 commit 的 CI 逐拍证据，确认是观测竞态还是产品视角真的平移。
- 已合并的 E2E 收口批：**#234 / #235 / #236 / #237 / #238**；已合并的证据与文档批：**#230 / #231 /
  #232 / #233 / #239**。重复的进度 PR **#242** 已关闭，不再作为当前状态来源。
- 当前分支：`feat/a4c-e2e-stability`。本轮尚未改动产品代码；只完成 A4c 本机 WebKit 复验，待取得新 CI 证据后再决定是否修改规格或产品逻辑。
