...
- 待合并：**#229**（实验：去掉 A4c 的 firefox skip，让 CI 判它是否已过期）、**#230**
  （`market-tape` 的 `localOnly` 理由写的机制在仓库里不存在 —— `aggTrade` 全仓零命中，真实原因是
  `src/hooks/useRecentTrades.ts:18` 的 `isPerfMode()` 早退；清单未动，仍是 ci 32 / localOnly 7）。
  本轮迁移批已全部落地：**#212 / #215 / #216 / #218 / #219 / #221 / #223 / #224 / #225 / #226 / #227 / #228**
  （main 现 @ `8d220f9`），#213 与 #214 关闭并由 #215、#218 重落。
  **#222 仍未定性**，但它不再阻塞合并：取证改由 #226 的新判词接手 —— 下次 CI 红会直接报出
  「第几拍落位」，不用再猜装载有没有被 14s 墙钟切断（详见 issue 第 5、6 条）。
...
- 下一项：**`localOnly → ?perf` 这条已经清完**（7 条留存理由现在全部落在具体机制上，最后一条编的
  ...
  的入口）。#191 仍需一次产品口径裁定（阻塞在决定，不在代码）；#222 仍待一次带新判词的 CI 红。
