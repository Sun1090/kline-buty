...
- 待合并：**#227**（`smoke-mobile` 一族迁 `?perf`，19 例里 18 例进 CI，见方法论 6）。
  本轮迁移批已全部落地：**#212 / #215 / #216 / #218 / #219 / #221 / #223 / #224 / #225 / #226 均已合并**
  （main 现 @ `1076ba8`），#213 与 #214 关闭并由 #215、#218 重落。
...
- 方法论（本批新增，都是可复用的判据而不是一次性结论）：
  ...
  6. **「现扫坐标」必须配「按结果重试」，否则扫到的还是上一个刻度下的位置。** `?perf` 的价格轴会在
     ...
     （**27.6px**）然后才稳，3/3 复现；线上数据 **0/3** 有跳变 —— 不是 perf 更稳，是网络请求本身
     把测试推过了那个时刻。于是「扫一次质心 → 照着它 tap/长按」在 perf 下会按在空白上：
...
- 版本判断：**已定 v0.5.33**（#198/#200/#204 是用户可见的图表行为修复，#203/#205/#206/#207 是同批测试面），
  不逐条发版
- 下一项：**`mobile.spec.ts` 一族按同样程序迁 `?perf`**（账本上最后一条「理由是数出来的」）。
  #191 仍需一次产品口径裁定（阻塞在决定，不在代码）；#222 仍待一次带新判词的 CI 红。
  ...
  （+18 例 × 3 浏览器 = +54，别按例数读）。七条 live 尾巴分别是：
  ...
  `smoke-mobile-live`（`market-row-*` 行数与点行换交易对）、以及仍未逐条判定的 `mobile` ——
  它的理由（「18 处 goto('/') 走线上数据」）只是数了一下 goto 的个数，跟这轮被证伪的那几条同一种形状；
  按 `smoke-mobile` 的经验（19 例里 1 例真吃线上）它大概率也能迁，但要先跑 live 基线、再只换 URL 数红，
  并且**准备好踩方法论 6 那条**：这一族全是「扫像素 → 做手势」。
...
- 更新日：2026-09-24（`localOnly → ?perf` 迁移轮 · 移动端一族 + A4c 观测窗按落位收尾）
