...
**批次 v0.5.32 之后：换周期时「那一格的数据其实没换」被抓出来（2026-09-24）**
- main 现状：**v0.5.32 已发布**（tag `v0.5.32` @ c8cbd24），其后合入 **#198 → a4b653f**、**#200 → 63755fb**、
  **#203 → a754f4d**
- 待合并：
  - **#204**（收口 issue **#199**）quad 里换一格周期会把其余三格的可视窗口甩走。**口径裁定：既不选 A 也不选 B**
...
- 仍开：**#194** `multi-chart-sync` 四格视角同段在 webkit 偶发红 —— #204 掐掉程序化落位的外传后，
  混周期互相回声压窄的链路已断，但**拖拽**之后谁该让位还没定口径；**#191** app-shell 与 AGENTS.md 边界冲突
  （待产品裁定）；**#56** 双击复位的「价格轴回自适应」那一半至今无任何测试覆盖
- 门禁现状（main@a754f4d）：`npm test` → **2030 passed / 175 files**；
  `node scripts/run-ci-e2e.mjs --list` → **426 tests in 20 files**。
  #204 分支实测：**2033 passed / 175 files**、**429 tests in 21 files**（+3 = 1 条用例 × 3 浏览器）
- ⚠️ **main 上有一条已知真红（已立单 #202）**：run 35899227852（main@c8cbd24）的 E2E job 红在
  `e2e/period-anchor.spec.ts:94` A2「回看历史后 `await expect(back).toBeVisible({timeout:8000})`」，
  三次尝试（含 2 次重试）全红。这条此前只被记成「本机负载下 1/3 红的既有 flake」，CI 红说明它不是本机问题。
  #203 合入后，下一次 CI 红会直接带 `away` 读数；#204 已 rebase 到 a754f4d，它的 E2E job 就是第一个读数的地方
- 版本判断：**#198/#200/#203 不进 v0.5.32**（tag 已经打在 c8cbd24），与 #204 一起定 **v0.5.33**；
  不逐条发版。#198/#200/#204 都是用户可见的图表行为修复
- 本地已知噪声：`period-anchor` A2 本机负载下约 1/3 红（见上条 #202，已升级为待查项而非噪声）
- 下一项：等 #204 的 CI 三浏览器绿 → squash 合并 → 按它留下的 `away` 读数给 #202 定性 →
  #194 拖拽口径 → #56 价格轴自适应的测试面
- 更新日：2026-09-24
