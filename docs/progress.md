...
**里程碑 v0.5.12 发布（2026-09-19，RELEASE_FREEZE）**
- 版本号：**0.5.12**（package.json / index.html meta app-version）
- 包含任务：提醒面板·现价 ±% 快捷填充
- 分支：`release/v0.5.12`；发布 PR：**待创建**
- 合并方式：rebase（禁 merge commit）；tag/release：release-tag workflow push main 后自动打 v0.5.12（幂等）
- 部署：merge 后 Pages/Vercel 自动部署；需 live 抽查（首页 200 + app-version=0.5.12 + 知识库 200）
