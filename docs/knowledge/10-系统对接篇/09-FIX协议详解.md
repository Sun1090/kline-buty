...
- [1. FIX 是什么](#1-fix-是什么)
- [2. FIX 在交易链路中的位置](#2-fix-在交易链路中的位置)
- [3. FIX 消息结构](#3-fix-消息结构)
- [4. 核心消息类型](#4-核心消息类型)
- [5. Session 生命周期](#5-session-生命周期)
- [6. 订单状态与 ExecType](#6-订单状态与-exectype)
- [7. FIX 的优缺点](#7-fix-的优缺点)
- [8. 衍生规范：FAST 与加密交易所现状](#8-衍生规范fast-与加密交易所现状)
- [风险提示](#风险提示)
...
| 角色 | 用 FIX 做什么 |
|---|---|
| 买方（对冲基金、资管） | 通过 FIX 连卖方执行通道（券商/银行），下单+拿回报 |
...
| 38 | OrderQty | 数量 |
| 40 | OrdType | 1=市价 2=限价 3=止损等 |
| 44 | Price | 限价单必填；市价单不填 |
