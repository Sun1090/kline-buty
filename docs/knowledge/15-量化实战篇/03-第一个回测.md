...
运行后你会看到一份包含总收益、年化、最大回撤、夏普、胜率的报表——这就是你第一个「真实」回测。
...
> **⚠️ 风险提示**
>
> **回测收益 ≠ 实盘收益**。回测天然低估滑点、高估成交、遗漏现实约束（涨跌停买不进、流动性不足、停牌无法交易）；经过参数优化的策略还叠加了过拟合风险。单次回测、参数遍历的高收益都不能作为投资依据；务必采用「样本内选参、样本外验证、留档可复现」的研究流程，并以模拟盘长期验证后再考虑小资金实盘。本文代码与结论仅用于学习与研究，不构成任何投资建议。
