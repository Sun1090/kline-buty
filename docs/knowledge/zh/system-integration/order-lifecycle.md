...
| CTP 专有二进制 | C++ DLL、结构体消息、回调模型，国内柜台事实标准 | 国内期货主流路径 | CTP（详见 [02-交易所与柜台.md](exchanges-oms.md)） |
...
::: warning ⚠️ 风险提示
交易接口环节的事故往往是「高频 + 错误叠加」：曾有团队上线时把下单参数里的价格小数位配错，在毫秒级循环里连续打出数千笔离谱价格的有效订单，等发现时已无法全部撤掉；也有团队因为重试逻辑没有幂等，一次网络抖动造成同一笔单重复成交了 30 次。请务必记住：**任何自动重试都必须先查询确认**；**任何参数变更都必须先仿真验证再小资金试运行**；上线当天禁止改代码。真实事故往往不是「某个 bug 多严重」，而是「错的代码跑得有多快」——把速度留给正确的系统，把刹车装在任何出错之前（风控详见 [05-风控与资金管理.md](risk-systems.md)）。
:::
