...
>
> 本篇把数据平台分成四层：量化数据源（代码/终端）、历史行情获取渠道、宏观与行业数据、新闻与研报，外加一个「另类数据」的概念科普，最后给出一套零成本研究工具箱——先把数据问题解决，再谈策略（策略部分见 [15-量化实战篇](../quant-practice/)）。
...
- 机构/深度研究 → **Wind**（见 [01-行情软件全景](charting-platforms.md) 的专业终端部分）。
...
拿到数据后第一件事不是建模，而是**和行情软件对数字**（详见 [15-量化实战篇-02](../quant-practice/data-acquisition.md) 的对账方法）：
...
::: tip 💡 读研报的正确姿势
**读研报的正确姿势**已在 [12-市场生态篇-04 信息生态](../market-ecosystem/information-ecosystem.md) 讲过：看逻辑与数据，忽略评级结论——研报是研究工具，不是下单指令。
:::
...
| 历史数据归档 | 自建脚本存 CSV/SQLite（见 [15-量化实战篇](../quant-practice/)） | 时序数据库 + 数据商批量采购 |
| 宏观数据 | 国家统计局 + 央行官网 | Choice/iFinD 个人版 → Wind |
| 公告/研报 | 巨潮资讯 + 东财研报中心 | 慧博专业版 |
| 快讯 | 财联社免费版 | 专业终端内嵌快讯 |
| 研究环境 | Jupyter + Python（本机） | 云服务器（见 [05-运行与自动化环境](automation-environment.md)） |
...
- 定时取数用 cron/系统计划任务，见 [05-运行与自动化环境](automation-environment.md)；
- 每次取数后跑一次**<mark>对账检查</mark>**（最新日期、条数、异常值），发现问题第一时间报警；
- 数据与代码分离：数据放 `data/`，代码放 `scripts/`，配置放 `config/`，目录规范见 [15-量化实战篇-01](../quant-practice/quant-toolchain.md)。

---

## 七、下一步

数据到手之后，下一步是把「数据 → 回测 → 策略」跑通，见 [15-量化实战篇](../quant-practice/)；在哪个平台交易这些数据对应的品种，看 [03-券商与期货公司选择](broker-selection.md)。
