...
::: tip 💡 语言选型结论
个人量化阶段**<mark>不要用 C++ 起步</mark>**。等你有「Python 解决不了的具体问题」时，再针对热点重写，而不是为了用 C++ 而用 C++。
:::
...
| pyfolio | 绩效分析 | 从收益序列一键产出**<mark>夏普</mark>**/回撤/月度热力图 |
...
::: tip 💡 环境管理统一原则
**<mark>每个项目一个虚拟环境</mark>**，把依赖写进 `requirements.txt` 或 `pyproject.toml`，保证换机器能复现。
:::
...
::: tip 💡 建议工作流
**<mark>Jupyter 里想清楚 → 沉淀成 .py 模块 → cron 跑数据脚本</mark>**。notebook 是草稿纸，脚本是成品，别让 notebook 成为你的生产环境。
:::
...
每次改动策略都提交一次，commit message 写明改动与动机——**<mark>三天后的你回头看历史，比看代码更值钱</mark>**。

### 6.2 实验结果留档

回测结果的「可复现」靠三样东西对齐：**<mark>代码版本（git commit）+ 参数 + 数据时间范围</mark>**。建议每次回测后追加一行到实验记录表：
...
回测里用了「当天收盘后才算得出来」的指标去参与**<mark>当天成交</mark>**，就是在偷看未来，回测会被严重虚高。最常见两类：
...
工具链就绪后，进入 [02-数据获取实战](data-acquisition.md)：先把 A 股、期货、加密三市场的数据真正拿干净。
