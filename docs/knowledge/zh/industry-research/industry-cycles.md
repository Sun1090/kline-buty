...
周期的根源是**<mark>供给的调整慢于需求的变化</mark>**：需求可以一个月变一次，而供给（建厂、扩产、投产）往往需要 1-3 年。于是永远存在两种错配：
...
- **<mark>产能周期</mark>决定大方向**（慢变量），**<mark>库存周期</mark>决定波段**（快变量）。
...
::: warning ⚠️ 判断底部最可靠的信号是"停留多久"，不是"跌了多少"
...
> **<mark>周期股</mark>买在亏损时、卖在暴利时；用 PE 是陷阱，用 PB 与价格位置是正解。** 周期股在高 PE 时往往是底部（分母小），低 PE 时往往是顶部（分母大）——**低 PE 周期股是价值陷阱的经典形态**（参考 03 篇案例 B）。
...
| 爆发期 | 10%-30% | 渗透率斜率最陡，量价齐升，业绩高增长 | **最甜蜜阶段：<mark>业绩与估值双击</mark>** |
...
| 顶部判断失灵 | 顶部无法精确预测，扩产潮开始后还可能再涨一年——**但那是<mark>刀尖上的利润</mark>** |
