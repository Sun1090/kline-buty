...
- Read [Stocks / Stock Analysis Methods](../stocks/stock-analysis.md) first for valuation basics (PE/PB/PS/DCF); this chapter extends and deepens that material.
...
① Reading the three statements (lay the foundation: understand the statements themselves)
   ↓
② Financial metrics in practice (sharpen the tools: turn numbers into judgment)
   ↓
③ Detecting financial fraud (install the radar: learn to rule out first)
   ↓
④ Cash flow analysis (grasp the essence: cash is king)
   ↓
⑤ Statement analysis in practice (run the pipeline: fully analyze one report)
```

- ① and ② are the foundation; ③ and ④ are intermediate; ⑤ is the capstone. Read them in order.
- After this chapter, combine with the valuation methods from [Chapter: Stocks](../stocks/): rule out mines first, then value, then decide.

---

## Chapter Contents

<DocCards dir="financial-statements" />
