...
| **Nanhua Commodity Index** | Nanhua Futures (domestic) | Main domestic futures market constituents, weighted by **<mark>liquidity</mark>**/trading volume (per latest methodology) | Tracks the overall performance of **China-priced commodities** (ferrous, chemicals, agricultural products) — the "domestic thermometer" for onshore commodity longs and shorts |
...
PPI rises (ex-factory prices of industrial goods)
   ↓ (months to quarters)
CPI rises (consumer prices) → central bank hikes rates → pressures all assets
```
...
| Policy implication | Much of the "inflation" central banks watch is first decided in commodity prices — **the commodity index is the forward sentry of rate policy** (see [08 - Macro Economy and Markets](macro-markets.md)) |
...
::: tip 💡 A supply-demand gap is the hardest evidence of a supercycle
...
::: warning ⚠️ By the time the supercycle narrative appears, the move is half over
...
| **Global manufacturing PMI** | **Positively correlated with industrial metals** | Copper/aluminum/zinc are the "coincident thermometer" of manufacturing: PMI expansion → demand up → metals stronger ("Dr. Copper" in [06 - Commodity Panorama](commodities-overview.md)) |
...
| **Real interest rates** | Mostly negatively correlated with gold | Real rates up → higher opportunity cost of holding gold → gold pressured (see [09 - Precious Metals and Energy Spot](precious-metals-energy.md)) |
...
1. **Pricing effect**: commodities are dollar-priced; dollar depreciation → the same dollar price is cheaper for non-US buyers → demand rises;
2. **Rates/liquidity channel**: a weakening dollar often accompanies easing expectations → global liquidity expands → speculative demand for commodities rises;
3. **Financial asset character**: the dollar is the vehicle of "risk-free assets"; when haven money flows out of dollar assets it often flows into real assets (gold the classic).
...
| Role | Logic | Caveats |
|---|---|---|
| **Inflation hedge** | Commodity prices are themselves part of inflation; holding commodities directly **<mark>hedges</mark>** "prices rising" risk | Works only in rising-inflation phases; in low-inflation/deflation phases commodities are a drag |
...
| Portfolio type | Commodity **<mark>position</mark>** | Composition of the commodity sleeve |
|---|---|---|
...
- Two principles: **① participate via indices/ETFs, never use high-leverage futures for allocation; ② book the commodity "allocation position" and "trading position" separately** — the allocation position may sit untouched for years; the trading position must run a strict **<mark>stop-loss</mark>** (figures are illustrative; per personal risk tolerance).
...
|---|---|---|---|
| **Commodity futures** | Trade futures contracts directly | High threshold (**<mark>margin</mark>** system + leverage), requires professional skill, roll and delivery issues | Professional traders |
...
- An oft-ignored difference: **futures/ETF returns ≈ spot price change + roll P&L (contango/backwardation)**. Buy a "rolling futures-based ETF" in a contango environment, and even with the spot price flat the NAV can keep sliding (compare the VXX decay logic in [13 - **<mark>Volatility</mark>** and VIX](volatility-vix.md)).
...
- Stocks grow on earnings and can be "bought, held, and left to grow"; **commodities have no earnings — holding them produces no intrinsic value** — commodity profits come from the "**<mark>spread</mark>**", so you must answer "when does it rise, by how much, and when do I leave".
...
- Inventories (LME/SHFE/port stocks, the EIA weekly crude report) and the term structure (contango/backwardation) are earlier and more honest data than price — **price is the conclusion; inventories and the term structure are the evidence** (see the "commodity watchlist" at the end of [06 - Commodity Panorama](commodities-overview.md)).
...
::: danger 💀 News is olds; inventories and term structure are the scene
...
1. **Commodity futures carry built-in leverage and violent swings**: 2020's negative WTI crude and the 2022 LME nickel squeeze are both textbook risk events (per latest historical data).
2. **Futures-based commodity ETFs have roll decay and tracking error**: in contango, long-term holding steadily erodes NAV, with premium-collapse risk on top — read the fund prospectus before buying.
3. **The "supercycle" narrative guarantees no returns**: history doesn't simply repeat; when the sell-side's "supercycle thesis" appears, the move is usually half over.
4. Commodity prices are frequently shocked by geopolitics, weather, and policy; historical correlations (with equities/the dollar) can break down completely in extreme conditions.
5. All index compositions, weights, ranges, and cycle dating in this article are teaching-basis descriptions — **defer to index providers' latest methodologies and exchange data**; this article does not constitute investment advice.
:::
