...
> **⚠️ Risk Warning**
>
> This article is for learning and research only and does not constitute investment advice. The REITs rules, price limits, payout ratios, tax policies, and rental-yield ranges mentioned here are generic teaching-basis descriptions — **always defer to the CSRC's/exchanges' latest rules, each fund's latest announcements, and the latest market data**. Real estate involves large sums and poor **<mark>liquidity</mark>**; REITs prices likewise carry the risk of falling below the issue price. Assess your risk tolerance before participating.
...
::: tip 💡 REITs are a fractional slice of the landlord business
...
::: danger 💀 The house itself is not the asset — the rent-collecting house is
...
Operator acquires/holds real estate (offices, malls, warehouses, data centers, apartments, etc.)
        ↓
Collects rent and other operating income, minus operating costs
        ↓
Distributes most of the income (about 90% commonly required overseas) as dividends to share holders
```
...
- **How US rate hikes hit REITs** is the key to REITs pricing:
  1. **Valuation**: REITs dividend cash flows are discounted by interest rates — **rates up → discount rate up → asset valuation down** — the most direct sensitivity to hikes;
  2. **Financing**: REITs expand on borrowed money — hikes raise funding costs and erode distributable profit;
  3. **Assets**: rising rates usually accompany **rising capitalization rates (Cap Rate)** — "annual rent of the same building ÷ asset value" rises — pressuring property valuations;
  4. Historical evidence: in the Fed's aggressive 2022-2023 hiking cycle, major US and global REITs indices saw significant **<mark>drawdowns</mark>** (per latest data); REITs often lead the recovery when rate-cut expectations warm — **REITs are a classic "rate-sensitive asset"**.
...
| **<mark>Leverage</mark>** | Leverage by default (70% mortgages common); the default risk is real | No leverage (the shares carry no forced leverage; underlying assets may carry operating loans, per fund) |
...
::: danger 💀 A rental yield below 1.5% is a pure appreciation bet
...
- Practical implication: **the Fed's / domestic rate path is a leading input for sizing your REITs **<mark>position</mark>**** — don't go heavy on "bond-like" REITs at the tail of a hiking cycle, and don't underestimate the recovery elasticity in a cutting cycle (historical data for reference only, per latest).
...
1. **REITs ≠ principal-guaranteed wealth products**: onshore public REITs have already broken below issue prices and traded at persistent secondary-market discounts in many cases (per latest data); the 30%/10% limit mechanism provides "caps", not "protection".
2. **Distributions ≠ fixed-income coupons**: they come from operating cash flow and fluctuate with underlying asset conditions; toll-road/subsidized-housing types are relatively stable, park/consumer types are more elastic.
3. **The risks of buying directly are broadly underestimated**: low rental yields, illiquidity, leveraged defaults, and policy control stack together; the "homes always rise" era has shifted with the population and urbanization inflection (per latest statistics).
4. Private real estate funds/trust products carry high thresholds, long terms, and historical redemption-default cases (per latest regulatory and industry data) — non-qualified investors should stay away.
5. All mechanisms, rates, rules, and ranges in this article are teaching-basis descriptions — **defer to the CSRC's/exchanges' latest rules, each fund's announcements, and the latest market data**; this article does not constitute investment advice.
:::
