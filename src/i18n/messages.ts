/** 界面文案字典（zh-CN / en / ja / ko）。
 * 结构：嵌套对象，叶子为字符串；支持 {name} 插值。
 * en 以 typeof zh 约束，保证键完全对齐（编译期校验）。
 */

export type Lang = 'zh-CN' | 'en' | 'ja' | 'ko' | 'es'

export const zh = {
  common: {
    none: '无',
    price: '价格',
    delete: '删除',
    cancel: '取消',
    confirm: '确定',
    retry: '重试',
    more: '更多',
    knowledge: '知识库',
    knowledgeTitle: '打开交易知识库（新标签页）',
    close: '关闭',
    backToLatest: '回到最新',
    feedback: '反馈',
    feedbackTitle: '报告问题 / 反馈建议（打开 GitHub Issues）',
  },
  scale: { linear: '线性', log: '对数', title: '价格坐标轴：线性 / 对数' },
  status: {
    loading: '加载历史数据…',
    connecting: '连接实时行情…',
    live: '实时',
    reconnecting: '重连中…',
    closed: '连接断开',
    error: '加载失败',
    chartError: '行情数据加载失败：请检查网络或数据源可达性',
    noData: '暂无数据',
    loadDemo: '加载演示数据',
    frameDrop: '实时帧丢帧 {rate}',
    frameDropUnit: '丢帧',
    refilling: '断线回补中 {done}/{total}',
    depthLoading: '加载盘口深度…',
    vpNotEnough: '数据不足',
    replayNotEnough: '数据不足（需 ≥30 根）',
  },
  disclaimer: {
    text: '数据仅供参考，不构成投资建议',
    watermark: '仅供学习参考 · 不构成投资建议',
  },
  settings: {
    watermark: '图表水印',
    watermarkTitle: '显示/隐藏图表免责声明水印',
    snapshot: '设置快照',
    snapshotExport: '导出设置',
    snapshotExportTitle: '导出全部设置（主题/自选/画线/账户等）为 JSON',
    snapshotImport: '导入设置',
    snapshotImportTitle: '从 JSON 快照恢复设置（刷新生效）',
  },

  chartType: {
    candlestick: '蜡烛',
    line: '折线',
    area: '面积',
  },
  chart: {
    subScaleFixed: '固定Y轴',
    subScaleToggleTitle: '副图 Y 轴锁定到理论极值（超买超卖稳定比较），再点恢复自动',
    sessionLines: '当日高低',
  },
  group: {
    type: '类型',
    main: '主图',
    sub: '副图',
  },
  period: {
    '1s': '1秒',
    '1m': '1分',
    '3m': '3分',
    '5m': '5分',
    '15m': '15分',
    '30m': '30分',
    '1h': '1时',
    '2h': '2时',
    '4h': '4时',
    '12h': '12时',
    '1d': '日',
    '3d': '3日',
    '1w': '周',
    '1M': '月',
  },
  lang: {
    switchTo: '切换语言（中/EN/日/韩/西）',
  },
  tz: { title: '时间轴时区', utc: 'UTC', local: '本地' },
  theme: {
    switchTitle: '切换主题',
    toLight: '浅色',
    toDark: '深色',
    toAuto: '自动（跟随系统）',
    toSchedule: '定时',
    scheduleDark: '深色时间',
    scheduleLight: '浅色时间',
    pickTitle: '主题色预设（强调/涨跌色）',
    presetClassic: '经典蓝',
    presetAShare: '红涨绿跌',
    presetPurple: '紫调',
    presetTeal: '青调',
    highContrastTitle: '高对比模式',
    highContrastOn: '高对比',
    highContrastOff: '标准',
  },
  layout: {
    switchTitle: '布局切换（单图/双图/四图）',
    presets: '面板布局方案',
    presetName: '方案名',
    savePreset: '保存方案',
    applyPreset: '套用',

    compare: '对比',
    compareTitle: '对比叠加品种',
    compareOff: '关闭',
    fontScale: '字号',
    fontScaleTitle: '图表字号缩放',
    single: '单图',
    pair: '双图',
    quad: '四图',
  },
  fullscreen: {
    enter: '全屏',
    exit: '退出全屏',
  },
  panel: {
    position: '仓位',
    positionTitle: '模拟仓位（开仓/止盈/止损线）',
    alerts: '提醒',
    alertsTitle: '价格提醒',
    depth: '深度',
    depthTitle: '盘口深度图',

    orderBook: '盘口',
    orderBookTitle: '盘口订单簿（买卖档位）',
    tape: '成交明细',
    tapeTitle: '最新逐笔成交（价格/数量/时间）',
    vp: '筹码',
    vpTitle: '筹码分布（成交量分布 VPVR）',
    settings: '参数',
    sentiment: '情绪',
    sentimentTitle: '衍生品情绪（多空比/主动买卖/未平仓）',
    statsTitle: '行情概览（最新价/涨跌/资金费率/未平仓）',
    resizeTitle: '拖拽调整侧边栏宽度',
    orderTitle: '侧栏面板顺序',
    moveUp: '上移',
    moveDown: '下移',

  },
  perf: {
    candleCap: '图表K线上限',
    candleCapTitle: '超过该数量时降采样渲染（0=关闭）',
    off: '自适应关',
    panelTitle: '性能诊断',
    noFrame: '暂无实时帧（当前无 WS 行情）',
    fps: '实时帧速率',
    dropRate: '丢帧率',
    avgInterval: '平均帧间隔',
    window: '窗口帧数',
    warn: '⚠ 丢帧率超 10%——图表可能卡顿（弱网/数据量过大）',
  },
  changelog: {
    title: '版本历史',
    v040r1: 'I 阶段收官：定时主题切换（深/浅色按设定时刻自动切换）、画线深链、模板市场、快照画廊；全阶段 A–I 完成（I3 云同步 / I11 移动端 Widget 待外部能力）',
    v04r1: '数据正确性：周期边界对齐 / 右侧锚定 / 断线补洞 / 多周期光标同步 / 数据延迟指示',
    v04r2: '指标引擎：光标取值 / 信号打点 / 阈值着色 / W%R·TRIX·DPO·Vortex / 参数导入导出 / 指标收藏',
    v04r3: '画线：截图导出 / 组锁 / 批量操作 / 模板跨品种 / 吸附四态 / 撤销深度 / 坐标角标',
    v04r4: '模拟交易：双向持仓 / 动态保证金率 / 费率·滑点可配 / 盈亏统计 / 强平预警 / 费拆分 / 快照 / 收益目标 / JSON 迁移',
    v04r5: '提醒：推送渠道 / 多品种监控 / 组开关 / 模板 / 到期 / 批量操作 / 触发次数 / 精度 / 角标 / 快捷键 / 备注',
    v04r6: '可访问性：焦点陷阱 / 高对比 / 信息条配置 / 侧栏调宽 / 面板顺序 / 布局方案',
    v04r7: '性能：文档站组件测试 / 数据量自适应降采样 / 卡顿诊断 / 泄漏审计',
    v04r8: '设置快照导出导入 / 快捷键速查卡打印',
    v03r1: 'P3/P4 深化：49 种画线工具、26 个指标、市场回放、模拟交易、价格提醒、移动端手势、PWA',
    v03r2: '键盘无障碍：全局 Tab 流、roving tabindex、focus-visible、aria-pressed 联动',
    v02r1: '知识库 27 章 201 篇、文档站上线、五语 i18n',
    v01r1: '实时 K 线终端：14 周期、盘口深度、多图表布局、CSV 导出、深浅主题',
  },
  snap: {
    title: '快照画廊',
    save: '存快照',
    saved: '已保存到画廊',
    preview: '预览',
    download: '下载图片',
    empty: '暂无快照——在图表右上角点击「存快照」保存对比图',
    clearAll: '清空全部',
  },
  pinned: {
    title: '自选实时行情',
    titleShort: '自选行情',
    empty: '暂无钉选品种——在自选列表点击★钉选',
    unpin: '取消钉选',
    add: '钉选',
    sortBy: '排序',
    sortPrice: '价格',
    sortChange: '涨跌',
  },
  sentiment: {
    globalRatio: '全账户多空比',
    topTraderRatio: '大户持仓多空比',
    takerRatio: '主动买卖比',
    openInterest: '未平仓 24h',
    long: '多',
    buy: '买',
    loading: '加载中…',
  },
  share: {
    title: '复制当前品种、周期与选中画线的分享链接',
    copy: '分享',
    copied: '已复制',
    exportTitle: '导出当前品种/周期的 K 线 CSV（含当前指标列）',
    export: '导出',
    exported: '已导出',
  },
  csv: {
    exportRange: '导出范围',
    all: '全部',
  },
  docs: {
    title: '文档索引',
    knowledge: '交易知识库',
    knowledgeDesc: '27 章 201 篇：现货/期货/股票/加密/外汇/期权/宏观/量化',
    readme: '项目 README',
    readmeDesc: '功能特性、技术栈、快速开始',
    repo: 'GitHub 仓库',
    repoDesc: '源码、Issue、Pull Request',
    changelog: 'CHANGELOG',
    changelogDesc: '完整变更历史',
  },
  replay: {
    start: '回放',
    title: '历史逐根回放',
    play: '播放',
    pause: '暂停',
    exit: '退出回放',
  },
  drawing: {
    snap: '吸附 K 线',
    snapOn: '开',
    snapOff: '关',
    snapTime: '时间对齐',
    snapGrid: '网格对齐',
    snapOhlc: 'OHLC 对齐',
    noteToggle: '便签显隐',
    coordBadge: '坐标角标常显',
    group: '画线',
    mouse: '鼠标',
    horizontal: '水平线',
    vertical: '垂直线',
    cross: '十字线',
    trend: '趋势线',
    extended: '延长线',
    angle: '趋势角度',
    channel: '平行通道',
    fib: '斐波那契',
    text: '文本',
    note: '备注',
    defaultText: '文本',
    rect: '矩形',
    ellipse: '椭圆',
    circle: '圆',
    triangle: '三角形',
    wedge: '楔形',
    arc: '圆弧',
    bezier: '贝塞尔曲线',
    ray: '射线',
    hray: '水平射线',
    vray: '垂直射线',
    parray: '平行射线',
    pchannel: '宽度通道',
    fibext: '斐波那契扩展',
    fibchannel: '斐波那契通道',
    fibfan: '斐波那契扇形',
    fibTime: '斐波那契时间线',
    cycle: '周期线',
    timerange: '时间区间',
    pband: '价格带',
    pricerange: '价格区间框',
    fibtz: '斐波那契时间区间',
    gann: '江恩角度线',
    gannbox: '江恩箱',
    pricelabel: '价格标签',
    arrow: '箭头',
    polyline: '多段线',
    measure: '量度',
    measureBars: '{bars}根',
    rr: '风险回报',
    positionTool: '持仓计划',
    forecast: '预测线',
    daterange: '日期范围',
    rrRatio: '盈亏比 1:{ratio}',
    speedlines: '速度线',
    regchan: '回归通道',
    hchannel: '水平通道',
    pitchfork: '安德鲁叉',
    xabcd: 'XABCD 形态',
    elliott: '艾略特波浪',
    editText: '改字',
    defaultColor: '默认颜色',
    searchPlaceholder: '搜索画线工具…',
    searchNoMatch: '无匹配工具',
    textPlaceholder: '文本内容',
    fontSize: '字号',
    color: '颜色',
    textBg: '底色',
    align: '对齐',
    alignLeft: '左对齐',
    alignCenter: '居中',
    alignRight: '右对齐',
    textDefault: '默认',
    favoriteToggle: '收藏切换',
    screenshot: '截图',
    share: '分享',
    shareTitle: '一键分享当前图表（系统分享，不支持时下载）',
    screenshotTitle: '截图分享',
    screenshotRegion: '框选',
    screenshotScale: '截图分辨率倍数',
    regionHint: '拖拽框选图表区域，松开导出 PNG',
  },
  layers: {
    title: '图层管理',
    undo: '撤销',
    redo: '重做',
    undoDepth: '撤销步数',
    undoDepthTitle: '画线撤销深度（1=只保留一步，上限 200）',
    globalOpacity: '全局透明度',
    delete: '删除',
    batchCount: '已选 {n} 项',
    select: '选择',
    copy: '复制画线',
    paste: '粘贴画线',
    opacity: '透明度',
    followLatest: '跟随最新价',
    ungrouped: '未分组',
    back: '返回画线',
    show: '显示',
    hide: '隐藏',
    lock: '锁定',
    unlock: '解锁',
    clearAll: '全部清除',
    showAll: '全部显示',
    hideAll: '全部隐藏',
    export: '导出画线 JSON',
    import: '导入画线 JSON',
    importSymbolMismatch: '导入失败：文件属于其他交易对',
    importBad: '导入失败：文件格式无法识别',
    empty: '暂无画线',
    search: '按名称/类型搜索画线…',
    rename: '重命名',
    templateTitle: '画线模板',
    templateSave: '保存为模板',
    templateNamePlaceholder: '模板名（如：趋势+斐波那契）',
    templateApply: '套用',
    templateEmpty: '暂无模板——保存当前画线组合后一键套用到任意品种',
    templateSaved: '已保存模板',
    templateExport: '导出模板',
    templateImport: '导入模板',
    templateImportDone: '已导入',
    templateImportFail: '导入失败：文件格式无效',
    statsLine: '线 {count} · 长度 {len}',
    statsArea: '面 {count} · 面积 {area}',
  },
  symbol: {
    searchPlaceholder: '搜索交易对…',
    popular: '常用',
    favorites: '自选',
    favoriteAdd: '加入自选',
    favoriteRemove: '取消自选',
    searchResults: '搜索结果 {count}',
    noMatch: '无匹配交易对',
  },
  indicator: {
    settings: '指标参数',
    resetDefault: '重置默认',
    recommend: '智能推荐',
    recommendTitle: '按趋势/波动率推荐主副图指标',
    recTrending: '趋势市',
    recRanging: '震荡市',
    recVolatile: '高波动',
    drawingSuggest: '画线建议',
    drawingSuggestTitle: '按当前品种已画图形建议指标',
    recSemanticsRange: '波动区间',
    recSemanticsTrend: '趋势画线',
    recSemanticsLevels: '支撑阻力',
    recSemanticsTime: '时间结构',
    noParams: '当前指标无参数可调',
    maPeriods: '{name} 周期(逗号分隔)',
    maOverlayEma: 'MA 上叠加 EMA',
    subOverlay: '副图叠加指标',
    bollPeriod: 'BOLL 周期',
    bollMult: 'BOLL 标准差倍数',
    macdFast: 'MACD 快线',
    macdSlow: 'MACD 慢线',
    macdSignal: 'MACD 信号',
    kdjN: 'KDJ N',
    kdjM1: 'KDJ M1',
    kdjM2: 'KDJ M2',
    rsiPeriod: 'RSI 周期',
    wrPeriod: 'WR 周期',
    obvMaPeriod: 'OBV 平滑(1=原始)',
    volMaPeriod: 'VOL 均量线周期(1=关)',
    atrPeriod: 'ATR 周期',
    dmiPeriod: 'DMI 周期',
    cciPeriod: 'CCI 周期',
    psyPeriod: 'PSY 周期',
    stochK: 'STOCH %K 周期',
    stochSmooth: 'STOCH 平滑',
    stochD: 'STOCH %D 周期',
    rocPeriod: 'ROC 周期',
    momPeriod: 'MOM 周期',
    bbwPeriod: 'BBW 周期',
    bbwMult: 'BBW 标准差倍数',
    stPeriod: 'ST 周期',
    stMult: 'ST 乘数',
    sarAfStart: 'SAR 起始加速',
    sarAfStep: 'SAR 加速步进',
    sarAfMax: 'SAR 加速上限',
    ichimokuTenkan: '转换线周期',
    ichimokuKijun: '基准线周期',
    ichimokuSpanB: '先行带B周期',
    ichimokuDisplacement: '位移周期',
    sar: 'SAR',
    ichimoku: 'Ichimoku',
    mfiPeriod: 'MFI 周期',
    trixPeriod: 'TRIX 周期',
    dpoPeriod: 'DPO 周期',
    vortexPeriod: 'Vortex 周期',
    aoFast: 'AO 快线',
    aoSlow: 'AO 慢线',
    cmfPeriod: 'CMF 周期',
    donchianPeriod: 'Donchian 通道周期',
    aroonPeriod: 'Aroon 周期',
    aroon: 'Aroon',
    reset: '重置为默认',
    preset: '参数预设',
    presetPlaceholder: '加载预设…',
    presetSave: '保存',
    presetSaved: '已保存',
    presetName: '预设名称',
    exportParams: '导出参数',
    importParams: '导入参数',
    importParamsOk: '参数已导入',
    lineColors: '线色',
    lineColorFor: '{name} 线颜色',
    lineColorReset: '恢复默认色',
  },
  stats: {
    lastPrice: '最新价',
    change24h: '24h涨跌',
    high24h: '24h高',
    low24h: '24h低',
    volume24h: '24h额',
    fundingRate: '资金费率',
    fundingNext: '距下次资金结算',
    openInterest: '未平仓',
    markPrice: '标记价',
    countdown: '收盘',
    marketPerp: '合约',
    marketSpot: '现货',
    volumeSurge: '量能',
    volumeSurgeTitle: '最新成交量 / 前 20 根均量 ≥ 3× 视为量能异动',
    gapHealthy: '数据完整',
    gapPartial: '少量缺口',
    gapDegraded: '数据缺口 {n} 段',
    dataLatency: '数据延迟',
    dataLatencyTitle: '实时帧滞后超过 5 秒，数据可能停更或弱网',
        marketType: '市场类型',
    health: '数据健康度',
    summary: '行情概览',
    barConfig: '信息条显示项',
  },
  marketList: {
    title: '行情',
    addFavorite: '加自选',
    removeFavorite: '移除自选',
    pair: '交易对',
    lastPrice: '最新价',
    change24h: '24h涨跌',
    volume: '成交额',
    refresh: '刷新',
    loading: '加载行情…',
    empty: '暂无行情数据',
    collapse: '收起',
    expand: '展开',
    sortTitle: '点击排序',
    stale: '刷新失败，展示缓存',
    searchPlaceholder: '过滤交易对…',
    tabAll: '全部',
    tabFavorites: '自选',
    tabRank: '榜单',
    rankChange: '涨幅榜',
    rankVolume: '成交榜',
    rotate: '自动轮播',
    rotateTitle: '每 5 秒在可见列表中循环切换当前品种',
    favoritesEmpty: '暂无自选——在交易对下拉中点 ★ 收藏后在此展示',
    noMatch: '无匹配交易对',
  },

  position: {
    title: '模拟仓位',
    close: '平仓',
    long: '开多',
    short: '开空',
    hedgeMode: '双向持仓',
    balance: '可用余额',
    unrealized: '浮动盈亏',
    qtyShort: '数量',
    noPosition: '暂无持仓',
    closeAll: '全部平仓',
    otherSymbols: '其他品种持仓',
    levels: '止盈/止损',
    levelBreakeven: '保本止损',
    levelClearHint: '留空即清除该线',
    reduce: '减仓',
    reduceQty: '减仓数量',
    reduceErr: '数量需大于 0 且不超过持仓量',
    levelTrail: '移动止损',
    levelTrailHint: '止损随最新价推进，保持 t% 距离（空头反向），价格回落时不回撤',
    levelErrInvalid: '止盈/止损需落在开仓价的合理一侧',
    levelErrCrossed: '止盈价与止损价不能重合',
    switch: '切换查看',
    entry: '开仓价',
    market: '现价',
    quantity: '数量',
    tpPct: '止盈%',
    slPct: '止损%',
    tpLine: '止盈线',
    tpPrice: '止盈价',
    slPrice: '止损价',
    levelModePct: '百分比',
    levelModePrice: '价位',
    slLine: '止损线',
    floatingPnl: '浮动盈亏',
    open: '开仓',
    lineEntry: '开仓',
    lineTp: '止盈',
    lineSl: '止损',
    margin: '保证金',
    leverage: '杠杆',
    liqPrice: '强平价',
    marginRate: '保证金率',
    liqWarn: '强平预警',
    liqCritical: '强平风险',
    reverse: '反手',
    todayPnl: '今日已实现',
  },
  alert: {
    history: '触发历史',
    sound: '触发提示音',
    clearHistory: '清空记录',
    title: '价格提醒 · {symbol}',
    granted: '通知已开启',
    unsupported: '环境不支持通知',
    enable: '开启通知',
    above: '价格 ≥',
    below: '价格 ≤',
    add: '添加提醒',
    invalid: '请输入有效价格（正数）',
    none: '暂无提醒',
    triggered: '已触发',
    reset: '重置',
    notifyTitle: 'Kline Buty · 价格提醒',
    notifyAbove: '{symbol} 已到达 {price}',
    notifyBelow: '{symbol} 已跌破 {price}',
    repeat: '循环提醒',
    soundPreview: '试听音效',
    repeatInterval: '重复间隔',
    minutes: '分钟',
    group: '分组',
    groupPlaceholder: '如：趋势',
    sort: '排序',
    filter: '筛选',
    filterAll: '全部',
    filterActive: '待触发',
    sortTime: '时间',
    sortPrice: '价格',
    sortSymbol: '品种',
    soundKind: '音效',
    soundBeep: '哔哔',
    soundChime: '风铃',
    soundPing: '叮',
    soundLow: '低沉',
    timeWindow: '时间窗口',
    timeFrom: '开始',
    timeTo: '结束',
    channel: '推送渠道',
    channelBoth: '两者',
    channelSystem: '系统通知',
    channelWeb: '站内横幅',
    note: '备注',
    notePlaceholder: '可选说明…',
    expiresAt: '到期时间',
    precision: '价格精度',
    precisionAuto: '自动',
    template: '模板',
    templateName: '名称',
    saveTemplate: '存为模板',
    applyTemplate: '套用',
    batch: '批量',
    selectAll: '全选',
    clearSelection: '清空',
    enableSelected: '启用所选',
    disableSelected: '停用所选',
    deleteSelected: '删除所选',
    select: '选择',
    expired: '已过期',
    clearExpired: '清理已过期',
    triggerCount: '触发',
    groupToggle: '组开关',
    voice: '语音播报（价格异动朗读）',
    adaptive: '波动率自适应（ATR 阈值）',
    quickPct: '快捷 ±%',
    pending: '待触发',
  },
  tooltip: {
    open: '开',
    high: '高',
    low: '低',
    close: '收',
    volume: '量',
  },
  depth: {
    title: '盘口深度 · {symbol}（实时）',
    bid: '买',
    ask: '卖',
  },
  orderBook: {
    title: '盘口订单簿 · {symbol}',
    qty: '数量',
    cum: '累计',
    group: '聚合',
    spread: '价差',
    markHint: '点击标记到主图（再点清除）；左侧按钮快捷下单',
    refresh: '刷新盘口（重连拉取最新快照）',
    bid: '买',
    ask: '卖',
    imbalance: '盘口买卖失衡：买/卖各占总量的百分比',
  },
  tape: {
    title: '最新成交 · {symbol}',
    time: '时间',
    qty: '数量',
    sideSummary: '窗口内主动买/主动卖成交笔数',
    markHint: '点击标记到主图（再点清除）',
    filterAll: '全部',
    filterBuy: '主动买',
    filterSell: '主动卖',
    filterSideHint: '按成交方向筛选（主动买/主动卖）',
    bigOrder: '大单',
    bigOrderHint: '仅显示数量达到窗口均值若干倍的成交',
    emptyFilter: '无符合筛选的成交',
  },
  trade: {
    buy: "买入",
    sell: "卖出",
    market: '市价',
    limit: '限价',
    limitHint: '限价单触价按当时最新价成交（优于挂单价时给价格改善），不计滑点',
    makerFee: '挂单费率 %',
    pendingTitle: '当前挂单',
    pendingEmpty: '暂无限价挂单',
    cancelOrder: '取消挂单',
    editOrder: '改价',
    editErr: '价格与数量需为正数',
    filledToast: '限价单已成交 · {symbol} {qty} @ {price}',
    cancelledToast: '余额不足，挂单已撤销 · {symbol}',
    tpslToast: '{symbol} 触发{reason}，已按 {price} 平仓',
    tpslToastMulti: '{symbol} 触发{reason}，共 {count} 笔按 {price} 平仓',
    tpWord: '止盈',
    slWord: '止损',
    tooManyOrders: '同品种挂单已达上限（{max} 条）',
    stats: '交易统计',
    winRate: '胜率',
    totalPnl: '累计盈亏',
    profitFactor: '盈亏比',
    avgWin: '平均盈利',
    avgLoss: '平均亏损',
    settings: '交易设置',
    feeRate: '吃单费率 %',
    slippage: '市价滑点 %',
    target: '收益目标',
    targetHit: '目标达成',
    detail: '明细',
    notional: '成交额',
    feeRateDetail: '费率',
    feeDetail: '手续费',
    grossPnl: '价差盈亏',
    netPnl: '净盈亏',
    maxDrawdown: '最大回撤',
    drawdown: '回撤',
    pnlBars: '逐笔盈亏',
    filterSymbol: '品种',
    filterSide: '方向',
    filterQuery: '搜索价格…',
    filterEmpty: '无匹配记录',
    all: '全部',
    dailyCount: '笔数',
    dailyPnl: '当日盈亏',
    weekPnl: '本周盈亏',
    monthPnl: '本月盈亏',
    period: '期间盈亏',
    locate: '定位图表',
    bySymbol: '按品种汇总',
  },
  paper: { title: '交易流水', empty: '暂无成交——开仓后此处记录成交流水', open: '开仓', close: '平仓', long: '多', short: '空', clear: '清空', export: '导出 CSV', reset: '重置账户', resetConfirm: '确认重置模拟账户？余额将恢复为 10,000 并清空全部流水。', exportEquity: '导出权益曲线', equity: '权益曲线', exportJson: '导出 JSON', importJson: '导入 JSON', importDone: '导入成功', importFail: '导入失败：格式无效', snapshot: '账户快照', snapshotName: '快照名', saveSnapshot: '保存', loadSnapshot: '载入', deleteSnapshot: '删除' },
  ctx: { copyPrice: '复制价格', copyOhlc: '复制 OHLC', addAlert: '添加提醒', limitBuy: '挂限价买入', limitSell: '挂限价卖出', clearDrawings: '清空画线', confirmClear: '确认清空当前品种的全部画线？', copied: '已复制' },
  pull: { pull: '下拉刷新', release: '松开刷新', refreshing: '刷新中…' },
  panelState: { loading: '加载中…', error: '加载失败', empty: '暂无数据', retry: '重试' },
  quickOrder: {
    balance: '可用余额',
    insufficient: '余额不足：需要名义金额 + 手续费作为保证金',
    bid: '买一',
    ask: '卖一',
    title: "快速下单",
    price: "价格",
    qty: "数量",
    notional: "预估金额",
    fill: "估计成交价",
    fee: "手续费 (0.1%)",
    total: "合计",
    confirm: "确认下单",
    hint: "将覆盖当前模拟仓位 · Enter 确认 / Esc 关闭",
    qtyPreset: "常用数量",
    customPct: "自定义%",
    apply: "应用",
    attachHint: '随单止盈/止损（可选）：这单成交即写进持仓价位线',
    attachErr: '止盈/止损需落在挂单价的合理一侧：买单止盈更高、止损更低，卖单相反',
    riskReward: '隐含盈亏比 {ratio} : 1',
  },
  volumeProfile: {
    title: '筹码分布（最近 300 根）· {symbol}',
    poc: '密集区 ≈ {price}',
    bidVol: '买量',
    askVol: '卖量',
    pocLabel: '密集区',
  },
  offline: {
    text: '网络已断开，行情暂停更新，恢复后自动重连',
  },
  storage: {
    warn: '本地存储占用已达 {size}，建议清理旧数据',
  },
  update: {
    banner: '发现新版本，点击刷新加载最新功能',
    reload: '刷新',
  },
  errorBoundary: {
    title: '图表渲染出错',
  },
  shortcuts: {
    title: '键盘快捷键',
    openSearch: '打开交易对搜索：{key}',
    period: '切换周期：[ / ]',
    periodPrev: '上一周期',
    periodNext: '下一周期',
    layout: '布局切换：1 / 2 / 3（单图 / 双图 / 四图）',
    layoutKeys: '1 / 2 / 3',
    cycleMain: '循环主图指标：M',
    cycleSub: '循环副图指标：N',
    fullscreen: '全屏切换：F',
    replay: '回放播放/暂停：Space',
    replayStep: '回放逐根步进：← / →',
    replaySpeed: '回放倍速：↑ / ↓',
    deleteDrawing: '删除选中画线：Delete',
    escape: '取消 / 关闭：Esc',
    help: '显示/隐藏快捷键',
    copyDrawing: '复制画线',
    pasteDrawing: '粘贴画线',
    hint: '显示/隐藏快捷键：?',
    groupNav: '导航',
    groupIndicators: '指标',
    groupReplay: '回放',
    groupDrawings: '画线',
    filter: '过滤快捷键…',
    noMatch: '无匹配快捷键',
    configTitle: '配置快捷键',
    recording: '请按下新快捷键…',
    clickToChange: '点击后按新键',
    cancel: '取消录制',
    configHint: '点击某项后按下新键',
    cycleLang: '切换语言',
    toggleAlerts: '打开提醒',
    print: '打印速查卡',
    reset: '恢复默认',
    conflict: '快捷键冲突：{names} 共用同一按键',
  },
   app: {
    titleZh: 'Kline Buty · 实时 K 线',
    titleEn: 'Kline Buty · Live Candles',
    titleJa: 'Kline Buty · リアルタイム K 線',
    titleKo: 'Kline Buty · 실시간 K 라인',
    titleEs: 'Kline Buty · Velas en vivo',
  },
}

export type Messages = typeof zh

export const en: Messages = {
  storage: {
    warn: 'Local storage usage reached {size}, consider clearing old data',
  },
  update: {
    banner: 'New version available - refresh to get the latest',
    reload: 'Refresh',
  },
  common: {
    none: 'None',
    price: 'Price',
    delete: 'Delete',
    cancel: 'Cancel',
    confirm: 'OK',
    retry: 'Retry',
    more: 'More',
    knowledge: 'Knowledge',
    knowledgeTitle: 'Open the trading knowledge base (new tab)',
    close: 'Close',
    backToLatest: 'Back to latest',
    feedback: 'Feedback',
    feedbackTitle: 'Report issues / suggestions (open GitHub Issues)',
  },
  scale: { linear: 'Linear', log: 'Log', title: 'Price scale: Linear / Log' },
  status: {
    loading: 'Loading history…',
    connecting: 'Connecting to live data…',
    live: 'Live',
    reconnecting: 'Reconnecting…',
    closed: 'Disconnected',
    error: 'Load failed',
    chartError: 'Failed to load market data: check network or data source',
    noData: 'No data',
    loadDemo: 'Load demo data',
    frameDrop: 'Frame drop {rate}',
    frameDropUnit: 'dropped',
    refilling: 'Backfilling gaps {done}/{total}',
    depthLoading: 'Loading order book…',
    vpNotEnough: 'Not enough data',
    replayNotEnough: 'Not enough data (needs ≥30 candles)',
  },
  disclaimer: {
    text: 'Data for reference only; not investment advice.',
    watermark: 'For reference only · Not investment advice',
  },
  settings: {
    watermark: 'Chart watermark',
    watermarkTitle: 'Show/hide the disclaimer watermark on the chart',
    snapshot: 'Settings snapshot',
    snapshotExport: 'Export settings',
    snapshotExportTitle: 'Export all settings (theme/favorites/drawings/account) as JSON',
    snapshotImport: 'Import settings',
    snapshotImportTitle: 'Restore settings from a JSON snapshot (reloads)',
  },

  chartType: {
    candlestick: 'Candles',
    line: 'Line',
    area: 'Area',
  },
  chart: {
    subScaleFixed: 'Fix Y',
    subScaleToggleTitle: 'Lock sub-pane Y-axis to theoretical extremes (stable overbought/oversold comparison); click again for auto',
    sessionLines: 'Session H/L',
  },
  group: {
    type: 'Type',
    main: 'Main',
    sub: 'Sub',
  },
  period: {
    '1s': '1s',
    '1m': '1m',
    '3m': '3m',
    '5m': '5m',
    '15m': '15m',
    '30m': '30m',
    '1h': '1h',
    '2h': '2h',
    '4h': '4h',
    '12h': '12h',
    '1d': '1D',
    '3d': '3D',
    '1w': '1W',
    '1M': '1M',
  },
  lang: {
    switchTo: 'Switch language (5)',
  },
  tz: { title: 'Time axis timezone', utc: 'UTC', local: 'Local' },
  theme: {
    switchTitle: 'Toggle theme',
    toLight: 'Light',
    toDark: 'Dark',
    toAuto: 'Auto (follow system)',
    toSchedule: 'Schedule',
    scheduleDark: 'Dark at',
    scheduleLight: 'Light at',
    pickTitle: 'Theme color presets (accent / up / down)',
    presetClassic: 'Classic blue',
    presetAShare: 'Red up / green down',
    presetPurple: 'Purple',
    presetTeal: 'Teal',
    highContrastTitle: 'High contrast',
    highContrastOn: 'HC',
    highContrastOff: 'Std',
  },
  layout: {
    switchTitle: 'Switch layout (single/pair/quad)',
    presets: 'Layout presets',
    presetName: 'Name',
    savePreset: 'Save',
    applyPreset: 'Apply',

    compare: 'Compare',
    compareTitle: 'Compare overlay symbol',
    compareOff: 'Off',
    fontScale: 'Font size',
    fontScaleTitle: 'Chart font scale',
    single: 'Single',
    pair: 'Pair',
    quad: 'Quad',
  },
  fullscreen: {
    enter: 'Fullscreen',
    exit: 'Exit fullscreen',
  },
  panel: {
    position: 'Position',
    positionTitle: 'Simulated position (entry/TP/SL lines)',
    alerts: 'Alerts',
    alertsTitle: 'Price alerts',
    depth: 'Depth',
    depthTitle: 'Order book depth',

    orderBook: 'Book',
    orderBookTitle: 'Order book ladder',
    tape: 'Time & Sales',
    tapeTitle: 'Recent time & sales (price/qty/time)',
    vp: 'VPVR',
    vpTitle: 'Volume profile (VPVR)',
    settings: 'Settings',
    sentiment: 'Sentiment',
    sentimentTitle: 'Derivatives sentiment (long/short, taker, OI)',
    statsTitle: 'Market overview (last price / change / funding / OI)',
    resizeTitle: 'Drag to resize side panel',
    orderTitle: 'Side panel order',
    moveUp: 'Move up',
    moveDown: 'Move down',

  },
  perf: {
    candleCap: 'Candle cap',
    candleCapTitle: 'Downsample rendering beyond this count (0=off)',
    off: 'Auto off',
    panelTitle: 'Perf diagnostics',
    noFrame: 'No live frames (no WS feed)',
    fps: 'FPS',
    dropRate: 'Drop rate',
    avgInterval: 'Avg interval',
    window: 'Window',
    warn: '⚠ Drop rate > 10% — chart may stutter (weak network / too much data)',
  },
  changelog: {
    title: 'Version history',
    v040r1: 'I-stage finale: scheduled theme switching (auto dark/light at set times), drawing deep-links, template market, snapshot gallery; stages A–I complete (I3 cloud sync / I11 mobile widget await external capability)',
    v04r1: 'Data integrity: period-aligned timestamps / right-side anchoring / gap refill / multi-chart crosshair sync / latency indicator',
    v04r2: 'Indicator engine: crosshair values / signal markers / threshold shading / W%R·TRIX·DPO·Vortex / import-export / favorites',
    v04r3: 'Drawing: screenshot export / group lock / batch operations / cross-symbol templates / 4-way snap / undo depth / coord badge',
    v04r4: 'Paper trading: hedged positions / dynamic margin rate / configurable fees & slippage / P&L stats / liquidation warning / fee breakdown / snapshot / profit target / JSON migration',
    v04r5: 'Alerts: channels / multi-symbol monitoring / group toggle / templates / expiry / batch ops / trigger count / precision / badge / shortcut / note',
    v04r6: 'Accessibility: focus trap / high contrast / stats bar config / resizable sidebar / panel order / layout presets',
    v04r7: 'Performance: docs-site component tests / adaptive downsampling / jank diagnostics / leak audit',
    v04r8: 'Settings snapshot export/import / shortcut print card',
    v03r1: 'P3/P4 deepening: 49 drawing tools, 26 indicators, market replay, paper trading, price alerts, mobile gestures, PWA',
    v03r2: 'Keyboard a11y: global tab flow, roving tabindex, focus-visible, aria-pressed',
    v02r1: 'Knowledge base: 27 chapters / docs site launch / 5-language i18n',
    v01r1: 'Real-time K-line terminal: 14 periods, depth, multi-chart layouts, CSV export, themes',
  },
  snap: {
    title: 'Snapshot gallery',
    save: 'Save snapshot',
    saved: 'Saved to gallery',
    preview: 'Preview',
    download: 'Download image',
    empty: 'No snapshots yet — click “Save snapshot” (top-right of chart) to keep a comparison image',
    clearAll: 'Clear all',
  },
  pinned: {
    title: 'Pinned live prices',
    titleShort: 'Pinned',
    empty: 'No pinned symbols — tap ★ in the watchlist to pin',
    unpin: 'Unpin',
    add: 'Pin',
    sortBy: 'Sort',
    sortPrice: 'Price',
    sortChange: 'Change',
  },
  sentiment: {
    globalRatio: 'Global long/short',
    topTraderRatio: 'Top trader ratio',
    takerRatio: 'Taker buy/sell',
    openInterest: 'OI 24h',
    long: 'Long',
    buy: 'Buy',
    loading: 'Loading…',
  },
  share: {
    title: 'Copy share link for current symbol, period & selected drawing',
    copy: 'Share',
    copied: 'Copied',
    exportTitle: 'Export OHLCV CSV for current symbol/period (with active indicator columns)',
    export: 'Export',
    exported: 'Exported',
  },
  csv: {
    exportRange: 'Export range',
    all: 'All',
  },
  docs: {
    title: 'Documentation',
    knowledge: 'Knowledge base',
    knowledgeDesc: '27 chapters, 201 articles: spot/futures/stocks/crypto/forex/options/macro/quant',
    readme: 'Project README',
    readmeDesc: 'Features, tech stack, quick start',
    repo: 'GitHub repo',
    repoDesc: 'Source, Issues, Pull Requests',
    changelog: 'CHANGELOG',
    changelogDesc: 'Full change history',
  },
  replay: {
    start: 'Replay',
    title: 'Step-by-step history replay',
    play: 'Play',
    pause: 'Pause',
    exit: 'Exit replay',
  },
  drawing: {
    snap: 'Snap to candles',
    snapOn: 'On',
    snapOff: 'Off',
    snapTime: 'Time align',
    snapGrid: 'Grid align',
    snapOhlc: 'OHLC align',
    noteToggle: 'Show/hide notes',
    coordBadge: 'Show coords',
    group: 'Draw',
    mouse: 'Mouse',
    horizontal: 'Horizontal',
    vertical: 'Vertical',
    cross: 'Cross line',
    trend: 'Trend line',
    extended: 'Extended line',
    angle: 'Trend Angle',
    channel: 'Channel',
    fib: 'Fibonacci',
    text: 'Text',
    note: 'Note',
    defaultText: 'Text',
    rect: 'Rectangle',
    ellipse: 'Ellipse',
    circle: 'Circle',
    triangle: 'Triangle',
    wedge: 'Wedge',
    arc: 'Arc',
    bezier: 'Bezier curve',
    ray: 'Ray',
    hray: 'Horizontal Ray',
    vray: 'Vertical Ray',
    parray: 'Parallel Ray',
    pchannel: 'Width Channel',
    fibext: 'Fib extension',
    fibchannel: 'Fib Channel',
    fibfan: 'Fib fan',
    fibTime: 'Fib time',
    cycle: 'Cycle Lines',
    timerange: 'Time Range',
    pband: 'Price Band',
    pricerange: 'Price Range',
    fibtz: 'Fib Time Zones',
    gann: 'Gann fan',
    gannbox: 'Gann Box',
    pricelabel: 'Price label',
    arrow: 'Arrow',
    polyline: 'Polyline',
    measure: 'Measure',
    measureBars: '{bars} bars',
    rr: 'Risk/Reward',
    positionTool: 'Position Tool',
    forecast: 'Forecast',
    daterange: 'Date Range',
    rrRatio: 'R:R 1:{ratio}',
    speedlines: 'Speed Lines',
    regchan: 'Regression Channel',
    hchannel: 'Horizontal Channel',
    pitchfork: 'Andrews Pitchfork',
    xabcd: 'XABCD Pattern',
    elliott: 'Elliott Wave',
    editText: 'Edit text',
    defaultColor: 'Default color',
    searchPlaceholder: 'Search drawing tools…',
    searchNoMatch: 'No matching tools',
    textPlaceholder: 'Text content',
    fontSize: 'Font size',
    color: 'Color',
    textBg: 'Background',
    align: 'Align',
    alignLeft: 'Left',
    alignCenter: 'Center',
    alignRight: 'Right',
    textDefault: 'Default',
    favoriteToggle: 'Toggle favorite',
    screenshot: 'Shot',
    share: 'Share',
    shareTitle: 'Share this chart (system share; falls back to download)',
    screenshotTitle: 'Share screenshot',
    screenshotRegion: 'Crop',
    screenshotScale: 'Screenshot resolution',
    regionHint: 'Drag a region on the chart, release to export PNG',
  },
  layers: {
    title: 'Drawing Layers',
    undo: 'Undo',
    redo: 'Redo',
    undoDepth: 'Undo steps',
    undoDepthTitle: 'Drawing undo depth (1=last step only, max 200)',
    globalOpacity: 'Global opacity',
    delete: 'Delete',
    batchCount: '{n} selected',
    select: 'Select',
    copy: 'Copy drawing',
    paste: 'Paste drawing',
    opacity: 'Opacity',
    followLatest: 'Follow latest price',
    ungrouped: 'Ungrouped',
    back: 'Back to tools',
    show: 'Show',
    hide: 'Hide',
    lock: 'Lock',
    unlock: 'Unlock',
    clearAll: 'Clear all',
    showAll: 'Show all',
    hideAll: 'Hide all',
    export: 'Export drawings JSON',
    import: 'Import drawings JSON',
    importSymbolMismatch: 'Import failed: the file belongs to another symbol',
    importBad: 'Import failed: unrecognized file format',
    empty: 'No drawings',
    search: 'Search drawings by name/type…',
    rename: 'Rename',
    templateTitle: 'Drawing templates',
    templateSave: 'Save as template',
    templateNamePlaceholder: 'Template name (e.g. Trend+Fibo)',
    templateApply: 'Apply',
    templateEmpty: 'No templates yet — save a drawing set to reuse it on any symbol',
    templateSaved: 'Template saved',
    templateExport: 'Export templates',
    templateImport: 'Import templates',
    templateImportDone: 'Imported',
    templateImportFail: 'Import failed: invalid file format',
    statsLine: 'Lines {count} · length {len}',
    statsArea: 'Areas {count} · area {area}',
  },
  symbol: {
    searchPlaceholder: 'Search symbols…',
    popular: 'Popular',
    favorites: 'Favorites',
    favoriteAdd: 'Add to favorites',
    favoriteRemove: 'Remove from favorites',
    searchResults: 'Results {count}',
    noMatch: 'No matching symbols',
  },
  indicator: {
    settings: 'Indicator settings',
    resetDefault: 'Reset defaults',
    recommend: 'Smart pick',
    recommendTitle: 'Recommend main/sub indicators by trend & volatility',
    recTrending: 'Trending',
    recRanging: 'Ranging',
    recVolatile: 'High volatility',
    drawingSuggest: 'From drawings',
    drawingSuggestTitle: 'Suggest indicators from this symbol\'s drawings',
    recSemanticsRange: 'Range / channel',
    recSemanticsTrend: 'Trend lines',
    recSemanticsLevels: 'Support / resistance',
    recSemanticsTime: 'Time structure',
    noParams: 'No adjustable parameters for this indicator',
    maPeriods: '{name} period (comma-separated)',
    maOverlayEma: 'Overlay EMA on MA',
    subOverlay: 'Sub overlay indicator',
    bollPeriod: 'BOLL period',
    bollMult: 'BOLL std-dev multiplier',
    macdFast: 'MACD fast',
    macdSlow: 'MACD slow',
    macdSignal: 'MACD signal',
    kdjN: 'KDJ N',
    kdjM1: 'KDJ M1',
    kdjM2: 'KDJ M2',
    rsiPeriod: 'RSI period',
    wrPeriod: 'WR period',
    obvMaPeriod: 'OBV smoothing (1=raw)',
    volMaPeriod: 'VOL MA period (1=off)',
    atrPeriod: 'ATR period',
    dmiPeriod: 'DMI period',
    cciPeriod: 'CCI period',
    psyPeriod: 'PSY period',
    stochK: 'STOCH %K period',
    stochSmooth: 'STOCH smoothing',
    stochD: 'STOCH %D period',
    rocPeriod: 'ROC period',
    momPeriod: 'MOM period',
    bbwPeriod: 'BBW period',
    bbwMult: 'BBW std-dev multiple',
    stPeriod: 'ST period',
    stMult: 'ST multiplier',
    sarAfStart: 'SAR initial AF',
    sarAfStep: 'SAR AF step',
    sarAfMax: 'SAR AF max',
    ichimokuTenkan: 'Tenkan period',
    ichimokuKijun: 'Kijun period',
    ichimokuSpanB: 'Senkou B period',
    ichimokuDisplacement: 'Displacement',
    sar: 'SAR',
    ichimoku: 'Ichimoku',
    mfiPeriod: 'MFI period',
    trixPeriod: 'TRIX period',
    dpoPeriod: 'DPO period',
    vortexPeriod: 'Vortex period',
    aoFast: 'AO fast',
    aoSlow: 'AO slow',
    cmfPeriod: 'CMF period',
    donchianPeriod: 'Donchian channel period',
    aroonPeriod: 'Aroon period',
    aroon: 'Aroon',
    reset: 'Reset to defaults',
    preset: 'Parameter presets',
    presetPlaceholder: 'Load preset…',
    presetSave: 'Save',
    presetSaved: 'Saved',
    presetName: 'Preset name',
    exportParams: 'Export params',
    importParams: 'Import params',
    importParamsOk: 'Params imported',
    lineColors: 'Line colors',
    lineColorFor: '{name} line color',
    lineColorReset: 'Reset to default',
  },
  stats: {
    lastPrice: 'Last price',
    change24h: '24h change',
    high24h: '24h high',
    low24h: '24h low',
    volume24h: '24h vol',
    fundingRate: 'Funding rate',
    fundingNext: 'until next settlement',
    openInterest: 'Open interest',
    markPrice: 'Mark price',
    countdown: 'Close in',
    marketPerp: 'Perp',
    marketSpot: 'Spot',
    volumeSurge: 'Volume',
    volumeSurgeTitle: 'Latest volume vs 20-bar average ≥ 3× flags a volume surge',
    gapHealthy: 'Data OK',
    gapPartial: 'Some gaps',
    gapDegraded: 'Data gaps: {n}',
    dataLatency: 'Data delay',
    dataLatencyTitle: 'Live frame lagging over 5s — data may be stalled or network is slow',
        marketType: 'Market type',
    health: 'Data health',
    summary: 'Overview',
    barConfig: 'Info bar items',
  },
  marketList: {
    title: 'Markets',
    addFavorite: 'Add favorite',
    removeFavorite: 'Remove favorite',
    pair: 'Pair',
    lastPrice: 'Last',
    change24h: '24h Chg',
    volume: 'Volume',
    refresh: 'Refresh',
    loading: 'Loading…',
    empty: 'No market data',
    collapse: 'Collapse',
    expand: 'Expand',
    sortTitle: 'Click to sort',
    stale: 'Refresh failed, showing cache',
    searchPlaceholder: 'Filter symbols…',
    tabAll: 'All',
    tabFavorites: 'Favorites',
    tabRank: 'Ranking',
    rankChange: 'Top gainers',
    rankVolume: 'Top volume',
    rotate: 'Auto-rotate',
    rotateTitle: 'Cycle the current symbol through the visible list every 5s',
    favoritesEmpty: 'No favorites yet — tap ★ in the symbol picker to add some',
    noMatch: 'No matching symbols',
  },

  position: {
    title: 'Simulated position',
    close: 'Close',
    long: 'Long',
    short: 'Short',
    hedgeMode: 'Hedge mode',
    balance: 'Available balance',
    unrealized: 'Unrealized PnL',
    qtyShort: 'Qty',
    noPosition: 'No position',
    closeAll: 'Close all',
    otherSymbols: 'Other symbols',
    levels: 'TP/SL',
    levelBreakeven: 'Breakeven stop',
    levelClearHint: 'Leave blank to clear a line',
    reduce: 'Reduce',
    reduceQty: 'Reduce qty',
    reduceErr: 'Quantity must be > 0 and no more than the position size',
    levelTrail: 'Trailing stop',
    levelTrailHint: 'The stop advances with the latest price, keeping a t% distance (reversed for shorts) and never loosens',
    levelErrInvalid: 'Take-profit and stop-loss must sit on the correct side of the entry',
    levelErrCrossed: 'Take-profit and stop-loss cannot be equal',
    switch: 'Switch',
    entry: 'Entry',
    market: 'Market',
    quantity: 'Qty',
    tpPct: 'TP%',
    slPct: 'SL%',
    tpLine: 'TP line',
    tpPrice: 'TP price',
    slPrice: 'SL price',
    levelModePct: 'Percent',
    levelModePrice: 'Price',
    slLine: 'SL line',
    floatingPnl: 'Float PnL',
    open: 'Open',
    lineEntry: 'Entry',
    lineTp: 'TP',
    lineSl: 'SL',
    margin: 'Margin',
    leverage: 'Leverage',
    liqPrice: 'Liq. price',
    marginRate: 'Margin rate',
    liqWarn: 'Liq. warning',
    liqCritical: 'Liq. critical',
    reverse: 'Reverse',
    todayPnl: 'Today PnL',
  },
  alert: {
    history: 'Trigger history',
    sound: 'Alert sound',
    clearHistory: 'Clear history',
    title: 'Price alert · {symbol}',
    granted: 'Notifications on',
    unsupported: 'Notifications unsupported',
    enable: 'Enable',
    above: 'Price ≥',
    below: 'Price ≤',
    add: 'Add alert',
    invalid: 'Enter a valid price (positive number)',
    none: 'No alerts',
    triggered: 'Triggered',
    reset: 'Reset',
    notifyTitle: 'Kline Buty · Price alert',
    notifyAbove: '{symbol} reached {price}',
    notifyBelow: '{symbol} broke below {price}',
    repeat: 'Repeat alert',
    soundPreview: 'Preview sound',
    repeatInterval: 'Repeat interval',
    minutes: 'min',
    group: 'Group',
    groupPlaceholder: 'e.g. trend',
    sort: 'Sort',
    filter: 'Filter',
    filterAll: 'All',
    filterActive: 'Pending',
    sortTime: 'Time',
    sortPrice: 'Price',
    sortSymbol: 'Symbol',
    soundKind: 'Sound',
    soundBeep: 'Beep',
    soundChime: 'Chime',
    soundPing: 'Ping',
    soundLow: 'Low',
    timeWindow: 'Time window',
    timeFrom: 'From',
    timeTo: 'To',
    channel: 'Channel',
    channelBoth: 'Both',
    channelSystem: 'System',
    channelWeb: 'In-app',
    note: 'Note',
    notePlaceholder: 'Optional note…',
    expiresAt: 'Expires',
    precision: 'Precision',
    precisionAuto: 'Auto',
    template: 'Template',
    templateName: 'Name',
    saveTemplate: 'Save',
    applyTemplate: 'Apply',
    batch: 'Batch',
    selectAll: 'All',
    clearSelection: 'Clear',
    enableSelected: 'Enable',
    disableSelected: 'Disable',
    deleteSelected: 'Delete',
    select: 'Select',
    expired: 'Expired',
    clearExpired: 'Clear expired',
    triggerCount: 'hits',
    groupToggle: 'Toggle group',
    voice: 'Voice alerts (read price moves aloud)',
    adaptive: 'Adaptive (ATR) threshold',
    quickPct: 'Quick ±%',
    pending: 'Pending',
  },
  tooltip: {
    open: 'O',
    high: 'H',
    low: 'L',
    close: 'C',
    volume: 'Vol',
  },
  depth: {
    title: 'Order book · {symbol} (live)',
    bid: 'Bid',
    ask: 'Ask',
  },
  orderBook: {
    title: 'Order book · {symbol}',
    qty: 'Qty',
    cum: 'Total',
    group: 'Group',
    spread: 'Spread',
    markHint: 'Click to mark on main chart (click again to clear); quick-order button on the left',
    refresh: 'Refresh order book (reconnect for latest snapshot)',
    bid: 'Bid',
    ask: 'Ask',
    imbalance: 'Order-book imbalance: bid/ask share of total volume',
  },
  tape: {
    title: 'Trades · {symbol}',
    time: 'Time',
    qty: 'Qty',
    sideSummary: 'Taker buy/sell trade count in the window',
    markHint: 'Click to mark on main chart (click again to clear)',
    filterAll: 'All',
    filterBuy: 'Taker buy',
    filterSell: 'Taker sell',
    filterSideHint: 'Filter by trade direction (taker buy / taker sell)',
    bigOrder: 'Big',
    bigOrderHint: 'Show only trades whose size reaches a multiple of the window average',
    emptyFilter: 'No trades match the filter',
  },
  trade: {
    buy: "Buy",
    sell: "Sell",
    market: 'Market',
    limit: 'Limit',
    limitHint: 'A limit order fills at the prevailing price once touched (price improvement past your limit), with no slippage',
    makerFee: 'Maker fee %',
    pendingTitle: 'Open orders',
    pendingEmpty: 'No open limit orders',
    cancelOrder: 'Cancel order',
    editOrder: 'Modify',
    editErr: 'Price and quantity must be positive',
    filledToast: 'Limit order filled · {symbol} {qty} @ {price}',
    cancelledToast: 'Insufficient balance, order cancelled · {symbol}',
    tpslToast: '{symbol} {reason} hit — closed at {price}',
    tpslToastMulti: '{symbol} {reason} hit — {count} positions closed at {price}',
    tpWord: 'take-profit',
    slWord: 'stop-loss',
    tooManyOrders: 'Too many open orders for this symbol (max {max})',
    stats: 'Trade stats',
    winRate: 'Win rate',
    totalPnl: 'Total PnL',
    profitFactor: 'Profit factor',
    avgWin: 'Avg win',
    avgLoss: 'Avg loss',
    settings: 'Trade settings',
    feeRate: 'Taker fee %',
    slippage: 'Slippage %',
    target: 'Profit target',
    targetHit: 'Target hit',
    detail: 'Detail',
    notional: 'Notional',
    feeRateDetail: 'Fee rate',
    feeDetail: 'Fee',
    grossPnl: 'Gross PnL',
    netPnl: 'Net PnL',
    maxDrawdown: 'Max drawdown',
    drawdown: 'Drawdown',
    pnlBars: 'PnL per trade',
    filterSymbol: 'Symbol',
    filterSide: 'Side',
    filterQuery: 'Search price…',
    filterEmpty: 'No matching trades',
    all: 'All',
    dailyCount: 'Trades',
    dailyPnl: 'Day PnL',
    weekPnl: 'Week PnL',
    monthPnl: 'Month PnL',
    period: 'Period PnL',
    locate: 'Locate chart',
    bySymbol: 'By symbol',
  },
  paper: { title: 'Trade History', empty: 'No trades yet — open a position and fills will be logged here', open: 'Open', close: 'Close', long: 'Long', short: 'Short', clear: 'Clear', export: 'Export CSV', reset: 'Reset account', resetConfirm: 'Reset the paper account? Balance returns to 10,000 and all trades are cleared.', exportEquity: 'Export equity', equity: 'Equity curve', exportJson: 'Export JSON', importJson: 'Import JSON', importDone: 'Imported', importFail: 'Import failed: invalid file', snapshot: 'Snapshots', snapshotName: 'Name', saveSnapshot: 'Save', loadSnapshot: 'Load', deleteSnapshot: 'Del' },
  ctx: { copyPrice: 'Copy price', copyOhlc: 'Copy OHLC', addAlert: 'Add alert', limitBuy: 'Limit buy here', limitSell: 'Limit sell here', clearDrawings: 'Clear drawings', confirmClear: 'Clear all drawings for this symbol?', copied: 'Copied' },
  pull: { pull: 'Pull to refresh', release: 'Release to refresh', refreshing: 'Refreshing…' },
  panelState: { loading: 'Loading…', error: 'Failed to load', empty: 'No data', retry: 'Retry' },
  quickOrder: {
    balance: 'Available balance',
    insufficient: 'Insufficient balance: notional + fee is required as margin',
    bid: 'Bid',
    ask: 'Ask',
    title: "Quick order",
    price: "Price",
    qty: "Qty",
    notional: "Est. notional",
    fill: "Est. fill price",
    fee: "Fee (0.1%)",
    total: "Total",
    confirm: "Place order",
    hint: "Overwrites current simulated position · Enter to confirm / Esc to close",
    qtyPreset: "Quick qty",
    customPct: "Custom %",
    apply: "Apply",
    attachHint: 'Attached TP/SL (optional): applied to the position as soon as this order fills',
    attachErr: 'Take-profit/stop-loss must sit on the correct side of the limit: TP above and SL below for buys, reversed for sells',
    riskReward: 'Risk / reward {ratio} : 1',
  },
  volumeProfile: {
    title: 'Volume profile (last 300) · {symbol}',
    poc: 'POC ≈ {price}',
    bidVol: 'Bid vol',
    askVol: 'Ask vol',
    pocLabel: 'POC',
  },
  offline: {
    text: 'Network disconnected; updates paused, will auto-reconnect',
  },
  errorBoundary: {
    title: 'Chart render error',
  },
  shortcuts: {
    title: 'Keyboard Shortcuts',
    openSearch: 'Open symbol search: {key}',
    period: 'Switch period: [ / ]',
    periodPrev: 'Previous period',
    periodNext: 'Next period',
    layoutKeys: '1 / 2 / 3',
    help: 'Show/hide shortcuts',
    copyDrawing: 'Copy drawing',
    pasteDrawing: 'Paste drawing',
    configTitle: 'Configure shortcuts',
    recording: 'Press new shortcut…',
    clickToChange: 'Click then press key',
    cancel: 'Cancel recording',
    configHint: 'Click an item then press a key',
    cycleLang: 'Switch language',
    toggleAlerts: 'Toggle alerts',
    print: 'Print card',
    reset: 'Reset to default',
    conflict: 'Shortcut conflict: {names} share the same key',
    layout: 'Layout: 1 / 2 / 3 (single / pair / quad)',
    cycleMain: 'Cycle main indicator: M',
    cycleSub: 'Cycle sub indicator: N',
    fullscreen: 'Toggle fullscreen: F',
    replay: 'Replay play/pause: Space',
    replayStep: 'Replay step candle: ← / →',
    replaySpeed: 'Replay speed: ↑ / ↓',
    deleteDrawing: 'Delete selected drawing: Del',
    escape: 'Cancel / close: Esc',
    hint: 'Show/hide shortcuts: ?',
    groupNav: 'Navigation',
    groupIndicators: 'Indicators',
    groupReplay: 'Replay',
    groupDrawings: 'Drawings',
    filter: 'Filter shortcuts…',
    noMatch: 'No matching shortcuts',
  },
  app: {
    titleZh: 'Kline Buty · 实时 K 线',
    titleEn: 'Kline Buty · Live Candles',
    titleJa: 'Kline Buty · リアルタイム K 線',
    titleKo: 'Kline Buty · 실시간 K 라인',
    titleEs: 'Kline Buty · Velas en vivo',
  },
}

export const ja: Messages = {
  storage: {
    warn: 'ローカルストレージ使用量が {size} に達しました。古いデータを整理してください',
  },
  update: {
    banner: '新しいバージョンがあります——更新して最新版を読み込みます',
    reload: '更新',
  },
  common: {
    none: 'なし',
    price: '価格',
    delete: '削除',
    cancel: 'キャンセル',
    confirm: 'OK',
    retry: '再試行',
    more: 'その他',
    knowledge: 'ナレッジベース',
    knowledgeTitle: '取引ナレッジベースを開く（新しいタブ）',
    close: '閉じる',
    backToLatest: '最新へ戻る',
    feedback: 'フィードバック',
    feedbackTitle: '問題報告 / 提案（GitHub Issues を開く）',
  },
  scale: { linear: '線形', log: '対数', title: '価格軸：線形 / 対数' },
  status: {
    loading: '履歴データを読み込み中…',
    connecting: 'リアルタイム行情に接続中…',
    live: 'リアルタイム',
    reconnecting: '再接続中…',
    closed: '接続が切断されました',
    error: '読み込み失敗',
    chartError: '行情データの読み込みに失敗しました：ネットワークまたはデータソースを確認してください',
    noData: 'データなし',
    loadDemo: 'デモデータを読み込む',
    frameDrop: 'リアルタイムフレーム欠落 {rate}',
    frameDropUnit: '欠落',
    refilling: '欠損区間を補填中 {done}/{total}',
    depthLoading: '板情報を読み込み中…',
    vpNotEnough: 'データ不足',
    replayNotEnough: 'データ不足（30 本以上必要）',
  },
  disclaimer: {
    text: 'データは参考用であり、投資助言ではありません。',
    watermark: '参考用のみ・投資助言ではありません',
  },
  settings: {
    watermark: 'チャート透かし',
    watermarkTitle: 'チャートの免責事項透かしを表示/非表示',
    snapshot: '設定スナップショット',
    snapshotExport: '設定をエクスポート',
    snapshotExportTitle: 'すべての設定（テーマ/お気に入り/描画/口座）を JSON で出力',
    snapshotImport: '設定をインポート',
    snapshotImportTitle: 'JSON スナップショットから復元（再読み込み）',
  },

  chartType: {
    candlestick: 'ローソク',
    line: '折れ線',
    area: 'エリア',
  },
  chart: {
    subScaleFixed: 'Y軸固定',
    subScaleToggleTitle: '副パネルY軸を理論極値に固定（過買過売を安定比較）、再クリックで自動に戻す',
    sessionLines: '当日高安',
  },
  group: {
    type: 'タイプ',
    main: 'メイン',
    sub: 'サブ',
  },
  period: {
    '1s': '1秒',
    '1m': '1分',
    '3m': '3分',
    '5m': '5分',
    '15m': '15分',
    '30m': '30分',
    '1h': '1時間',
    '2h': '2時間',
    '4h': '4時間',
    '12h': '12時間',
    '1d': '日足',
    '3d': '3日',
    '1w': '週足',
    '1M': '月足',
  },
  lang: {
    switchTo: '言語切替（5 言語）',
  },
  tz: { title: '時間軸のタイムゾーン', utc: 'UTC', local: 'ローカル' },
  theme: {
    switchTitle: 'テーマ切替',
    toLight: 'ライト',
    toDark: 'ダーク',
    toAuto: '自動（システムに従う）',
    toSchedule: 'タイマー',
    scheduleDark: 'ダーク時間',
    scheduleLight: 'ライト時間',
    pickTitle: 'テーマカープリセット（アクセント/上昇・下落色）',
    presetClassic: 'クラシックブルー',
    presetAShare: '赤上昇・緑下落',
    presetPurple: 'パープル',
    presetTeal: 'ティール',
    highContrastTitle: 'ハイコントラスト',
    highContrastOn: 'HC',
    highContrastOff: '標準',
  },
  layout: {
    switchTitle: 'レイアウト切替（1枚/2枚/4枚）',
    presets: 'パネルレイアウト',
    presetName: '名前',
    savePreset: '保存',
    applyPreset: '適用',

    compare: '比較',
    compareTitle: '比較銘柄を重ねる',
    compareOff: 'オフ',
    fontScale: '文字サイズ',
    fontScaleTitle: 'チャート文字サイズ',
    single: '1枚',
    pair: '2枚',
    quad: '4枚',
  },
  fullscreen: {
    enter: '全画面',
    exit: '全画面終了',
  },
  panel: {
    position: 'ポジション',
    positionTitle: 'シミュレーションポジション（エントリー/TP/SL ライン）',
    alerts: 'アラート',
    alertsTitle: '価格アラート',
    depth: '板情報',
    depthTitle: '板情報デプス',

    orderBook: '板',
    orderBookTitle: '板情報（売買レベルの一覧）',
    tape: '約定',
    tapeTitle: '直近の約定一覧（価格/数量/時刻）',
    vp: 'VPVR',
    vpTitle: '出来高プロファイル（VPVR）',
    settings: '設定',
    sentiment: 'センチメント',
    sentimentTitle: 'デリバティブセンチメント（ロング/ショート、テイカー、OI）',
    statsTitle: '市場概況（最新値/変動/資金費率/未決済）',
    resizeTitle: 'ドラッグでサイドバー幅を調整',
    orderTitle: 'サイドパネル順序',
    moveUp: '上へ',
    moveDown: '下へ',

  },
  perf: {
    candleCap: 'ローソク上限',
    candleCapTitle: 'この数を超えるとダウンサンプリング（0=オフ）',
    off: '自動オフ',
    panelTitle: 'パフォーマンス診断',
    noFrame: 'リアルタイムフレームなし（WS なし）',
    fps: 'フレームレート',
    dropRate: 'ドロップ率',
    avgInterval: '平均間隔',
    window: '窓フレーム数',
    warn: '⚠ ドロップ率 10% 超——チャートがカクつく可能性（弱回線/データ過多）',
  },
  changelog: {
    title: 'バージョン履歴',
    v040r1: 'I期総仕上げ：タイマーテーマ切替（設定時刻でダーク/ライト自動切替）、描画ディープリンク、テンプレートマーケット、スナップショットギャラリー；A–I期すべて完了（I3 クラウド同期 / I11 モバイルウィジェットは外部能力待ち）',
    v04r1: 'データ整合性：周期境界アライメント / 右端アンカー / 欠損穴埋め / マルチチャート十字同期 / 遅延表示',
    v04r2: '指標エンジン：カーソル値 / シグナルマーカー / 閾値着色 / W%R·TRIX·DPO·Vortex / パラメータ入出力 / お気に入り',
    v04r3: '描画：スクリーンショット出力 / グループロック / 一括操作 / クロス銘柄テンプレート / スナップ4態 / アンドゥ深度 / 座標バッジ',
    v04r4: 'ペーパートレード：両建て / 動的証拠金率 / 手数料・スリッページ設定 / 損益統計 / 強制決済警告 / 手数料内訳 / スナップショット / 利益目標 / JSON移行',
    v04r5: 'アラート：チャネル / 複数銘柄監視 / グループ切替 / テンプレート / 期限 / 一括操作 / 発火回数 / 精度 / バッジ / ショートカット / メモ',
    v04r6: 'アクセシビリティ：フォーカストラップ / ハイコントラスト / 情報バー設定 / サイドバー幅調節 / パネル順序 / レイアウトプリセット',
    v04r7: 'パフォーマンス：ドキュメントサイト・コンポーネントテスト / 適応型ダウンサンプリング / カクつき診断 / リーク監査',
    v04r8: '設定スナップショットのエクスポート・インポート / ショートカット印刷カード',
    v03r1: 'P3/P4 深化：描画ツール49種、指標26個、マーケットリプレイ、ペーパートレード、価格アラート、モバイルジェスチャー、PWA',
    v03r2: 'キーボードアクセシビリティ：グローバルTabフロー、roving tabindex、focus-visible、aria-pressed',
    v02r1: 'ナレッジベース全27章・201篇、ドキュメントサイト公開、5言語i18n',
    v01r1: 'リアルタイムKライン端末：14期間、板情報、マルチチャート、CSV出力、テーマ',
  },
  snap: {
    title: 'スナップショットギャラリー',
    save: 'スナップショット保存',
    saved: 'ギャラリーに保存しました',
    preview: 'プレビュー',
    download: '画像をダウンロード',
    empty: 'スナップショットはありません——チャート右上の「スナップショット保存」で保存してください',
    clearAll: 'すべて削除',
  },
  pinned: {
    title: 'ピン留めリアルタイム行情',
    titleShort: 'ピン留め',
    empty: 'ピン留め銘柄なし——ウォッチリストの★でピン留め',
    unpin: 'ピン解除',
    add: 'ピン留め',
    sortBy: '並び替え',
    sortPrice: '価格',
    sortChange: '変動',
  },
  sentiment: {
    globalRatio: '全口座ロング/ショート比',
    topTraderRatio: '大口トレーダー比率',
    takerRatio: 'テイカー売買比',
    openInterest: 'OI 24h',
    long: 'ロング',
    buy: '買い',
    loading: '読み込み中…',
  },
  share: {
    title: '現在の銘柄・時間足・選択中描画の共有リンクをコピー',
    copy: '共有',
    copied: 'コピー済み',
    exportTitle: '現在の銘柄/時間足の K 線 CSV をエクスポート（有効な指標列を含む）',
    export: 'エクスポート',
    exported: 'エクスポート済み',
  },
  csv: {
    exportRange: '出力範囲',
    all: 'すべて',
  },
  docs: {
    title: 'ドキュメント索引',
    knowledge: '取引ナレッジベース',
    knowledgeDesc: '27 章 201 記事：現物/先物/株式/暗号/為替/オプション/マクロ/クオンツ',
    readme: 'プロジェクト README',
    readmeDesc: '機能、技術スタック、クイックスタート',
    repo: 'GitHub リポジトリ',
    repoDesc: 'ソース、Issue、Pull Request',
    changelog: 'CHANGELOG',
    changelogDesc: '完全な変更履歴',
  },
  replay: {
    start: 'リプレイ',
    title: '履歴を 1 本ずつ再生',
    play: '再生',
    pause: '一時停止',
    exit: 'リプレイ終了',
  },
  drawing: {
    snap: 'ローソクにスナップ',
    snapOn: 'オン',
    snapOff: 'オフ',
    snapTime: '時間合わせ',
    snapGrid: 'グリッド合わせ',
    snapOhlc: 'OHLC 合わせ',
    noteToggle: 'メモ表示切替',
    coordBadge: '座標ラベル常時表示',
    group: '描画',
    mouse: 'マウス',
    horizontal: '水平線',
    vertical: '垂直線',
    cross: '十字線',
    trend: 'トレンドライン',
    extended: '延長ライン',
    angle: 'トレンド角度',
    channel: 'チャネル',
    fib: 'フィボナッチ',
    text: 'テキスト',
    note: 'メモ',
    defaultText: 'テキスト',
    rect: '矩形',
    ellipse: '楕円',
    circle: '円',
    triangle: '三角形',
    wedge: 'くさび形',
    arc: '円弧',
    bezier: 'ベジェ曲線',
    ray: 'レイ',
    hray: '水平レイ',
    vray: '垂直レイ',
    parray: '平行レイ',
    pchannel: '幅チャンネル',
    fibext: 'フィボナッチ拡張',
    fibchannel: 'フィボナッチチャネル',
    fibfan: 'フィボナッチ扇形',
    fibTime: 'フィボナッチ時間',
    cycle: 'サイクルライン',
    timerange: '時間範囲',
    pband: 'プライスバンド',
    pricerange: '価格レンジ',
    fibtz: 'フィボナッチタイムゾーン',
    gann: 'ギャン角度線',
    gannbox: 'ギャン箱',
    pricelabel: '価格ラベル',
    arrow: '矢印',
    polyline: '折れ線',
    measure: '計測',
    measureBars: '{bars}本',
    rr: 'リスクリワード',
    positionTool: 'ポジションツール',
    forecast: 'フォーキャスト',
    daterange: '日付範囲',
    rrRatio: 'リスクリワード 1:{ratio}',
    speedlines: 'スピードライン',
    regchan: '回帰チャネル',
    hchannel: '水平チャネル',
    pitchfork: 'アンドリュースピッチフォーク',
    xabcd: 'XABCD パターン',
    elliott: 'エリオット波動',
    editText: 'テキスト編集',
    defaultColor: 'デフォルト色',
    searchPlaceholder: '描画ツールを検索…',
    searchNoMatch: '一致するツールなし',
    textPlaceholder: 'テキスト内容',
    fontSize: '文字サイズ',
    color: '色',
    textBg: '背景色',
    align: '配置',
    alignLeft: '左寄せ',
    alignCenter: '中央',
    alignRight: '右寄せ',
    textDefault: 'デフォルト',
    favoriteToggle: 'お気に入り切替',
    screenshot: 'スクショ',
    share: '共有',
    shareTitle: 'チャートを共有（対応時はシステム共有、非対応はダウンロード）',
    screenshotTitle: 'スクリーンショット共有',
    screenshotRegion: '範囲指定',
    screenshotScale: 'スクリーンショット解像度',
    regionHint: 'チャートの領域をドラッグ選択、離すと PNG を書き出し',
  },
  layers: {
    title: '画層管理',
    undo: '元に戻す',
    redo: 'やり直す',
    undoDepth: '元に戻す歩数',
    undoDepthTitle: '図形の元に戻す深さ（1=直前のみ、上限 200）',
    globalOpacity: '全体の不透明度',
    delete: '削除',
    batchCount: '{n} 件選択中',
    select: '選択',
    copy: '図形をコピー',
    paste: '図形を貼り付け',
    opacity: '不透明度',
    followLatest: '最新値に追従',
    ungrouped: '未グループ',
    back: 'ツールに戻る',
    show: '表示',
    hide: '非表示',
    lock: 'ロック',
    unlock: 'ロック解除',
    clearAll: 'すべて削除',
    showAll: 'すべて表示',
    hideAll: 'すべて非表示',
    export: '描画を書き出し',
    import: '描画を読み込み',
    importSymbolMismatch: '読み込み失敗：ファイルは他の銘柄のものです',
    importBad: '読み込み失敗：ファイル形式を認識できません',
    empty: '画線なし',
    search: '名前/種類で画線を検索…',
    rename: '名前を変更',
    templateTitle: '画線テンプレート',
    templateSave: 'テンプレートとして保存',
    templateNamePlaceholder: 'テンプレート名（例：トレンド＋フィボ）',
    templateApply: '適用',
    templateEmpty: 'テンプレートなし——現在の画線を保存すると任意の銘柄に再利用できます',
    templateSaved: 'テンプレートを保存しました',
    templateExport: 'テンプレートをエクスポート',
    templateImport: 'テンプレートをインポート',
    templateImportDone: 'インポート済み',
    templateImportFail: 'インポート失敗：ファイル形式が無効です',
    statsLine: '線 {count} · 長さ {len}',
    statsArea: '面 {count} · 面積 {area}',
  },
  symbol: {
    searchPlaceholder: '銘柄を検索…',
    popular: '人気',
    favorites: 'お気に入り',
    favoriteAdd: 'お気に入りに追加',
    favoriteRemove: 'お気に入りから削除',
    searchResults: '検索結果 {count}',
    noMatch: '一致する銘柄なし',
  },
  indicator: {
    settings: '指標パラメータ',
    resetDefault: 'デフォルトに戻す',
    recommend: 'スマート推奨',
    recommendTitle: 'トレンド/ボラティリティで主副指標を推薦',
    recTrending: 'トレンド相場',
    recRanging: 'レンジ相場',
    recVolatile: '高ボラ',
    drawingSuggest: '描画から',
    drawingSuggestTitle: '現在の銘柄の描画から指標を提案',
    recSemanticsRange: 'レンジ/チャネル',
    recSemanticsTrend: 'トレンド線',
    recSemanticsLevels: 'サポート/レジスタンス',
    recSemanticsTime: '時間構造',
    noParams: 'この指標に調整可能なパラメータはありません',
    maPeriods: '{name} 周期（カンマ区切り）',
    maOverlayEma: 'MA に EMA を重ねる',
    subOverlay: '副図の重ね合わせ指標',
    bollPeriod: 'BOLL 周期',
    bollMult: 'BOLL 標準偏差倍率',
    macdFast: 'MACD 短期線',
    macdSlow: 'MACD 長期線',
    macdSignal: 'MACD シグナル',
    kdjN: 'KDJ N',
    kdjM1: 'KDJ M1',
    kdjM2: 'KDJ M2',
    rsiPeriod: 'RSI 周期',
    wrPeriod: 'WR 周期',
    obvMaPeriod: 'OBV 平滑化（1=生値）',
    volMaPeriod: 'VOL 移動平均期間（1=オフ）',
    atrPeriod: 'ATR 周期',
    dmiPeriod: 'DMI 周期',
    cciPeriod: 'CCI 周期',
    psyPeriod: 'PSY 周期',
    stochK: 'STOCH %K 周期',
    stochSmooth: 'STOCH 平滑化',
    stochD: 'STOCH %D 周期',
    rocPeriod: 'ROC 周期',
    momPeriod: 'MOM 周期',
    bbwPeriod: 'BBW 期間',
    bbwMult: 'BBW 標準偏差倍率',
    stPeriod: 'ST 期間',
    stMult: 'ST 乗数',
    sarAfStart: 'SAR 初期加速',
    sarAfStep: 'SAR 加速ステップ',
    sarAfMax: 'SAR 加速上限',
    ichimokuTenkan: '転換線周期',
    ichimokuKijun: '基準線周期',
    ichimokuSpanB: '先行帯B周期',
    ichimokuDisplacement: '先行スパン移動周期',
    sar: 'SAR',
    ichimoku: '一目均衡表',
    mfiPeriod: 'MFI 周期',
    trixPeriod: 'TRIX 期間',
    dpoPeriod: 'DPO 期間',
    vortexPeriod: 'Vortex 期間',
    aoFast: 'AO 短期線',
    aoSlow: 'AO 長期線',
    cmfPeriod: 'CMF 周期',
    donchianPeriod: 'ドンチャンチャネル周期',
    aroonPeriod: 'Aroon 周期',
    aroon: 'Aroon',
    reset: 'デフォルトにリセット',
    preset: 'パラメータプリセット',
    presetPlaceholder: 'プリセットを読み込む…',
    presetSave: '保存',
    presetSaved: '保存済み',
    presetName: 'プリセット名',
    exportParams: 'パラメータ出力',
    importParams: 'パラメータ読込',
    importParamsOk: 'パラメータを読込みました',
    lineColors: 'ライン色',
    lineColorFor: '{name} ラインの色',
    lineColorReset: 'デフォルトに戻す',
  },
  stats: {
    lastPrice: '最新価格',
    change24h: '24h 変動',
    high24h: '24h 高値',
    low24h: '24h 安値',
    volume24h: '24h 出来高',
    fundingRate: '資金調達率',
    fundingNext: '次回資金決済まで',
    openInterest: '未決済建玉',
    markPrice: 'マーク価格',
    countdown: '終了まで',
    marketPerp: '先物',
    marketSpot: '現物',
    volumeSurge: '出来高',
    volumeSurgeTitle: '最新出来高 ÷ 直近20本平均 ≥ 3倍で出来高急増と判定',
    gapHealthy: 'データ正常',
    gapPartial: '軽微な欠落',
    gapDegraded: 'データ欠落 {n} 箇所',
    dataLatency: '遅延',
    dataLatencyTitle: 'リアルタイムフレームが 5 秒以上遅延——データ停止または弱回線の可能性',
        marketType: '市場タイプ',
    health: 'データ健全性',
    summary: '相場概況',
    barConfig: '情報バー項目',
  },
  marketList: {
    title: '相場',
    addFavorite: 'お気に入りに追加',
    removeFavorite: 'お気に入りから削除',
    pair: 'ペア',
    lastPrice: '最新価格',
    change24h: '24h 変動',
    volume: '出来高',
    refresh: '更新',
    loading: '読込中…',
    empty: '相場データなし',
    collapse: '閉じる',
    expand: '開く',
    sortTitle: 'クリックで並べ替え',
    stale: '更新失敗、キャッシュ表示',
    searchPlaceholder: '銘柄を絞り込み…',
    tabAll: 'すべて',
    tabFavorites: 'お気に入り',
    tabRank: 'ランキング',
    rankChange: '上昇率',
    rankVolume: '出来高',
    rotate: '自動巡回',
    rotateTitle: '5 秒ごとに表示中のリストを次の銘柄へ巡回',
    favoritesEmpty: 'お気に入りなし——銘柄一覧で ★ で追加できます',
    noMatch: '一致する銘柄なし',
  },

  position: {
    title: 'シミュレーションポジション',
    close: '決済',
    long: 'ロング',
    short: 'ショート',
    hedgeMode: 'ヘッジモード',
    balance: '利用可能残高',
    unrealized: '評価損益',
    qtyShort: '数量',
    noPosition: 'ポジションなし',
    closeAll: '全決済',
    otherSymbols: '他銘柄のポジション',
    levels: '利確/損切り',
    levelBreakeven: '損切りを建値へ',
    levelClearHint: '空欄でその線を解除',
    reduce: '部分決済',
    reduceQty: '部分決済数量',
    reduceErr: '数量は 0 超かつ建玉数量以下にしてください',
    levelTrail: 'トレーリング損切り',
    levelTrailHint: '損切りは最新価格に追従して t% の距離を保ち、逆行しても緩みません',
    levelErrInvalid: '利確・損切りは建値に対して適切な側に置いてください',
    levelErrCrossed: '利確と損切りの価格は同じにできません',
    switch: '切替',
    entry: 'エントリー価格',
    market: '現在価格',
    quantity: '数量',
    tpPct: 'TP%',
    slPct: 'SL%',
    tpLine: 'TP ライン',
    tpPrice: '利確価格',
    slPrice: '損切価格',
    levelModePct: 'パーセント',
    levelModePrice: '価格',
    slLine: 'SL ライン',
    floatingPnl: '浮動損益',
    open: 'エントリー',
    lineEntry: 'エントリー',
    lineTp: '利確 (TP)',
    lineSl: '損切り (SL)',
    margin: '証拠金',
    leverage: 'レバレッジ',
    liqPrice: '強制決済価格',
    marginRate: '証拠金率',
    liqWarn: '強制決済警告',
    liqCritical: '強制決済リスク',
    reverse: '反対売買',
    todayPnl: '本日実現損益',
  },
  alert: {
    history: '発火履歴',
    sound: 'アラート音',
    clearHistory: '履歴を削除',
    title: '価格アラート · {symbol}',
    granted: '通知オン',
    unsupported: 'この環境では通知を利用できません',
    enable: '通知を有効化',
    above: '価格 ≥',
    below: '価格 ≤',
    add: 'アラート追加',
    invalid: '有効な価格を入力（正の数）',
    none: 'アラートなし',
    triggered: '発動済み',
    reset: 'リセット',
    notifyTitle: 'Kline Buty · 価格アラート',
    notifyAbove: '{symbol} が {price} に到達',
    notifyBelow: '{symbol} が {price} を下回りました',
    repeat: '繰り返し通知',
    soundPreview: '音を試聴',
    repeatInterval: '繰り返し間隔',
    minutes: '分',
    group: 'グループ',
    groupPlaceholder: '例：トレンド',
    sort: '並び替え',
    filter: '絞り込み',
    filterAll: 'すべて',
    filterActive: '未発動',
    sortTime: '時間',
    sortPrice: '価格',
    sortSymbol: '銘柄',
    soundKind: '音色',
    soundBeep: 'ビープ',
    soundChime: 'チャイム',
    soundPing: 'ピン',
    soundLow: 'ロー',
    timeWindow: '時間帯',
    timeFrom: '開始',
    timeTo: '終了',
    channel: '通知チャネル',
    channelBoth: '両方',
    channelSystem: 'システム通知',
    channelWeb: 'アプリ内',
    note: 'メモ',
    notePlaceholder: '任意の説明…',
    expiresAt: '有効期限',
    precision: '価格精度',
    precisionAuto: '自動',
    template: 'テンプレート',
    templateName: '名前',
    saveTemplate: '保存',
    applyTemplate: '適用',
    batch: '一括',
    selectAll: 'すべて選択',
    clearSelection: 'クリア',
    enableSelected: '選択を有効化',
    disableSelected: '選択を無効化',
    deleteSelected: '選択を削除',
    select: '選択',
    expired: '期限切れ',
    clearExpired: '期限切れを削除',
    triggerCount: '発動回数',
    groupToggle: 'グループ切替',
    voice: '音声アラート（価格変動を読み上げ）',
    adaptive: 'ボラティリティ適応（ATR 閾値）',
    quickPct: 'クイック ±%',
    pending: '待機中',
  },
  tooltip: {
    open: '始',
    high: '高',
    low: '安',
    close: '終',
    volume: '出来高',
  },
  depth: {
    title: '板情報デプス · {symbol}（リアルタイム）',
    bid: '買い',
    ask: '売り',
  },
  orderBook: {
    title: '板情報オーダーブック · {symbol}',
    qty: '数量',
    cum: '累計',
    group: 'グループ',
    spread: 'スプレッド',
    markHint: 'クリックでメインにマーク（再クリックで解除）；左ボタンでクイック注文',
    refresh: '板を更新（再接続して最新スナップショット取得）',
    bid: '買い',
    ask: '売り',
    imbalance: '板の買売不均衡：買/売の合計に占める割合',
  },
  tape: {
    title: '直近約定 · {symbol}',
    time: '時刻',
    qty: '数量',
    sideSummary: '表示期間内の主動買い/売り約定件数',
    markHint: 'クリックでメインにマーク（再クリックで解除）',
    filterAll: 'すべて',
    filterBuy: '主動買い',
    filterSell: '主動売り',
    filterSideHint: '約定方向で絞り込む（主動買い/売り）',
    bigOrder: '大口',
    bigOrderHint: '窗口平均数量の一定倍以上の約定のみ表示',
    emptyFilter: '条件に合う約定はありません',
  },
  trade: {
    buy: "買い",
    sell: "売り",
    market: '成行',
    limit: '指値',
    limitHint: '指値注文は到達時点の現在価格で約定します（指定価格より有利なら価格改善）。スリッページなし',
    makerFee: 'メイカー手数料 %',
    pendingTitle: '出信中の注文',
    pendingEmpty: '指値注文はありません',
    cancelOrder: '注文を取消',
    editOrder: '注文変更',
    editErr: '価格と数量は正の数してください',
    filledToast: '指値注文が約定しました · {symbol} {qty} @ {price}',
    cancelledToast: '残高不足のため注文を取消しました · {symbol}',
    tpslToast: '{symbol} {reason}に到達し、{price} で決済しました',
    tpslToastMulti: '{symbol} {reason}に到達、{count} 件を {price} で決済しました',
    tpWord: '利確',
    slWord: '損切り',
    tooManyOrders: 'この銘柄の注文が上限に達しました（最大 {max} 件）',
    stats: '取引統計',
    winRate: '勝率',
    totalPnl: '累計損益',
    profitFactor: '損益比',
    avgWin: '平均利益',
    avgLoss: '平均損失',
    settings: '取引設定',
    feeRate: '吃单手数料 %',
    slippage: '成行スリッページ %',
    target: '収益目標',
    targetHit: '目標達成',
    detail: '詳細',
    notional: '想定元本',
    feeRateDetail: '手数料率',
    feeDetail: '手数料',
    grossPnl: '価格差損益',
    netPnl: '純損益',
    maxDrawdown: '最大ドローダウン',
    drawdown: 'ドローダウン',
    pnlBars: '取引ごとの損益',
    filterSymbol: '銘柄',
    filterSide: '方向',
    filterQuery: '価格を検索…',
    filterEmpty: '一致する取引なし',
    all: 'すべて',
    dailyCount: '件数',
    dailyPnl: '当日損益',
    weekPnl: '今週損益',
    monthPnl: '今月損益',
    period: '期間損益',
    locate: 'チャートへ移動',
    bySymbol: '銘柄別集計',
  },
  paper: { title: '取引履歴', empty: '取引なし——ポジションを建てるとここに記録されます', open: '新規', close: '決済', long: 'ロング', short: 'ショート', clear: '消去', export: 'CSV 出力', reset: '口座リセット', resetConfirm: '口座をリセットしますか？残高は 10,000 に戻り、取引履歴はすべて消去されます。', exportEquity: '資産曲線を出力', equity: '資産曲線', exportJson: 'JSON 出力', importJson: 'JSON 取り込み', importDone: '取り込み完了', importFail: '取り込み失敗：形式が無効', snapshot: '口座スナップショット', snapshotName: '名前', saveSnapshot: '保存', loadSnapshot: '読込', deleteSnapshot: '削除' },
  ctx: { copyPrice: '価格をコピー', copyOhlc: 'OHLC をコピー', addAlert: 'アラートを追加', limitBuy: '指値買い', limitSell: '指値売り', clearDrawings: '描画を消去', confirmClear: 'この銘柄の描画をすべて消去しますか？', copied: 'コピーしました' },
  pull: { pull: '引き下げて更新', release: '指を離して更新', refreshing: '更新中…' },
  panelState: { loading: '読み込み中…', error: '読み込み失敗', empty: 'データなし', retry: '再試行' },
  quickOrder: {
    balance: '利用可能残高',
    insufficient: '残高不足：名義金額＋手数料が証拠金として必要です',
    bid: '買値',
    ask: '売値',
    title: "クイック注文",
    price: "価格",
    qty: "数量",
    notional: "想定金額",
    fill: "想定約定価格",
    fee: "手数料 (0.1%)",
    total: "合計",
    confirm: "注文を確定",
    hint: "現在の模擬ポジションを上書きします · Enter 確定 / Esc 閉じる",
    qtyPreset: "よく使う数量",
    customPct: "カスタム%",
    apply: "適用",
    attachHint: '指値に付ける利確/損切り（任意）：約定と同時に保有序位に反映',
    attachErr: '利確/損切りは指値価格の正しい側にある必要があります（買いは利確が高く損切りが低く、売りは逆）',
    riskReward: '隐含リスクリワード {ratio} : 1',
  },
  volumeProfile: {
    title: '出来高プロファイル（直近 300 本）· {symbol}',
    poc: 'POC ≈ {price}',
    bidVol: '買い出来高',
    askVol: '売り出来高',
    pocLabel: 'POC',
  },
  offline: {
    text: 'ネットワークが切断され、行情の更新を停止しました。復旧後は自動再接続します',
  },
  errorBoundary: {
    title: 'チャート描画エラー',
  },
  shortcuts: {
    title: 'キーボードショートカット',
    openSearch: '銘柄検索を開く：{key}',
    period: '期間切替：[ / ]',
    periodPrev: '前の期間',
    periodNext: '次の期間',
    layoutKeys: '1 / 2 / 3',
    help: 'ショートカット表示切替',
    copyDrawing: '描画をコピー',
    pasteDrawing: '描画を貼り付け',
    configTitle: 'ショートカット設定',
    recording: '新しいキーを押してください…',
    clickToChange: 'クリック後キーを押す',
    cancel: '録音キャンセル',
    configHint: '項目をクリックしてからキーを押す',
    cycleLang: '言語を切替',
    toggleAlerts: 'アラートを開く',
    print: 'カードを印刷',
    reset: '既定に戻す',
    conflict: 'ショートカット競合：{names} が同じキーを共有しています',
    layout: 'レイアウト切替：1 / 2 / 3（単一 / ペア / 4分割）',
    cycleMain: 'メイン指標を循環：M',
    cycleSub: 'サブ指標を循環：N',
    fullscreen: '全画面切替：F',
    replay: 'リプレイ再生 / 一時停止：Space',
    replayStep: 'リプレイを 1 本ずつ進む：← / →',
    replaySpeed: 'リプレイ速度：↑ / ↓',
    deleteDrawing: '選択中の描画を削除：Delete',
    escape: 'キャンセル / 閉じる：Esc',
    hint: 'ショートカット表示 / 非表示：?',
    groupNav: 'ナビゲーション',
    groupIndicators: 'インジケーター',
    groupReplay: 'リプレイ',
    groupDrawings: '描画',
    filter: 'ショートカットを絞り込み…',
    noMatch: '一致するショートカットなし',
  },
  app: {
    titleZh: 'Kline Buty · 实时 K 线',
    titleEn: 'Kline Buty · Live Candles',
    titleJa: 'Kline Buty · リアルタイム K 線',
    titleKo: 'Kline Buty · 실시간 K 라인',
    titleEs: 'Kline Buty · Velas en vivo',
  },
}


export const ko: Messages = {
  storage: {
    warn: '로컬 저장소 사용량이 {size}에 도달했습니다. 오래된 데이터를 정리하세요',
  },
  update: {
    banner: '새 버전이 있습니다 — 새로고침하여 최신 기능을 불러오세요',
    reload: '새로고침',
  },
  common: {
    none: '없음',
    price: '가격',
    delete: '삭제',
    cancel: '취소',
    confirm: '확인',
    retry: '재시도',
    more: '더보기',
    knowledge: '지식 베이스',
    knowledgeTitle: '거래 지식 베이스 열기 (새 탭)',
    close: '닫기',
    backToLatest: '최신으로',
    feedback: '피드백',
    feedbackTitle: '문제 보고 / 제안 (GitHub Issues 열기)',
  },
  scale: { linear: '선형', log: '로그', title: '가격 축: 선형 / 로그' },
  status: {
    loading: '과거 데이터 불러오는 중…',
    connecting: '실시간 시세 연결 중…',
    live: '실시간',
    reconnecting: '재연결 중…',
    closed: '연결 끊김',
    error: '불러오기 실패',
    chartError: '시세 데이터를 불러오지 못했습니다: 네트워크 또는 데이터 소스를 확인하세요',
    noData: '데이터 없음',
    loadDemo: '데모 데이터 로드',
    frameDrop: '실시간 프레임 손실 {rate}',
    frameDropUnit: '손실',
    refilling: '공백 구간 보충 중 {done}/{total}',
    depthLoading: '호가창 불러오는 중…',
    vpNotEnough: '데이터 부족',
    replayNotEnough: '데이터 부족 (30개 이상 필요)',
  },
  disclaimer: {
    text: '데이터는 참고용이며 투자 조언이 아닙니다.',
    watermark: '참고용·투자 조언 아님',
  },
  settings: {
    watermark: '차트 워터마크',
    watermarkTitle: '차트 면책 워터마크 표시/숨기기',
    snapshot: '설정 스냅샷',
    snapshotExport: '설정 내보내기',
    snapshotExportTitle: '모든 설정(테마/즐겨찾기/그림/계정)을 JSON으로 내보내기',
    snapshotImport: '설정 가져오기',
    snapshotImportTitle: 'JSON 스냅샷에서 복원(새로고침)',
  },

  chartType: {
    candlestick: '캔들',
    line: '라인',
    area: '영역',
  },
  chart: {
    subScaleFixed: 'Y축 고정',
    subScaleToggleTitle: '보조 패널 Y축을 이론적 극값으로 고정(과매수/과매도 안정 비교), 다시 클릭 시 자동 복귀',
    sessionLines: '당일 고저',
  },
  group: {
    type: '유형',
    main: '메인',
    sub: '보조',
  },
  period: {
    '1s': '1초',
    '1m': '1분',
    '3m': '3분',
    '5m': '5분',
    '15m': '15분',
    '30m': '30분',
    '1h': '1시간',
    '2h': '2시간',
    '4h': '4시간',
    '12h': '12시간',
    '1d': '일',
    '3d': '3일',
    '1w': '주',
    '1M': '월',
  },
  lang: {
    switchTo: '언어 전환 (5개 언어)',
  },
  tz: { title: '시간축 시간대', utc: 'UTC', local: '현지' },
  theme: {
    switchTitle: '테마 전환',
    toLight: '라이트',
    toDark: '다크',
    toAuto: '자동(시스템 따르기)',
    toSchedule: '예약',
    scheduleDark: '다크 시간',
    scheduleLight: '라이트 시간',
    pickTitle: '테마 색상 프리셋 (강조/상승·하락 색상)',
    presetClassic: '클래식 블루',
    presetAShare: '빨강 상승·초록 하락',
    presetPurple: '퍼플',
    presetTeal: '틸',
    highContrastTitle: '고대비 모드',
    highContrastOn: 'HC',
    highContrastOff: '기본',
  },
  layout: {
    switchTitle: '레이아웃 전환 (단일/2개/4개)',
    presets: '패널 레이아웃',
    presetName: '이름',
    savePreset: '저장',
    applyPreset: '적용',

    compare: '비교',
    compareTitle: '비교 종목 중첩',
    compareOff: '끄기',
    fontScale: '글자 크기',
    fontScaleTitle: '차트 글자 크기',
    single: '단일',
    pair: '2개',
    quad: '4개',
  },
  fullscreen: {
    enter: '전체화면',
    exit: '전체화면 종료',
  },
  panel: {
    position: '포지션',
    positionTitle: '모의 포지션 (진입/TP/SL 라인)',
    alerts: '알림',
    alertsTitle: '가격 알림',
    depth: '호가',
    depthTitle: '호가창 깊이',

    orderBook: '주문장',
    orderBookTitle: '호가 주문장 (매수/매도 호가)',
    tape: '체결',
    tapeTitle: '최근 체결 목록(가격/수량/시간)',
    vp: 'VPVR',
    vpTitle: '거래량 프로파일 (VPVR)',
    settings: '설정',
    sentiment: '심리',
    sentimentTitle: '파생 심리 (롱/숏, 테이커, 미청산)',
    statsTitle: '시장 개요 (최근가/등락/자금비용/미청산)',
    resizeTitle: '드래그로 사이드 패널 너비 조정',
    orderTitle: '사이드 패널 순서',
    moveUp: '위로',
    moveDown: '아래로',

  },
  perf: {
    candleCap: '캔들 상한',
    candleCapTitle: '이 수를 초과하면 다운샘플링 (0=끄기)',
    off: '자동 끄기',
    panelTitle: '성능 진단',
    noFrame: '실시간 프레임 없음 (WS 없음)',
    fps: '프레임 속도',
    dropRate: '드롭률',
    avgInterval: '평균 간격',
    window: '창 프레임 수',
    warn: '⚠ 드롭률 10% 초과 — 차트 버벅임 가능 (약한 네트워크/데이터 과다)',
  },
  changelog: {
    title: '버전 기록',
    v040r1: 'I 단계 마무리: 예약 테마 전환(설정 시간에 다크/라이트 자동 전환), 드로잉 딥링크, 템플릿 마켓, 스냅샷 갤러리; A–I 단계 완료(I3 클라우드 동기화 / I11 모바일 위젯은 외부 역량 대기)',
    v04r1: '데이터 정합성: 주기 경계 정렬 / 오른쪽 앵커 / 갭 메우기 / 멀티차트 크로스헤어 동기화 / 지연 표시',
    v04r2: '지표 엔진: 크로스헤어 값 / 시그널 마커 / 임계값 배경 / W%R·TRIX·DPO·Vortex / 파라미터 가져오기·내보내기 / 즐겨찾기',
    v04r3: '그리기: 스크린샷 내보내기 / 그룹 잠금 / 일괄 작업 / 종목 간 템플릿 / 스냅 4단계 / 실행취소 깊이 / 좌표 배지',
    v04r4: '모의거래: 양방향 포지션 / 동적 증거금률 / 수수료·슬리피지 설정 / 손익 통계 / 청산 경고 / 수수료 분리 / 스냅샷 / 목표 수익 / JSON 마이그레이션',
    v04r5: '알림: 채널 / 다중종목 모니터링 / 그룹 토글 / 템플릿 / 만료 / 일괄 작업 / 트리거 횟수 / 정밀도 / 배지 / 단축키 / 메모',
    v04r6: '접근성: 포커스 트랩 / 고대비 / 정보줄 설정 / 사이드바 너비 / 패널 순서 / 레이아웃 프리셋',
    v04r7: '성능: 문서사이트 컴포넌트 테스트 / 적응형 다운샘플링 / 랙 진단 / 누수 감사',
    v04r8: '설정 스냅샷 내보내기·가져오기 / 단축키 인쇄 카드',
    v03r1: 'P3/P4 심화: 그리기 도구 49종, 지표 26개, 시장 리플레이, 모의거래, 가격 알림, 모바일 제스처, PWA',
    v03r2: '키보드 접근성: 전역 탭 흐름, roving tabindex, focus-visible, aria-pressed',
    v02r1: '지식창고 27장·201편, 문서사이트 출시, 5개국어 i18n',
    v01r1: '실시간 K-라인 터미널: 14주기, 호가창, 멀티차트, CSV 내보내기, 테마',
  },
  snap: {
    title: '스냅샷 갤러리',
    save: '스냅샷 저장',
    saved: '갤러리에 저장됨',
    preview: '미리보기',
    download: '이미지 다운로드',
    empty: '스냅샷 없음 — 차트 오른쪽 위「스냅샷 저장」으로 비교 이미지를 저장하세요',
    clearAll: '모두 지우기',
  },
  pinned: {
    title: '고정 실시간 시세',
    titleShort: '고정',
    empty: '고정 종목 없음 — 관심목록의 ★로 고정',
    unpin: '고정 해제',
    add: '고정',
    sortBy: '정렬',
    sortPrice: '가격',
    sortChange: '변동',
  },
  sentiment: {
    globalRatio: '전체 계정 롱/숏 비율',
    topTraderRatio: '대형 트레이더 비율',
    takerRatio: '테이커 매수/매도 비율',
    openInterest: 'OI 24h',
    long: '롱',
    buy: '매수',
    loading: '불러오는 중…',
  },
  share: {
    title: '현재 종목/주기/선택 도형의 공유 링크 복사',
    copy: '공유',
    copied: '복사됨',
    exportTitle: '현재 종목/주기의 K라인 CSV 내보내기 (활성 지표 열 포함)',
    export: '내보내기',
    exported: '내보냄',
  },
  csv: {
    exportRange: '내보내기 범위',
    all: '전체',
  },
  docs: {
    title: '문서 색인',
    knowledge: '거래 지식 베이스',
    knowledgeDesc: '27장 201편: 현물/선물/주식/암호화폐/외환/옵션/매크로/퀀트',
    readme: '프로젝트 README',
    readmeDesc: '기능, 기술 스택, 빠른 시작',
    repo: 'GitHub 저장소',
    repoDesc: '소스, Issue, Pull Request',
    changelog: 'CHANGELOG',
    changelogDesc: '전체 변경 기록',
  },
  replay: {
    start: '다시보기',
    title: '과거 캔들 단계별 재생',
    play: '재생',
    pause: '일시정지',
    exit: '다시보기 종료',
  },
  drawing: {
    snap: '캔들 스냅',
    snapOn: '켜짐',
    snapOff: '꺼짐',
    snapTime: '시간 정렬',
    snapGrid: '그리드 정렬',
    snapOhlc: 'OHLC 정렬',
    noteToggle: '메모 표시 전환',
    coordBadge: '좌표 라벨 상시 표시',
    group: '그리기',
    mouse: '마우스',
    horizontal: '수평선',
    vertical: '수직선',
    cross: '십자선',
    trend: '추세선',
    extended: '연장선',
    angle: '추세 각도',
    channel: '채널',
    fib: '피보나치',
    text: '텍스트',
    note: '메모',
    defaultText: '텍스트',
    rect: '사각형',
    ellipse: '타원',
    circle: '원',
    triangle: '삼각형',
    wedge: '쐐기',
    arc: '호',
    bezier: '베지어 곡선',
    ray: '레이',
    hray: '수평 레이',
    vray: '수직 레이',
    parray: '평행 레이',
    pchannel: '폭 채널',
    fibext: '피보나치 확장',
    fibchannel: '피보나치 채널',
    fibfan: '피보나치 팬',
    fibTime: '피보나치 시간',
    cycle: '사이클 라인',
    timerange: '시간 범위',
    pband: '가격 밴드',
    pricerange: '가격 범위',
    fibtz: '피보나치 시간 구간',
    gann: '간 각도선',
    gannbox: '갠 박스',
    pricelabel: '가격 라벨',
    arrow: '화살표',
    polyline: '다중선',
    measure: '측정',
    measureBars: '{bars}개',
    rr: '손익비',
    positionTool: '포지션 도구',
    forecast: '포캐스트',
    daterange: '날짜 범위',
    rrRatio: '손익비 1:{ratio}',
    speedlines: '스피드 라인',
    regchan: '회귀 채널',
    hchannel: '수평 채널',
    pitchfork: '앤드루스 피치포크',
    xabcd: 'XABCD 패턴',
    elliott: '엘리엇 파동',
    editText: '텍스트 편집',
    defaultColor: '기본 색상',
    searchPlaceholder: '드로잉 도구 검색…',
    searchNoMatch: '일치하는 도구 없음',
    textPlaceholder: '텍스트 내용',
    fontSize: '글자 크기',
    color: '색상',
    textBg: '배경색',
    align: '정렬',
    alignLeft: '왼쪽',
    alignCenter: '가운데',
    alignRight: '오른쪽',
    textDefault: '기본',
    favoriteToggle: '즐겨찾기 전환',
    screenshot: '캡처',
    share: '공유',
    shareTitle: '차트 공유(시스템 공유, 미지원 시 다운로드)',
    screenshotTitle: '스크린샷 공유',
    screenshotRegion: '영역 선택',
    screenshotScale: '스크린샷 해상도',
    regionHint: '차트 영역을 드래그로 선택하고 놓으면 PNG 내보내기',
  },
  layers: {
    title: '레이어 관리',
    undo: '실행 취소',
    redo: '다시 실행',
    undoDepth: '실행취소 단계',
    undoDepthTitle: '도면 실행취소 깊이 (1=직전만, 최대 200)',
    globalOpacity: '전체 투명도',
    delete: '삭제',
    batchCount: '{n}개 선택됨',
    select: '선택',
    copy: '도면 복사',
    paste: '도면 붙여넣기',
    opacity: '투명도',
    followLatest: '최신가 추적',
    ungrouped: '그룹 없음',
    back: '도구로 돌아가기',
    show: '표시',
    hide: '숨기기',
    lock: '잠금',
    unlock: '잠금 해제',
    clearAll: '전체 삭제',
    showAll: '모두 표시',
    hideAll: '모두 숨기기',
    export: '선 내보내기',
    import: '선 가져오기',
    importSymbolMismatch: '가져오기 실패：파일이 다른 심볼의 것입니다',
    importBad: '가져오기 실패：파일 형식을 인식할 수 없습니다',
    empty: '도면 없음',
    search: '이름/유형으로 도면 검색…',
    rename: '이름 바꾸기',
    templateTitle: '도면 템플릿',
    templateSave: '템플릿으로 저장',
    templateNamePlaceholder: '템플릿 이름 (예: 트렌드+피보)',
    templateApply: '적용',
    templateEmpty: '템플릿 없음——현재 도면 조합을 저장하면 모든 종목에 재사용할 수 있습니다',
    templateSaved: '템플릿 저장됨',
    templateExport: '템플릿 내보내기',
    templateImport: '템플릿 가져오기',
    templateImportDone: '가져옴',
    templateImportFail: '가져오기 실패: 잘못된 파일 형식',
    statsLine: '선 {count} · 길이 {len}',
    statsArea: '면 {count} · 면적 {area}',
  },
  symbol: {
    searchPlaceholder: '종목 검색…',
    popular: '인기',
    favorites: '즐겨찾기',
    favoriteAdd: '즐겨찾기에 추가',
    favoriteRemove: '즐겨찾기에서 제거',
    searchResults: '검색 결과 {count}',
    noMatch: '일치하는 종목 없음',
  },
  indicator: {
    settings: '지표 파라미터',
    resetDefault: '기본값으로 재설정',
    recommend: '스마트 추천',
    recommendTitle: '추세/변동성에 따라 주·보조 지표 추천',
    recTrending: '추세장',
    recRanging: '횡보장',
    recVolatile: '고변동',
    drawingSuggest: '그림에서 추천',
    drawingSuggestTitle: '현재 종목의 그림에서 지표 추천',
    recSemanticsRange: '레인지/채널',
    recSemanticsTrend: '추세선',
    recSemanticsLevels: '지지/저항',
    recSemanticsTime: '시간 구조',
    noParams: '이 지표에는 조정 가능한 파라미터가 없습니다',
    maPeriods: '{name} 주기 (쉼표 구분)',
    maOverlayEma: 'MA 위에 EMA 겹치기',
    subOverlay: '보조지표 중첩',
    bollPeriod: 'BOLL 주기',
    bollMult: 'BOLL 표준편차 배수',
    macdFast: 'MACD 빠른선',
    macdSlow: 'MACD 느린선',
    macdSignal: 'MACD 시그널',
    kdjN: 'KDJ N',
    kdjM1: 'KDJ M1',
    kdjM2: 'KDJ M2',
    rsiPeriod: 'RSI 주기',
    wrPeriod: 'WR 주기',
    obvMaPeriod: 'OBV 평활 (1=원본)',
    volMaPeriod: 'VOL 이동평균 주기(1=끔)',
    atrPeriod: 'ATR 주기',
    dmiPeriod: 'DMI 주기',
    cciPeriod: 'CCI 주기',
    psyPeriod: 'PSY 주기',
    stochK: 'STOCH %K 주기',
    stochSmooth: 'STOCH 평활',
    stochD: 'STOCH %D 주기',
    rocPeriod: 'ROC 주기',
    momPeriod: 'MOM 주기',
    bbwPeriod: 'BBW 기간',
    bbwMult: 'BBW 표준편차 배수',
    stPeriod: 'ST 기간',
    stMult: 'ST 배수',
    sarAfStart: 'SAR 초기 AF',
    sarAfStep: 'SAR AF 단계',
    sarAfMax: 'SAR AF 최대',
    ichimokuTenkan: '전환선 주기',
    ichimokuKijun: '기준선 주기',
    ichimokuSpanB: '선행스팬B 주기',
    ichimokuDisplacement: '스팬 이동 주기',
    sar: 'SAR',
    ichimoku: '일목균형표',
    mfiPeriod: 'MFI 주기',
    trixPeriod: 'TRIX 주기',
    dpoPeriod: 'DPO 주기',
    vortexPeriod: 'Vortex 주기',
    aoFast: 'AO 빠른선',
    aoSlow: 'AO 느린선',
    cmfPeriod: 'CMF 주기',
    donchianPeriod: '돈치안 채널 주기',
    aroonPeriod: 'Aroon 주기',
    aroon: 'Aroon',
    reset: '기본값으로 초기화',
    preset: '파라미터 프리셋',
    presetPlaceholder: '프리셋 불러오기…',
    presetSave: '저장',
    presetSaved: '저장됨',
    presetName: '프리셋 이름',
    exportParams: '매개변수 내보내기',
    importParams: '매개변수 가져오기',
    importParamsOk: '매개변수를 가져왔습니다',
    lineColors: '선 색상',
    lineColorFor: '{name} 선 색상',
    lineColorReset: '기본 색상으로 초기화',
  },
  stats: {
    lastPrice: '최신가',
    change24h: '24h 변동',
    high24h: '24h 고가',
    low24h: '24h 저가',
    volume24h: '24h 거래량',
    fundingRate: '펀딩 비율',
    fundingNext: '다음 펀딩 결제까지',
    openInterest: '미청산 계약',
    markPrice: '마크 가격',
    countdown: '마감',
    marketPerp: '선물',
    marketSpot: '현물',
    volumeSurge: '거래량',
    volumeSurgeTitle: '최신 거래량 ÷ 최근 20봉 평균 ≥ 3배면 거래량 급증으로 판정',
    gapHealthy: '데이터 정상',
    gapPartial: '약간의 공백',
    gapDegraded: '데이터 공백 {n}곳',
    dataLatency: '지연',
    dataLatencyTitle: '실시간 프레임이 5초 이상 지연됨 — 데이터 중단 또는 약한 네트워크 가능성',
        marketType: '시장 유형',
    health: '데이터 건전성',
    summary: '시세 개요',
    barConfig: '정보 표시 항목',
  },
  marketList: {
    title: '시세',
    addFavorite: '즐겨찾기 추가',
    removeFavorite: '즐겨찾기 제거',
    pair: '거래쌍',
    lastPrice: '최신가',
    change24h: '24h 변동',
    volume: '거래액',
    refresh: '새로고침',
    loading: '불러오는 중…',
    empty: '시세 데이터 없음',
    collapse: '접기',
    expand: '펼치기',
    sortTitle: '클릭하여 정렬',
    stale: '새로고침 실패, 캐시 표시',
    searchPlaceholder: '심볼 필터링…',
    tabAll: '전체',
    tabFavorites: '즐겨찾기',
    tabRank: '랭킹',
    rankChange: '상승률',
    rankVolume: '거래량',
    rotate: '자동 순환',
    rotateTitle: '5초마다 표시 목록에서 다음 심볼로 순환',
    favoritesEmpty: '즐겨찾기 없음——심볼 목록에서 ★로 추가하세요',
    noMatch: '일치하는 심볼 없음',
  },

  position: {
    title: '모의 포지션',
    close: '청산',
    long: '롱',
    short: '숏',
    hedgeMode: '헤지 모드',
    balance: '가용 잔고',
    unrealized: '평가 손익',
    qtyShort: '수량',
    noPosition: '포지션 없음',
    closeAll: '전체 청산',
    otherSymbols: '다른 종목 포지션',
    levels: '익절/손절',
    levelBreakeven: '손절을 진입가로',
    levelClearHint: '비우면 그 선이 해제됩니다',
    reduce: '부분 청산',
    reduceQty: '부분 청산 수량',
    reduceErr: '수량은 0보다 크고 보유 수량 이하여야 합니다',
    levelTrail: '트레일링 손절',
    levelTrailHint: '손절이 최신 가격을 따라 t% 거리를 유지하며, 반대로는 완화되지 않습니다',
    levelErrInvalid: '익절과 손절은 진입가 대비 올바른 쪽에 있어야 합니다',
    levelErrCrossed: '익절가와 손절가는 같을 수 없습니다',
    switch: '전환',
    entry: '진입가',
    market: '현재가',
    quantity: '수량',
    tpPct: 'TP%',
    slPct: 'SL%',
    tpLine: 'TP 라인',
    tpPrice: 'TP 가격',
    slPrice: 'SL 가격',
    levelModePct: '퍼센트',
    levelModePrice: '가격',
    slLine: 'SL 라인',
    floatingPnl: '평가손익',
    open: '진입',
    lineEntry: '진입',
    lineTp: '익절 (TP)',
    lineSl: '손절 (SL)',
    margin: '증거금',
    leverage: '레버리지',
    liqPrice: '강제청산가',
    marginRate: '증거금률',
    liqWarn: '강제청산 경고',
    liqCritical: '강제청산 위험',
    reverse: '반대 매매',
    todayPnl: '오늘 실현손익',
  },
  alert: {
    history: '발동 기록',
    sound: '알림 소리',
    clearHistory: '기록 지우기',
    title: '가격 알림 · {symbol}',
    granted: '알림 켜짐',
    unsupported: '이 환경에서는 알림을 지원하지 않습니다',
    enable: '알림 활성화',
    above: '가격 ≥',
    below: '가격 ≤',
    add: '알림 추가',
    invalid: '유효한 가격 입력(양수)',
    none: '알림 없음',
    triggered: '트리거됨',
    reset: '초기화',
    notifyTitle: 'Kline Buty · 가격 알림',
    notifyAbove: '{symbol} 이(가) {price} 에 도달',
    notifyBelow: '{symbol} 이(가) {price} 아래로 하락',
    repeat: '반복 알림',
    soundPreview: '소리 미리듣기',
    repeatInterval: '반복 간격',
    minutes: '분',
    group: '그룹',
    groupPlaceholder: '예: 추세',
    sort: '정렬',
    filter: '필터',
    filterAll: '전체',
    filterActive: '대기',
    sortTime: '시간',
    sortPrice: '가격',
    sortSymbol: '종목',
    soundKind: '음향',
    soundBeep: '삐',
    soundChime: '차임',
    soundPing: '핑',
    soundLow: '저음',
    timeWindow: '시간 창',
    timeFrom: '시작',
    timeTo: '종료',
    channel: '알림 채널',
    channelBoth: '둘 다',
    channelSystem: '시스템 알림',
    channelWeb: '앱 내 배너',
    note: '메모',
    notePlaceholder: '선택 설명…',
    expiresAt: '만료 시간',
    precision: '가격 정밀도',
    precisionAuto: '자동',
    template: '템플릿',
    templateName: '이름',
    saveTemplate: '저장',
    applyTemplate: '적용',
    batch: '일괄',
    selectAll: '모두 선택',
    clearSelection: '지우기',
    enableSelected: '선택 활성화',
    disableSelected: '선택 비활성화',
    deleteSelected: '선택 삭제',
    select: '선택',
    expired: '만료됨',
    clearExpired: '만료 항목 삭제',
    triggerCount: '발동 횟수',
    groupToggle: '그룹 전환',
    voice: '음성 알림 (가격 변동 읽어주기)',
    adaptive: '변동성 적응(ATR 임계값)',
    quickPct: '빠른 ±%',
    pending: '대기 중',
  },
  tooltip: {
    open: '시가',
    high: '고가',
    low: '저가',
    close: '종가',
    volume: '거래량',
  },
  depth: {
    title: '호가창 · {symbol} (실시간)',
    bid: '매수',
    ask: '매도',
  },
  orderBook: {
    title: '호가 주문장 · {symbol}',
    qty: '수량',
    cum: '누적',
    group: '그룹',
    spread: '스프레드',
    markHint: '클릭하여 메인 차트에 표시(다시 클릭 시 해제); 왼쪽 버튼으로 빠른 주문',
    refresh: '호가창 새로고침(재접속하여 최신 스냅샷 조회)',
    bid: '매수',
    ask: '매도',
    imbalance: '호가 매매 불균형: 매수/매도 비중',
  },
  tape: {
    title: '최근 체결 · {symbol}',
    time: '시간',
    qty: '수량',
    sideSummary: '표시 구간의 매수/매도 체결 건수',
    markHint: '클릭하여 메인 차트에 표시(다시 클릭 시 해제)',
    filterAll: '전체',
    filterBuy: '능동 매수',
    filterSell: '능동 매도',
    filterSideHint: '체결 방향 필터(능동 매수/매도)',
    bigOrder: '대량',
    bigOrderHint: '창 평균 수량의 일정 배수 이상 체결만 표시',
    emptyFilter: '조건에 맞는 체결이 없습니다',
  },
  trade: {
    buy: "매수",
    sell: "매도",
    market: '시장가',
    limit: '지정가',
    limitHint: '지정가 주문은 도달 시점의 현재가로 체결됩니다(지정가보다 유리하면 가격 개선). 슬리피지 없음',
    makerFee: '메이커 수수료 %',
    pendingTitle: '미체결 주문',
    pendingEmpty: '지정가 주문이 없습니다',
    cancelOrder: '주문 취소',
    editOrder: '주문 수정',
    editErr: '가격과 수량은 양수여야 합니다',
    filledToast: '지정가 주문 체결 · {symbol} {qty} @ {price}',
    cancelledToast: '잔고 부족으로 주문이 취소되었습니다 · {symbol}',
    tpslToast: '{symbol} {reason} 도달 — {price}에 청산했습니다',
    tpslToastMulti: '{symbol} {reason} 도달 — {count}건을 {price}에 청산했습니다',
    tpWord: '익절',
    slWord: '손절',
    tooManyOrders: '이 심볼의 주문이 상한에 도달했습니다(최대 {max}건)',
    stats: '거래 통계',
    winRate: '승률',
    totalPnl: '누적 손익',
    profitFactor: '손익비',
    avgWin: '평균 수익',
    avgLoss: '평균 손실',
    settings: '거래 설정',
    feeRate: '테이커 수수료 %',
    slippage: '시장 슬리피지 %',
    target: '수익 목표',
    targetHit: '목표 달성',
    detail: '상세',
    notional: '명목 금액',
    feeRateDetail: '수수료율',
    feeDetail: '수수료',
    grossPnl: '가격차 손익',
    netPnl: '순 손익',
    maxDrawdown: '최대 하락',
    drawdown: '하락폭',
    pnlBars: '거래별 손익',
    filterSymbol: '종목',
    filterSide: '방향',
    filterQuery: '가격 검색…',
    filterEmpty: '일치하는 거래 없음',
    all: '전체',
    dailyCount: '건수',
    dailyPnl: '당일 손익',
    weekPnl: '이번 주 손익',
    monthPnl: '이번 달 손익',
    period: '기간 손익',
    locate: '차트로 이동',
    bySymbol: '종목별 집계',
  },
  paper: { title: '거래 내역', empty: '거래 없음——포지션을 열면 여기에 기록됩니다', open: '진입', close: '청산', long: '롱', short: '숏', clear: '지우기', export: 'CSV 내보내기', reset: '계정 초기화', resetConfirm: '모의 계정을 초기화할까요? 잔액이 10,000으로 돌아가고 거래 내역이 모두 지워집니다.', exportEquity: '자산 곡선 내보내기', equity: '자산 곡선', exportJson: 'JSON 내보내기', importJson: 'JSON 가져오기', importDone: '가져오기 완료', importFail: '가져오기 실패: 잘못된 형식', snapshot: '계정 스냅샷', snapshotName: '이름', saveSnapshot: '저장', loadSnapshot: '불러오기', deleteSnapshot: '삭제' },
  ctx: { copyPrice: '가격 복사', copyOhlc: 'OHLC 복사', addAlert: '알림 추가', limitBuy: '지정가 매수', limitSell: '지정가 매도', clearDrawings: '선 모두 지우기', confirmClear: '이 심볼의 모든 선을 지울까요?', copied: '복사됨' },
  pull: { pull: '당겨서 새로고침', release: '놓아서 새로고침', refreshing: '새로고침 중…' },
  panelState: { loading: '로딩 중…', error: '로드 실패', empty: '데이터 없음', retry: '재시도' },
  quickOrder: {
    balance: '가능 잔고',
    insufficient: '잔고 부족：명목금액＋수수료가 증거금으로 필요합니다',
    bid: '매수호가',
    ask: '매도호가',
    title: "빠른 주문",
    price: "가격",
    qty: "수량",
    notional: "예상 금액",
    fill: "예상 체결가",
    fee: "수수료 (0.1%)",
    total: "합계",
    confirm: "주문 확정",
    hint: "현재 모의 포지션을 덮어씁니다 · Enter 확인 / Esc 닫기",
    qtyPreset: "자주 쓰는 수량",
    customPct: "사용자 정의%",
    apply: "적용",
    attachHint: '지정가에 동반하는 익절/손절 (선택): 체결 즉시 포지션에 반영',
    attachErr: '익절/손절은 주문 가격의 올바른 쪽에 있어야 합니다 (매수는 익절 위·손절 아래, 매도는 반대)',
    riskReward: '위험 대비 수익 {ratio} : 1',
  },
  volumeProfile: {
    title: '거래량 프로파일 (최근 300개) · {symbol}',
    poc: 'POC ≈ {price}',
    bidVol: '매수량',
    askVol: '매도량',
    pocLabel: 'POC',
  },
  offline: {
    text: '네트워크가 끊겨 시세 업데이트가 중지되었습니다. 복구되면 자동으로 재연결됩니다',
  },
  errorBoundary: {
    title: '차트 렌더링 오류',
  },
  shortcuts: {
    title: '키보드 단축키',
    openSearch: '거래쌍 검색 열기: {key}',
    period: '주기 전환: [ / ]',
    periodPrev: '이전 주기',
    periodNext: '다음 주기',
    layoutKeys: '1 / 2 / 3',
    help: '단축키 표시 전환',
    copyDrawing: '도면 복사',
    pasteDrawing: '도면 붙여넣기',
    configTitle: '단축키 설정',
    recording: '새 키를 누르세요…',
    clickToChange: '클릭 후 키 입력',
    cancel: '녹화 취소',
    configHint: '항목 클릭 후 키 입력',
    cycleLang: '언어 전환',
    toggleAlerts: '알림 열기',
    print: '카드 인쇄',
    reset: '기본값 복원',
    conflict: '단축키 충돌: {names}가 같은 키를 공유합니다',
    layout: '레이아웃 전환: 1 / 2 / 3 (단일 / 쌍 / 4분할)',
    cycleMain: '메인 지표 순환: M',
    cycleSub: '서브 지표 순환: N',
    fullscreen: '전체화면 전환: F',
    replay: '리플레이 재생/일시정지: Space',
    replayStep: '리플레이 한 봉씩 이동: ← / →',
    replaySpeed: '리플레이 속도: ↑ / ↓',
    deleteDrawing: '선택한 드로잉 삭제: Delete',
    escape: '취소 / 닫기: Esc',
    hint: '단축키 표시/숨기기: ?',
    groupNav: '탐색',
    groupIndicators: '지표',
    groupReplay: '리플레이',
    groupDrawings: '드로잉',
    filter: '단축키 필터링…',
    noMatch: '일치하는 단축키 없음',
  },
  app: {
    titleZh: 'Kline Buty · 实时 K 线',
    titleEn: 'Kline Buty · Live Candles',
    titleJa: 'Kline Buty · リアルタイム K 線',
    titleKo: 'Kline Buty · 실시간 K 라인',
    titleEs: 'Kline Buty · Velas en vivo',
  },
}



/** 扁平叶子键（如 "status.loading"），带类型提示 */
type LeafKeyOf<T, Prefix extends string = ''> = {
  [K in keyof T & string]: T[K] extends string
    ? Prefix extends ''
      ? K
      : `${Prefix}.${K}`
    : LeafKeyOf<T[K], Prefix extends '' ? K : `${Prefix}.${K}`>
}[keyof T & string]

export type MessageKey = LeafKeyOf<Messages>

export const es: Messages = {
  storage: {
    warn: 'El almacenamiento local alcanzó {size}, considera limpiar datos antiguos',
  },
  update: {
    banner: 'Nueva versión disponible — actualiza para cargar lo último',
    reload: 'Actualizar',
  },
  common: {
    none: 'Ninguno',
    price: 'Precio',
    delete: 'Eliminar',
    cancel: 'Cancelar',
    confirm: 'Aceptar',
    retry: 'Reintentar',
    more: 'Más',
    knowledge: 'Base de conocimiento',
    knowledgeTitle: 'Abrir la base de conocimiento (nueva pestaña)',
    close: 'Cerrar',
    backToLatest: 'Volver al último',
    feedback: 'Retroalimentación',
    feedbackTitle: 'Reportar problema / sugerencia (abrir GitHub Issues)',
  },
  scale: { linear: 'Lineal', log: 'Log', title: 'Escala de precios: Lineal / Log' },
  status: {
    loading: 'Cargando historial…',
    connecting: 'Conectando a datos en vivo…',
    live: 'En vivo',
    reconnecting: 'Reconectando…',
    closed: 'Desconectado',
    error: 'Error al cargar',
    chartError: 'No se pudieron cargar los datos de mercado: revisa la red o la fuente de datos',
    noData: 'Sin datos',
    loadDemo: 'Cargar datos de demostración',
    frameDrop: 'Pérdida de fotogramas {rate}',
    frameDropUnit: 'perdidos',
    refilling: 'Rellenando huecos {done}/{total}',
    depthLoading: 'Cargando libro de órdenes…',
    vpNotEnough: 'Datos insuficientes',
    replayNotEnough: 'Datos insuficientes (se necesitan ≥30 velas)',
  },
  disclaimer: {
    text: 'Los datos son solo de referencia; no constituyen asesoramiento de inversión.',
    watermark: 'Solo referencia · No es asesoramiento de inversión',
  },
  settings: {
    watermark: 'Marca de agua',
    watermarkTitle: 'Mostrar/ocultar la marca de agua de exención en el gráfico',
    snapshot: 'Instantánea de ajustes',
    snapshotExport: 'Exportar ajustes',
    snapshotExportTitle: 'Exportar todos los ajustes (tema/favoritos/dibujos/cuenta) como JSON',
    snapshotImport: 'Importar ajustes',
    snapshotImportTitle: 'Restaurar desde instantánea JSON (recarga)',
  },

  chartType: {
    candlestick: 'Velas',
    line: 'Línea',
    area: 'Área',
  },
  chart: {
    subScaleFixed: 'Fijar Y',
    subScaleToggleTitle: 'Bloquear el eje Y del subpanel a los extremos teóricos (comparación estable de sobrecompra/sobreventa); clic de nuevo para automático',
    sessionLines: 'Máx/Mín del día',
  },
  group: {
    type: 'Tipo',
    main: 'Principal',
    sub: 'Sub',
  },
  period: {
    '1s': '1s',
    '1m': '1m',
    '3m': '3m',
    '5m': '5m',
    '15m': '15m',
    '30m': '30m',
    '1h': '1h',
    '2h': '2h',
    '4h': '4h',
    '12h': '12h',
    '1d': '1D',
    '3d': '3D',
    '1w': '1W',
    '1M': '1M',
  },
  lang: {
    switchTo: 'Cambiar idioma (5)',
  },
  tz: { title: 'Zona horaria del eje', utc: 'UTC', local: 'Local' },
  theme: {
    switchTitle: 'Cambiar tema',
    toLight: 'Claro',
    toDark: 'Oscuro',
    toAuto: 'Automático (según el sistema)',
    toSchedule: 'Horario',
    scheduleDark: 'Oscuro a las',
    scheduleLight: 'Claro a las',
    pickTitle: 'Presets de color del tema (acento / subida / bajada)',
    presetClassic: 'Azul clásico',
    presetAShare: 'Sube rojo / baja verde',
    presetPurple: 'Púrpura',
    presetTeal: 'Turquesa',
    highContrastTitle: 'Alto contraste',
    highContrastOn: 'AC',
    highContrastOff: 'Estándar',
  },
  layout: {
    switchTitle: 'Cambiar diseño (único/par/cuádruple)',
    presets: 'Presets de diseño',
    presetName: 'Nombre',
    savePreset: 'Guardar',
    applyPreset: 'Aplicar',

    compare: 'Comparar',
    compareTitle: 'Símbolo superpuesto',
    compareOff: 'Apagado',
    fontScale: 'Tamaño',
    fontScaleTitle: 'Escala de fuente',
    single: 'Único',
    pair: 'Par',
    quad: 'Cuádruple',
  },
  fullscreen: {
    enter: 'Pantalla completa',
    exit: 'Salir de pantalla completa',
  },
  panel: {
    position: 'Posición',
    positionTitle: 'Posición simulada (líneas entrada/TP/SL)',
    alerts: 'Alertas',
    alertsTitle: 'Alertas de precio',
    depth: 'Profundidad',
    depthTitle: 'Profundidad del libro de órdenes',
    orderBook: 'Libro',
    orderBookTitle: 'Escalera del libro de órdenes',
    tape: 'Operaciones',
    tapeTitle: 'Últimas operaciones (precio/cantidad/hora)',
    vp: 'VPVR',
    vpTitle: 'Perfil de volumen (VPVR)',
    settings: 'Ajustes',
    sentiment: 'Sentimiento',
    sentimentTitle: 'Sentimiento de derivados (largo/corto, taker, OI)',
    statsTitle: 'Resumen de mercado (último / variación / fondeo / OI)',
    resizeTitle: 'Arrastrar para ajustar el panel',
    orderTitle: 'Orden del panel',
    moveUp: 'Subir',
    moveDown: 'Bajar',

  },
  perf: {
    candleCap: 'Límite de velas',
    candleCapTitle: 'Reducir muestras más allá de esta cantidad (0=apagado)',
    off: 'Auto apagado',
    panelTitle: 'Diagnóstico de rendimiento',
    noFrame: 'Sin frames en vivo (sin WS)',
    fps: 'FPS',
    dropRate: 'Tasa de caída',
    avgInterval: 'Intervalo medio',
    window: 'Ventana',
    warn: '⚠ Tasa de caída > 10% — el gráfico puede tartamudear (red débil / demasiados datos)',
  },
  changelog: {
    title: 'Historial de versiones',
    v040r1: 'Final de la etapa I: cambio de tema programado (oscuro/claro automático a horas fijadas), enlaces profundos de dibujo, mercado de plantillas, galería de instantáneas; etapas A–I completas (I3 sincronización en la nube / I11 widget móvil esperan capacidad externa)',
    v04r1: 'Integridad de datos: alineación de límites de período / anclaje derecho / relleno de huecos / sincronización de crosshair multi-gráfico / indicador de latencia',
    v04r2: 'Motor de indicadores: valores de crosshair / marcadores de señal / coloreado de umbrales / W%R·TRIX·DPO·Vortex / importar-exportar / favoritos',
    v04r3: 'Dibujo: exportar captura / bloqueo de grupo / operaciones por lotes / plantillas entre símbolos / ajuste 4 modos / profundidad de deshacer / insignia de coordenadas',
    v04r4: 'Simulador: posiciones a ambos lados / margen dinámico / comisiones y deslizamiento configurables / estadísticas de P&L / advertencia de liquidación / desglose de comisiones / instantánea / objetivo de ganancia / migración JSON',
    v04r5: 'Alertas: canales / monitoreo multi-símbolo / interruptor de grupo / plantillas / expiración / operaciones por lotes / recuento de disparos / precisión / insignia / atajo / nota',
    v04r6: 'Accesibilidad: trampa de foco / alto contraste / configuración de barra / ancho de panel lateral / orden de paneles / ajustes de diseño',
    v04r7: 'Rendimiento: pruebas de componentes del sitio de docs / remuestreo adaptativo / diagnóstico de bloqueos / auditoría de fugas',
    v04r8: 'Exportar/importar instantánea de ajustes / tarjeta imprimible de atajos',
    v03r1: 'Profundización P3/P4: 49 herramientas de dibujo, 26 indicadores, repetición de mercado, simulador, alertas de precio, gestos móviles, PWA',
    v03r2: 'Accesibilidad de teclado: flujo de tab global, roving tabindex, focus-visible, aria-pressed',
    v02r1: 'Base de conocimiento 27 capítulos / sitio de docs en línea / i18n en 5 idiomas',
    v01r1: 'Terminal K-line en tiempo real: 14 períodos, profundidad, diseños multi-gráfico, exportación CSV, temas',
  },
  snap: {
    title: 'Galería de capturas',
    save: 'Guardar captura',
    saved: 'Guardada en la galería',
    preview: 'Vista previa',
    download: 'Descargar imagen',
    empty: 'Sin capturas — usa “Guardar captura” (arriba a la derecha del gráfico) para guardar una comparativa',
    clearAll: 'Borrar todas',
  },
  pinned: {
    title: 'Precios fijos en vivo',
    titleShort: 'Fijados',
    empty: 'Sin símbolos fijados — toca ★ en la lista para fijar',
    unpin: 'Desfijar',
    add: 'Fijar',
    sortBy: 'Orden',
    sortPrice: 'Precio',
    sortChange: 'Cambio',
  },
  sentiment: {
    globalRatio: 'Largo/corto global',
    topTraderRatio: 'Ratio de traders top',
    takerRatio: 'Compra/venta taker',
    openInterest: 'OI 24h',
    long: 'Largo',
    buy: 'Compra',
    loading: 'Cargando…',
  },
  share: {
    title: 'Copiar enlace de la moneda, el período y el dibujo seleccionado',
    copy: 'Compartir',
    copied: 'Copiado',
    exportTitle: 'Exportar CSV OHLCV de la moneda/período actuales (con columnas de indicadores activos)',
    export: 'Exportar',
    exported: 'Exportado',
  },
  csv: {
    exportRange: 'Rango de exportación',
    all: 'Todo',
  },
  docs: {
    title: 'Índice de documentos',
    knowledge: 'Base de conocimiento',
    knowledgeDesc: '27 capítulos, 201 artículos: spot/futuros/acciones/cripto/forex/opciones/macro/cuantitativo',
    readme: 'README del proyecto',
    readmeDesc: 'Funciones, stack, inicio rápido',
    repo: 'Repositorio GitHub',
    repoDesc: 'Código, Issues, Pull Requests',
    changelog: 'CHANGELOG',
    changelogDesc: 'Historial completo de cambios',
  },
  replay: {
    start: 'Reproducción',
    title: 'Reproducción histórica paso a paso',
    play: 'Reproducir',
    pause: 'Pausa',
    exit: 'Salir de la reproducción',
  },
  drawing: {
    snap: 'Ajustar a velas',
    snapOn: 'Sí',
    snapOff: 'No',
    snapTime: 'Alinear tiempo',
    snapGrid: 'Alinear cuadrícula',
    snapOhlc: 'Alinear OHLC',
    noteToggle: 'Mostrar/ocultar notas',
    coordBadge: 'Mostrar coordenadas',
    group: 'Dibujar',
    mouse: 'Ratón',
    horizontal: 'Horizontal',
    vertical: 'Vertical',
    cross: 'Línea cruzada',
    trend: 'Línea de tendencia',
    extended: 'Línea extendida',
    angle: 'Ángulo de tendencia',
    channel: 'Canal',
    fib: 'Fibonacci',
    text: 'Texto',
    note: 'Nota',
    defaultText: 'Texto',
    rect: 'Rectángulo',
    ellipse: 'Elipse',
    circle: 'Círculo',
    triangle: 'Triángulo',
    wedge: 'Cuña',
    arc: 'Arco',
    bezier: 'Curva Bézier',
    ray: 'Rayo',
    hray: 'Rayo horizontal',
    vray: 'Rayo vertical',
    parray: 'Rayo paralelo',
    pchannel: 'Canal con ancho',
    fibext: 'Ext. Fibonacci',
    fibchannel: 'Canal de Fibonacci',
    fibfan: 'Abanico Fib',
    fibTime: 'Tiempo Fib',
    cycle: 'Líneas de ciclo',
    timerange: 'Rango de tiempo',
    pband: 'Banda de Precio',
    pricerange: 'Rango de Precio',
    fibtz: 'Zonas de tiempo Fib',
    gann: 'Abanico Gann',
    gannbox: 'Caja de Gann',
    pricelabel: 'Etiqueta de precio',
    arrow: 'Flecha',
    polyline: 'Polilínea',
    measure: 'Medición',
    measureBars: '{bars} barras',
    rr: 'Riesgo/Beneficio',
    positionTool: 'Herramienta de Posición',
    forecast: 'Pronóstico',
    daterange: 'Rango de Fecha',
    rrRatio: 'R:R 1:{ratio}',
    speedlines: 'Líneas de Velocidad',
    regchan: 'Canal de Regresión',
    hchannel: 'Canal Horizontal',
    pitchfork: 'Horquilla de Andrews',
    xabcd: 'Patrón XABCD',
    elliott: 'Onda de Elliott',
    editText: 'Editar texto',
    defaultColor: 'Color predeterminado',
    searchPlaceholder: 'Buscar herramientas…',
    searchNoMatch: 'Sin herramientas coincidentes',
    textPlaceholder: 'Contenido del texto',
    fontSize: 'Tamaño de letra',
    color: 'Color',
    textBg: 'Fondo',
    align: 'Alinear',
    alignLeft: 'Izquierda',
    alignCenter: 'Centro',
    alignRight: 'Derecha',
    textDefault: 'Predeterminado',
    favoriteToggle: 'Alternar favorito',
    screenshot: 'Captura',
    share: 'Compartir',
    shareTitle: 'Compartir gráfico (compartir del sistema; descarga si no)',
    screenshotTitle: 'Compartir captura',
    screenshotRegion: 'Recortar',
    screenshotScale: 'Resolución de captura',
    regionHint: 'Arrastra para seleccionar una zona del gráfico y suelta para exportar PNG',
  },
  layers: {
    title: 'Capas de dibujo',
    undo: 'Deshacer',
    redo: 'Rehacer',
    undoDepth: 'Pasos de deshacer',
    undoDepthTitle: 'Profundidad de deshacer de dibujos (1=solo el último, máx. 200)',
    globalOpacity: 'Opacidad global',
    delete: 'Eliminar',
    batchCount: '{n} seleccionados',
    select: 'Seleccionar',
    copy: 'Copiar dibujo',
    paste: 'Pegar dibujo',
    opacity: 'Opacidad',
    followLatest: 'Seguir último precio',
    ungrouped: 'Sin grupo',
    back: 'Volver a herramientas',
    show: 'Mostrar',
    hide: 'Ocultar',
    lock: 'Bloquear',
    unlock: 'Desbloquear',
    clearAll: 'Borrar todo',
    showAll: 'Mostrar todo',
    hideAll: 'Ocultar todo',
    export: 'Exportar dibujos JSON',
    import: 'Importar dibujos JSON',
    importSymbolMismatch: 'Error al importar: el archivo pertenece a otro símbolo',
    importBad: 'Error al importar: formato no reconocido',
    empty: 'Sin dibujos',
    search: 'Buscar dibujos por nombre/tipo…',
    rename: 'Renombrar',
    templateTitle: 'Plantillas de dibujo',
    templateSave: 'Guardar como plantilla',
    templateNamePlaceholder: 'Nombre de plantilla (p. ej. Tendencia+Fibo)',
    templateApply: 'Aplicar',
    templateEmpty: 'Sin plantillas: guarda un conjunto de dibujos para reutilizarlo en cualquier símbolo',
    templateSaved: 'Plantilla guardada',
    templateExport: 'Exportar plantillas',
    templateImport: 'Importar plantillas',
    templateImportDone: 'Importado',
    templateImportFail: 'Error al importar: formato de archivo no válido',
    statsLine: 'Líneas {count} · longitud {len}',
    statsArea: 'Áreas {count} · área {area}',
  },
  symbol: {
    searchPlaceholder: 'Buscar símbolos…',
    popular: 'Populares',
    favorites: 'Favoritos',
    favoriteAdd: 'Añadir a favoritos',
    favoriteRemove: 'Quitar de favoritos',
    searchResults: 'Resultados {count}',
    noMatch: 'Sin símbolos coincidentes',
  },
  indicator: {
    settings: 'Ajustes de indicadores',
    resetDefault: 'Restablecer valores',
    recommend: 'Recomendación',
    recommendTitle: 'Recomendar indicadores por tendencia y volatilidad',
    recTrending: 'Tendencia',
    recRanging: 'Rango',
    recVolatile: 'Alta volatilidad',
    drawingSuggest: 'Desde dibujos',
    drawingSuggestTitle: 'Sugerir indicadores según los dibujos de este símbolo',
    recSemanticsRange: 'Rango / canal',
    recSemanticsTrend: 'Líneas de tendencia',
    recSemanticsLevels: 'Soporte / resistencia',
    recSemanticsTime: 'Estructura temporal',
    noParams: 'Este indicador no tiene parámetros ajustables',
    maPeriods: 'Período de {name} (separado por comas)',
    maOverlayEma: 'Superponer EMA sobre MA',
    subOverlay: 'Indicador superpuesto',
    bollPeriod: 'Período BOLL',
    bollMult: 'Multiplicador de desviación BOLL',
    macdFast: 'MACD rápido',
    macdSlow: 'MACD lento',
    macdSignal: 'Señal MACD',
    kdjN: 'KDJ N',
    kdjM1: 'KDJ M1',
    kdjM2: 'KDJ M2',
    rsiPeriod: 'Período RSI',
    wrPeriod: 'Período WR',
    obvMaPeriod: 'Suavizado OBV (1=crudo)',
    volMaPeriod: 'Período MA de VOL (1=off)',
    atrPeriod: 'Período ATR',
    dmiPeriod: 'Período DMI',
    cciPeriod: 'Período CCI',
    psyPeriod: 'Período PSY',
    stochK: 'Período STOCH %K',
    stochSmooth: 'Suavizado STOCH',
    stochD: 'Período STOCH %D',
    rocPeriod: 'Período ROC',
    momPeriod: 'Período MOM',
    bbwPeriod: 'Período BBW',
    bbwMult: 'Múltiplo de desviación BBW',
    stPeriod: 'Período ST',
    stMult: 'Multiplicador ST',
    sarAfStart: 'AF inicial SAR',
    sarAfStep: 'Paso AF SAR',
    sarAfMax: 'AF máx SAR',
    ichimokuTenkan: 'Período Tenkan',
    ichimokuKijun: 'Período Kijun',
    ichimokuSpanB: 'Período Senkou B',
    ichimokuDisplacement: 'Desplazamiento',
    sar: 'SAR',
    ichimoku: 'Ichimoku',
    mfiPeriod: 'Período MFI',
    trixPeriod: 'Período TRIX',
    dpoPeriod: 'Período DPO',
    vortexPeriod: 'Período Vortex',
    aoFast: 'AO rápido',
    aoSlow: 'AO lento',
    cmfPeriod: 'Período CMF',
    donchianPeriod: 'Período del canal Donchian',
    aroonPeriod: 'Período Aroon',
    aroon: 'Aroon',
    reset: 'Restablecer valores predeterminados',
    preset: 'Ajustes preestablecidos',
    presetPlaceholder: 'Cargar ajustes…',
    presetSave: 'Guardar',
    presetSaved: 'Guardado',
    presetName: 'Nombre del ajuste',
    exportParams: 'Exportar parámetros',
    importParams: 'Importar parámetros',
    importParamsOk: 'Parámetros importados',
    lineColors: 'Colores de línea',
    lineColorFor: 'Color de línea de {name}',
    lineColorReset: 'Restablecer al predeterminado',
  },
  stats: {
    lastPrice: 'Último precio',
    change24h: 'Cambio 24h',
    high24h: 'Máx 24h',
    low24h: 'Mín 24h',
    volume24h: 'Vol 24h',
    fundingRate: 'Tasa de financiación',
    fundingNext: 'hasta el próximo asentamiento',
    openInterest: 'Interés abierto',
    markPrice: 'Precio de marca',
    countdown: 'Cierre en',
    marketPerp: 'Perpetuo',
    marketSpot: 'Spot',
    volumeSurge: 'Volumen',
    volumeSurgeTitle: 'Volumen actual ÷ media de 20 velas ≥ 3× se marca como pico de volumen',
    gapHealthy: 'Datos completos',
    gapPartial: 'Algunas lagunas',
    gapDegraded: 'Lagunas de datos: {n}',
    dataLatency: 'Retardo de datos',
    dataLatencyTitle: 'El frame en vivo va retrasado más de 5s — los datos pueden estar detenidos o la red lenta',
        marketType: 'Tipo de mercado',
    health: 'Salud de datos',
    summary: 'Resumen',
    barConfig: 'Elementos de la barra',
  },
  marketList: {
    title: 'Mercados',
    addFavorite: 'Añadir favorito',
    removeFavorite: 'Quitar favorito',
    pair: 'Par',
    lastPrice: 'Último',
    change24h: 'Cambio 24h',
    volume: 'Volumen',
    refresh: 'Actualizar',
    loading: 'Cargando…',
    empty: 'Sin datos de mercado',
    collapse: 'Plegar',
    expand: 'Expandir',
    sortTitle: 'Haz clic para ordenar',
    stale: 'Error al actualizar, mostrando caché',
    searchPlaceholder: 'Filtrar símbolos…',
    tabAll: 'Todos',
    tabFavorites: 'Favoritos',
    tabRank: 'Ranking',
    rankChange: 'Subidas',
    rankVolume: 'Volumen',
    rotate: 'Rotación automática',
    rotateTitle: 'Cicla el símbolo actual por la lista visible cada 5s',
    favoritesEmpty: 'Sin favoritos: toca ★ en el selector de símbolos para añadir',
    noMatch: 'Sin coincidencias',
  },

  position: {
    title: 'Posición simulada',
    close: 'Cerrar',
    long: 'Largo',
    short: 'Corto',
    hedgeMode: 'Modo cobertura',
    balance: 'Saldo disponible',
    unrealized: 'PnL no realizado',
    qtyShort: 'Cant.',
    noPosition: 'Sin posición',
    closeAll: 'Cerrar todo',
    otherSymbols: 'Otros símbolos',
    levels: 'TP/SL',
    levelBreakeven: 'Stop en el punto de entrada',
    levelClearHint: 'Déjalo vacío para borrar esa línea',
    reduce: 'Reducir',
    reduceQty: 'Cantidad a reducir',
    reduceErr: 'La cantidad debe ser > 0 y no superior a la posición',
    levelTrail: 'Stop de seguimiento',
    levelTrailHint: 'El stop avanza con el último precio manteniendo una distancia del t% (invertido para cortos) y nunca se afloja',
    levelErrInvalid: 'El take-profit y el stop-loss deben estar al lado correcto de la entrada',
    levelErrCrossed: 'El take-profit y el stop-loss no pueden coincidir',
    switch: 'Cambiar',
    entry: 'Entrada',
    market: 'Mercado',
    quantity: 'Cant.',
    tpPct: 'TP%',
    slPct: 'SL%',
    tpLine: 'Línea TP',
    tpPrice: 'Precio TP',
    slPrice: 'Precio SL',
    levelModePct: 'Porcentaje',
    levelModePrice: 'Precio',
    slLine: 'Línea SL',
    floatingPnl: 'PnL flotante',
    open: 'Abrir',
    lineEntry: 'Entrada',
    lineTp: 'TP',
    lineSl: 'SL',
    margin: 'Margen',
    leverage: 'Apalancamiento',
    liqPrice: 'Precio de liquidación',
    marginRate: 'Tasa de margen',
    liqWarn: 'Advertencia de liquidación',
    liqCritical: 'Liquidación crítica',
    reverse: 'Invertir',
    todayPnl: 'PnL de hoy',
  },
  alert: {
    history: 'Historial de alertas',
    sound: 'Sonido de alerta',
    clearHistory: 'Borrar historial',
    title: 'Alerta de precio · {symbol}',
    granted: 'Notificaciones activadas',
    unsupported: 'Notificaciones no compatibles',
    enable: 'Activar',
    above: 'Precio ≥',
    below: 'Precio ≤',
    add: 'Añadir alerta',
    invalid: 'Ingrese precio válido (positivo)',
    none: 'Sin alertas',
    triggered: 'Disparada',
    reset: 'Restablecer',
    notifyTitle: 'Kline Buty · Alerta de precio',
    notifyAbove: '{symbol} alcanzó {price}',
    notifyBelow: '{symbol} cayó por debajo de {price}',
    repeat: 'Alerta recurrente',
    soundPreview: 'Probar sonido',
    repeatInterval: 'Intervalo',
    minutes: 'min',
    group: 'Grupo',
    groupPlaceholder: 'p. ej. tendencia',
    sort: 'Ordenar',
    filter: 'Filtrar',
    filterAll: 'Todos',
    filterActive: 'Pendiente',
    sortTime: 'Hora',
    sortPrice: 'Precio',
    sortSymbol: 'Símbolo',
    soundKind: 'Sonido',
    soundBeep: 'Bip',
    soundChime: 'Campanilla',
    soundPing: 'Ping',
    soundLow: 'Grave',
    timeWindow: 'Ventana de tiempo',
    timeFrom: 'Desde',
    timeTo: 'Hasta',
    channel: 'Canal',
    channelBoth: 'Ambos',
    channelSystem: 'Sistema',
    channelWeb: 'En la app',
    note: 'Nota',
    notePlaceholder: 'Nota opcional…',
    expiresAt: 'Expira',
    precision: 'Precisión',
    precisionAuto: 'Auto',
    template: 'Plantilla',
    templateName: 'Nombre',
    saveTemplate: 'Guardar',
    applyTemplate: 'Aplicar',
    batch: 'Lote',
    selectAll: 'Todo',
    clearSelection: 'Limpiar',
    enableSelected: 'Activar',
    disableSelected: 'Desactivar',
    deleteSelected: 'Eliminar',
    select: 'Seleccionar',
    expired: 'Expirada',
    clearExpired: 'Limpiar expiradas',
    triggerCount: 'disparos',
    groupToggle: 'Alternar grupo',
    voice: 'Alertas por voz (leer movimientos)',
    adaptive: 'Umbral adaptativo (ATR)',
    quickPct: 'Rápido ±%',
    pending: 'Pendiente',
  },
  tooltip: {
    open: 'Abr',
    high: 'Máx',
    low: 'Mín',
    close: 'Cie',
    volume: 'Vol',
  },
  depth: {
    title: 'Libro de órdenes · {symbol} (en vivo)',
    bid: 'Compra',
    ask: 'Venta',
  },
  orderBook: {
    title: 'Libro de órdenes · {symbol}',
    qty: 'Cant.',
    cum: 'Total',
    group: 'Agrupar',
    spread: 'Diferencial',
    markHint: 'Haz clic para marcar en el gráfico principal (clic de nuevo para borrar); botón de orden rápida a la izquierda',
    refresh: 'Actualizar cartera (reconectar para el último snapshot)',
    bid: 'Compra',
    ask: 'Venta',
    imbalance: 'Desequilibrio del libro: proporción de compra/venta',
  },
  tape: {
    title: 'Operaciones · {symbol}',
    time: 'Hora',
    qty: 'Cant.',
    sideSummary: 'N.º de operaciones comprador/vendedor en la ventana',
    markHint: 'Haz clic para marcar en el gráfico principal (clic de nuevo para borrar)',
    filterAll: 'Todas',
    filterBuy: 'Comprador',
    filterSell: 'Vendedor',
    filterSideHint: 'Filtra por la dirección de la operación (comprador / vendedor)',
    bigOrder: 'Grandes',
    bigOrderHint: 'Muestra solo las operaciones cuyo tamaño alcanza un múltiplo de la media de la ventana',
    emptyFilter: 'Ninguna operación coincide con el filtro',
  },
  trade: {
    buy: "Comprar",
    sell: "Vender",
    market: 'Mercado',
    limit: 'Límite',
    limitHint: 'Las órdenes límite se ejecutan al precio vigente al tocarse (mejora de precio sobre tu límite), sin deslizamiento',
    makerFee: 'Comisión maker %',
    pendingTitle: 'Órdenes abiertas',
    pendingEmpty: 'Sin órdenes límite abiertas',
    cancelOrder: 'Cancelar orden',
    editOrder: 'Modificar',
    editErr: 'El precio y la cantidad deben ser positivos',
    filledToast: 'Orden límite ejecutada · {symbol} {qty} @ {price}',
    cancelledToast: 'Saldo insuficiente, orden cancelada · {symbol}',
    tpslToast: '{symbol} alcanzó {reason}: cerrado a {price}',
    tpslToastMulti: '{symbol} alcanzó {reason}: {count} posiciones cerradas a {price}',
    tpWord: 'toma de ganancias',
    slWord: 'stop de pérdidas',
    tooManyOrders: 'Demasiadas órdenes abiertas para este símbolo (máx. {max})',
    stats: 'Estadísticas',
    winRate: 'Ratio de victorias',
    totalPnl: 'PnL total',
    profitFactor: 'Factor de beneficio',
    avgWin: 'Ganancia media',
    avgLoss: 'Pérdida media',
    settings: 'Ajustes',
    feeRate: 'Comisión taker %',
    slippage: 'Deslizamiento %',
    target: 'Objetivo de ganancia',
    targetHit: 'Objetivo alcanzado',
    detail: 'Detalle',
    notional: 'Nocional',
    feeRateDetail: 'Tarifa',
    feeDetail: 'Comisión',
    grossPnl: 'PnL bruto',
    netPnl: 'PnL neto',
    maxDrawdown: 'Retroceso máximo',
    drawdown: 'Retroceso',
    pnlBars: 'PnL por operación',
    filterSymbol: 'Símbolo',
    filterSide: 'Lado',
    filterQuery: 'Buscar precio…',
    filterEmpty: 'Sin operaciones que coincidan',
    all: 'Todos',
    dailyCount: 'Operaciones',
    dailyPnl: 'PnL del día',
    weekPnl: 'PnL de la semana',
    monthPnl: 'PnL del mes',
    period: 'PnL del período',
    locate: 'Ver en gráfico',
    bySymbol: 'Por símbolo',
  },
  paper: { title: 'Historial de operaciones', empty: 'Sin operaciones: abre una posición y se registrará aquí', open: 'Abrir', close: 'Cerrar', long: 'Largo', short: 'Corto', clear: 'Borrar', export: 'Exportar CSV', reset: 'Restablecer cuenta', resetConfirm: '¿Restablecer la cuenta simulada? El saldo vuelve a 10,000 y se borran todas las operaciones.', exportEquity: 'Exportar curva', equity: 'Curva de capital', exportJson: 'Exportar JSON', importJson: 'Importar JSON', importDone: 'Importado', importFail: 'Importación fallida: archivo no válido', snapshot: 'Instantáneas de cuenta', snapshotName: 'Nombre', saveSnapshot: 'Guardar', loadSnapshot: 'Cargar', deleteSnapshot: 'Borrar' },
  ctx: { copyPrice: 'Copiar precio', copyOhlc: 'Copiar OHLC', addAlert: 'Añadir alerta', limitBuy: 'Compra límite aquí', limitSell: 'Venta límite aquí', clearDrawings: 'Borrar dibujos', confirmClear: '¿Borrar todos los dibujos de este símbolo?', copied: 'Copiado' },
  pull: { pull: 'Tira para actualizar', release: 'Suelta para actualizar', refreshing: 'Actualizando…' },
  panelState: { loading: 'Cargando…', error: 'Error al cargar', empty: 'Sin datos', retry: 'Reintentar' },
  quickOrder: {
    balance: 'Saldo disponible',
    insufficient: 'Saldo insuficiente: se requiere nominal + comisión como margen',
    bid: 'Compra',
    ask: 'Venta',
    title: "Orden rápida",
    price: "Precio",
    qty: "Cantidad",
    notional: "Importe estimado",
    fill: "Precio de ejecución estimado",
    fee: "Comisión (0.1%)",
    total: "Total",
    confirm: "Colocar orden",
    hint: "Sobrescribe la posición simulada actual · Enter confirmar / Esc cerrar",
    qtyPreset: "Cant. rápida",
    customPct: "Personalizado %",
    apply: "Aplicar",
    attachHint: 'TP/SL adjuntos a la orden (opcional): se aplican en cuanto se ejecuta',
    attachErr: 'El take-profit/stop-loss debe estar al lado correcto del precio límite: en compras TP arriba y SL abajo; en ventas al revés',
    riskReward: 'Riesgo/beneficio {ratio} : 1',
  },
  volumeProfile: {
    title: 'Perfil de volumen (últimas 300) · {symbol}',
    poc: 'POC ≈ {price}',
    bidVol: 'Vol compra',
    askVol: 'Vol venta',
    pocLabel: 'POC',
  },
  offline: {
    text: 'Red desconectada; actualizaciones en pausa, se reconectará automáticamente',
  },
  errorBoundary: {
    title: 'Error al renderizar el gráfico',
  },
  shortcuts: {
    title: 'Atajos de teclado',
    openSearch: 'Abrir búsqueda de símbolo: {key}',
    period: 'Cambiar período: [ / ]',
    periodPrev: 'Período anterior',
    periodNext: 'Período siguiente',
    layoutKeys: '1 / 2 / 3',
    help: 'Mostrar/ocultar atajos',
    copyDrawing: 'Copiar dibujo',
    pasteDrawing: 'Pegar dibujo',
    configTitle: 'Configurar atajos',
    recording: 'Pulsa la nueva tecla…',
    clickToChange: 'Haz clic y pulsa tecla',
    cancel: 'Cancelar grabación',
    configHint: 'Haz clic en un elemento y pulsa una tecla',
    cycleLang: 'Cambiar idioma',
    toggleAlerts: 'Abrir alertas',
    print: 'Imprimir tarjeta',
    reset: 'Restablecer',
    conflict: 'Conflicto de atajos: {names} comparten la misma tecla',
    layout: 'Diseño: 1 / 2 / 3 (único / par / cuádruple)',
    cycleMain: 'Ciclar indicador principal: M',
    cycleSub: 'Ciclar indicador secundario: N',
    fullscreen: 'Alternar pantalla completa: F',
    replay: 'Reproducir / pausar replay: Espacio',
    replayStep: 'Avanzar replay vela a vela: ← / →',
    replaySpeed: 'Velocidad de replay: ↑ / ↓',
    deleteDrawing: 'Eliminar dibujo seleccionado: Supr',
    escape: 'Cancelar / cerrar: Esc',
    hint: 'Mostrar / ocultar atajos: ?',
    groupNav: 'Navegación',
    groupIndicators: 'Indicadores',
    groupReplay: 'Repetición',
    groupDrawings: 'Dibujos',
    filter: 'Filtrar atajos…',
    noMatch: 'Sin atajos coincidentes',
  },
  app: {
    titleZh: 'Kline Buty · 实时 K 线',
    titleEn: 'Kline Buty · Live Candles',
    titleJa: 'Kline Buty · リアルタイム K 線',
    titleKo: 'Kline Buty · 실시간 K 라인',
    titleEs: 'Kline Buty · Velas en vivo',
  },
}

export const DICTIONARIES: Record<Lang, Messages> = { 'zh-CN': zh, en, ja, ko, es }

export const DEFAULT_LANG: Lang = 'zh-CN'

/** 语言 → Intl locale（日期/数字格式化用） */
export function localeFor(lang: Lang): string {
  switch (lang) {
    case 'zh-CN':
      return 'zh-CN'
    case 'en':
      return 'en-US'
    case 'ja':
      return 'ja-JP'
    case 'ko':
      return 'ko-KR'
    case 'es':
      return 'es-ES'
  }
}

/** 图表渲染层（adapter）所需的本地化标签：文本标注默认文案 + 仓位线标签 */
export interface ChartLabels {
  defaultText: string
  entry: string
  tp: string
  sl: string
  /** 量度标签的 K 线根数单位（{bars} 占位） */
  measureBars: string
  /** 风险回报比标签（{ratio} 占位） */
  rrRatio: string
  /** 图表水印文案（免责声明） */
  watermark: string
}

export function chartLabelsFor(lang: Lang): ChartLabels {
  const m = DICTIONARIES[lang]
  return {
    defaultText: m.drawing.defaultText,
    entry: m.position.lineEntry,
    tp: m.position.lineTp,
    sl: m.position.lineSl,
    measureBars: m.drawing.measureBars,
    rrRatio: m.drawing.rrRatio,
    watermark: m.disclaimer.watermark,
  }
}


/** 页面标题：按当前语言从 app 区块取对应 title 键 */
const TITLE_KEYS: Record<Lang, 'titleZh' | 'titleEn' | 'titleJa' | 'titleKo' | 'titleEs'> = {
  'zh-CN': 'titleZh',
  en: 'titleEn',
  ja: 'titleJa',
  ko: 'titleKo',
  es: 'titleEs',
}

export function titleFor(lang: Lang): string {
  return DICTIONARIES[lang].app[TITLE_KEYS[lang]]
}
