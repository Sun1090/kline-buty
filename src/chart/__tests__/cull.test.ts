...
describe('nextCullWindow 窗口迁移判定（A2 视野塌缩根因护栏）', () => {
  ...
  it('视角比装载窗口更宽（窗口左边被甩在外面）→ 必须迁移回覆盖视角', () => {
    // 现场：perf=20000（数据源截到 3000 根）首帧按窄视角建了 501 根窗口，布局完成后
    // fitContent 的视角横跨全部 3000 根 —— 判定必须看到「视角越出窗口左边」并迁回 {0,3000}，
    // 否则屏幕上一片空白、十字光标取不到数据（此前 gFrom 被夹成 base 看起来「已覆盖」）
    const cur = { start: 2499, end: 3000 }
    const r = next(cur, { from: 0, to: 2999 }, 3000, cur)!
    expect(r).not.toBe(cur)
