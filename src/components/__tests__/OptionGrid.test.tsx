...
/** 网格容器 = 第一个按钮的父级（渲染为纯 div，不含 listbox role，保持按钮语义） */
const gridOf = (btns: HTMLElement[]) => btns[0].parentElement!
...
describe('OptionGrid 方向键导航', () => {
  ...
  it('ArrowRight 聚焦下一项', () => {
    ...
    const btns = screen.getAllByRole('button')
    btns[0].focus()
    fireEvent.keyDown(gridOf(btns), { key: 'ArrowRight' })
  ...
  it('ArrowLeft 聚焦上一项', () => {
    ...
    const btns = screen.getAllByRole('button')
    btns[2].focus()
    fireEvent.keyDown(gridOf(btns), { key: 'ArrowLeft' })
    expect(document.activeElement).toBe(btns[1])
  })
  ...
  it('ArrowDown 聚焦下一项（同 ArrowRight 方向）', () => {
    ...
    const btns = screen.getAllByRole('button')
    btns[0].focus()
    fireEvent.keyDown(gridOf(btns), { key: 'ArrowDown' })
    expect(document.activeElement).toBe(btns[1])
  })
  ...
  it('ArrowUp 聚焦上一项（同 ArrowLeft 方向）', () => {
    ...
    const btns = screen.getAllByRole('button')
    btns[3].focus()
    fireEvent.keyDown(gridOf(btns), { key: 'ArrowUp' })
  ...
  it('Home 聚焦首项', () => {
    ...
    const btns = screen.getAllByRole('button')
    btns[3].focus()
    fireEvent.keyDown(gridOf(btns), { key: 'Home' })
    expect(document.activeElement).toBe(btns[0])
  })
  ...
  it('End 聚焦末项', () => {
    ...
    const btns = screen.getAllByRole('button')
    btns[0].focus()
    fireEvent.keyDown(gridOf(btns), { key: 'End' })
    expect(document.activeElement).toBe(btns[3])
  })
  ...
  it('aria-pressed 与实际选中项同步（保持 button 语义）', () => {
    render(<OptionGrid options={OPTIONS} value="c" onPick={vi.fn()} label={(o) => o.label!} />)
    const btns = screen.getAllByRole('button')
  ...
  it('focus 不在网格按钮上时按键不拦截', () => {
    const onPick = vi.fn()
    render(<OptionGrid options={OPTIONS} value="a" onPick={onPick} label={(o) => o.label!} />)
    const grid = gridOf(screen.getAllByRole('button'))
