...
describe('DesktopHeader（桌面顶栏）', () => {
  ...
  it('I5 编辑态：textarea + 字号 +/- + 确定/取消', () => {
    const onConfirmText = vi.fn()
    const onCancelText = vi.fn()
    const onTextFontSizeChange = vi.fn()
    setup({ editingTextId: 'd1', textFontSize: 14, onConfirmText, onCancelText, onTextFontSizeChange })
    expect(screen.getByPlaceholderText('文本内容')).toBeDefined()
    fireEvent.click(screen.getByTestId('text-font-inc'))
    expect(onTextFontSizeChange).toHaveBeenCalledWith(16)
    fireEvent.click(screen.getByTestId('text-font-dec'))
    expect(onTextFontSizeChange).toHaveBeenCalledWith(12)
    fireEvent.click(screen.getByTestId('text-confirm'))
    expect(onConfirmText).toHaveBeenCalled()
    fireEvent.click(screen.getByText('取消'))
    expect(onCancelText).toHaveBeenCalled()
  })

  it('画线选中态：显示「改字/删除」行内按钮并触发回调', () => {
    const onEditSelectedText = vi.fn()
    const onDeleteSelectedDrawing = vi.fn()
    setup({ drawingSelected: true, onEditSelectedText, onDeleteSelectedDrawing })
    fireEvent.click(screen.getByText('改字'))
    expect(onEditSelectedText).toHaveBeenCalled()
    fireEvent.click(screen.getByText('删除'))
    expect(onDeleteSelectedDrawing).toHaveBeenCalled()
  })

  it('画线面板：吸附/备注开关 aria-pressed + 打开图层面板', () => {
    const onToggleDrawingSnap = vi.fn()
    const onToggleNotesHidden = vi.fn()
    setup({ onToggleDrawingSnap, onToggleNotesHidden })
    fireEvent.click(screen.getByTestId('drawing-toggle'))
    const snap = screen.getByTestId('drawing-snap-toggle')
    expect(snap.getAttribute('aria-pressed')).toBe('false') // drawingSnap='off'
    fireEvent.click(snap)
    expect(onToggleDrawingSnap).toHaveBeenCalled()
    fireEvent.click(screen.getByTestId('drawing-note-toggle'))
