...
export function DesktopHeader(props: DesktopHeaderProps) {
  ...
  const moreToggles: { label: string; active: boolean; onToggle: () => void; title?: string; disabled?: boolean; badge?: number }[] = [
    ...
    { label: t('snap.title'), active: props.galleryActive === true, onToggle: closeMore(props.onToggleGallery ?? (() => {})), title: t('snap.title') },
      ...
      {menu === 'more' && (
        <div style={PANEL_STYLE} data-testid="desktop-more-panel">
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: 6 }}>
            ...
            <PanelButton
              onClick={props.onCycleFontScale}
              title={t('layout.fontScaleTitle')}
              ariaLabel={`${t('layout.fontScaleTitle')}: ${Math.round(props.fontScale * 100)}%`}
              active={props.fontScale !== 1}
              testId="fontscale-toggle"
            >
              {t('layout.fontScale')} {Math.round(props.fontScale * 100)}%
            </PanelButton>
            <PanelButton
