...
- **交易流水过滤**：交易流水面板多品种时显示过滤行——品种下拉 + 方向下拉 + 关键词搜索
  ...
  空匹配显示提示）；五语 i18n 新增 `trade.filterSymbol/filterSide/filterQuery/filterEmpty/all`；
  单测 +13；全量 unit **1628** + chromium E2E 14/14 全绿
- **逐笔盈亏条形图**：交易流水面板权益曲线下方新增 `PnlBars`（`src/components/PnlBars.tsx`）——
  已平仓记录按时间升序排布，盈利向上 / 亏损向下，零轴 + 按最大 |pnl| 归一化；
  `src/trade/perf.ts` 新增 `pnlBars` 纯函数；五语 i18n 新增 `trade.pnlBars`；单测 +10
