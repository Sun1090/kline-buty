...
- [x] 顶栏紧凑化 + 窄屏防横向滚动条：桌面端「画线/更多」等低频功能折叠进「更多」面板、顶栏保持重要一行；移动端周期条改为**单行横向滚动**（nowrap + overflowX:auto，隐藏滚动条，不换行占两行），SymbolPicker 可收缩省略、状态文字 maxWidth 42% 省略——320/390px 实测 header 136→109px、无横向滚动条（document.scrollWidth===innerWidth）；单测 +3（PeriodBar 桌面换行/移动端单行横滚/点击回调）、E2E +1（周期条单行不换行 + 隐藏滚动条 + 无横向滚动条 + 末尾周期可点）
